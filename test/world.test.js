      ...
      return {
        ...
        async json() {
          return {
            choices: [{
              message: {
                content: 'Сцена держится в пределах видимого, без уточнения времени суток.'
              }
            }]
          };
        }
...
test('intent classifier derives explicit focus for checks', () => {
  ...
  assert.equal(intent.focus, 'ranged_combat');
});
...
test('semantic audit contract rejects concerns when pass=true', () => {
  const invalid = validateSemanticAudit({
    version: 1,
    schema: 'semantic_audit',
    pass: true,
    concerns: ['лишнее замечание'],
    evidence: ['ok']
  });
...
test('semantic audit parser adds technical fallback evidence when pass=true and model returns none', () => {
  ...
  assert.deepEqual(parsed.evidence, ['semantic audit passed: no blocking concerns found']);
});

test('semantic audit parser never adds fallback evidence when pass=false', () => {
  const parsed = parseSemanticAuditResponse(JSON.stringify({
    version: 1,
    schema: 'semantic_audit',
    pass: false,
    concerns: ['нет основания'],
    evidence: []
  }));
...
test('actor profile audit failure fails with a real reason after repair attempts', async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => generateActorProfiles(world, process.env, { onStage: (stage) => stages.push(stage) }),
      /Unable to generate actor profiles: stage_failed \/ needs_manual_review: нет причины быть здесь \| evidence: audit\./
    );
    assert.equal(actorAuditCalls, 4);
    assert.equal(actorRepairCalls, 2);
    assert.ok(stages.some((stage) => stage.model_tier === 'senior_pro_thinking_max' && stage.repair_attempt_index === 2));
  } finally {
    globalThis.fetch = originalFetch;
    globalThis.setTimeout = originalSetTimeout;
  }
...
test('location profile audit failure fails with a real reason after repair attempts', async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      /Unable to generate location profiles: stage_failed \/ needs_manual_review: не хватает причины быть здесь \| evidence: audit path exercised\./
    );
    assert.equal(locationAuditCalls, 4);
    assert.equal(locationRepairCalls, 2);
  } finally {
    globalThis.fetch = originalFetch;
    globalThis.setTimeout = originalSetTimeout;
  }
...
test('location profiles stop on blocked_by_missing_approved_fact instead of inventing a route', async () => {
  const originalFetch = globalThis.fetch;
  const originalSetTimeout = globalThis.setTimeout;
  globalThis.setTimeout = (fn, _ms, ...args) => originalSetTimeout(fn, 0, ...args);
  const stages = [];
  ...
  globalThis.fetch = async (_url, options) => {
    ...
    if (/location_semantic_dossier/i.test(userText)) {
      return {
        ok: true,
        async json() {
          return { choices: [{ message: { content: 'Текущая точка и ближайшие связи описаны без лишней конкретики.' } }] };
        }
      };
    }
    ...
    if (/semantic audit/i.test(systemText) || /отдельный проверяющий/i.test(systemText)) {
      return {
        ...
        async json() {
          return {
            choices: [{ message: { content: JSON.stringify({ version: 1, schema: 'semantic_audit', pass: true, concerns: [], evidence: [] }) } }]
          };
        }
      };
    }

    if (/(LocationProfileShaper|SemanticDataShaper)/i.test(systemText) && /location_profiles/i.test(userText)) {
      return {
        ok: true,
        async json() {
          return {
            choices: [{
              message: {
                content: JSON.stringify({
                  status: 'blocked_by_missing_approved_fact',
                  missing_fact_type: 'route',
                  missing_fact_id: null,
                  missing_fact_description: 'No approved route/edge from current location to requested target',
                  allowed_next_step: 'return_to_candidate_selection_or_materialization_stage'
                })
              }
            }]
          };
  ...
  try {
    const world = createWorldState({ startText: 'проверка blocked location outcome' });

    await assert.rejects(
      () => generateLocationProfiles(world, process.env, { onStage: (stage) => stages.push(stage) }),
      /Unable to generate location profiles: blocked_by_missing_approved_fact: route - No approved route\/edge from current location to requested target -> return_to_candidate_selection_or_materialization_stage/
    );
    assert.ok(stages.some((stage) => stage.terminal_status === 'blocked_by_missing_approved_fact'));
  } finally {
    globalThis.fetch = originalFetch;
    globalThis.setTimeout = originalSetTimeout;
  }
...
test('actor profiles stop on blocked_by_missing_approved_fact instead of inventing a new NPC', async () => {
  const originalFetch = globalThis.fetch;
  const originalSetTimeout = globalThis.setTimeout;
  globalThis.setTimeout = (fn, _ms, ...args) => originalSetTimeout(fn, 0, ...args);
  ...
  globalThis.fetch = async (_url, options) => {
    ...
    if (/actor_semantic_dossier/i.test(userText)) {
      return {
        ok: true,
        async json() {
          return { choices: [{ message: { content: 'В сцене виден только один человек без добавления новых identity.' } }] };
        }
      };
    }
    ...
    if (/(ActorProfileShaper|SemanticDataShaper)/i.test(systemText) && /actor_profiles/i.test(userText)) {
      return {
        ok: true,
        async json() {
          return {
            choices: [{
              message: {
                content: JSON.stringify({
                  status: 'blocked_by_missing_approved_fact',
                  missing_fact_type: 'npc',
                  missing_fact_id: null,
                  missing_fact_description: 'No approved NPC candidate or placed NPC matches the requested interaction target',
                  allowed_next_step: 'return_to_candidate_selection_or_materialization_stage'
                })
              }
            }]
  ...
  try {
    const world = createWorldState({ startText: 'проверка blocked actor outcome' });

    await assert.rejects(
      () => generateActorProfiles(world),
      /Unable to generate actor profiles: blocked_by_missing_approved_fact: npc - No approved NPC candidate or placed NPC matches the requested interaction target -> return_to_candidate_selection_or_materialization_stage/
    );
  } finally {
    globalThis.fetch = originalFetch;
    globalThis.setTimeout = originalSetTimeout;
...
test('fresh world helper builds the layered start instead of a raw seed', async () => {
  ...
  globalThis.fetch = async (_url, options) => {
    ...
    if (/narrator/i.test(stageText) || /UI-прозы/i.test(stageText) || /prose для UI/i.test(stageText)) {
      if (/semantic_audit/i.test(systemText) || /Narrator audit/i.test(stageText)) {
        return {
          ok: true,
          async json() {
            return {
    ...
    if (/MasterNarrativeShaper/i.test(stageText) || /"schema"\s*:\s*"master_narrative"/i.test(userText)) {
      return {
        ok: true,
        async json() {
          return {
    ...
    if (/MasterNarrativeShaper|master_narrative/i.test(systemText) || /"schema"\s*:\s*"master_narrative"/i.test(userText)) {
      return {
        ok: true,
        async json() {
          return {
...
test('party turn pipeline applies time consequences before building the next visible screen', async () => {
  const world = createWorldState({ startText: 'двор у переправы', clock: { day: 1, hour: 8, minute: 0 } });
  const result = await handlePlayerInput(world, 'жду час');

  assert.equal(world.clock.hour, 9);
  assert.equal(world.partyRuntimeState?.current_turn_number, 1);
  assert.equal(world.partyScreenPayload?.partyTurnScreen?.schema, 'party_turn_screen');
  assert.match(String(result.text ?? ''), /время|ожидани|жд/u);
  assert.match(String(world.partyScreenPayload?.partyTurnScreen?.main_prose ?? ''), /время|ожидани|жд/u);
});
...
test('combat checks use explicit intent focus instead of raw text heuristics', () => {
  ...
  assert.equal(check.profile.skillKey, 'ranged_combat');
