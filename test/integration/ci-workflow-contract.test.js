...
test('world_base PostgreSQL gate tracks the 208-table schema and grants every table read-only', async () => {
  const workflow = await readFile(resolve(process.cwd(), '.github/workflows/test.yml'), 'utf8');

  assert.doesNotMatch(workflow, /test "\$table_count" -eq 62/u);
  assert.match(workflow, /test "\$table_count" -eq 208/u);
