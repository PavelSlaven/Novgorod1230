...
for (const symbol of ['buildSpatialV3SourceExtract', 'readV2WorldSource', 'readV2PartySource', 'V2_WORLD_SOURCE_TABLES', 'V2_PARTY_SOURCE_TABLES', 'requireInventorySourceBindings', 'WORLD_CHAIN', 'PARTY_CHAIN', 'buildSpatialV3MigrationInventory', 'buildSpatialV3MigrationInventoryFromSource', 'buildSpatialV3MigrationCoverageArtifact', 'classifyV2PartyG5', 'classifyV2Journey', 'adaptV2PartyEntities', 'constructP14JourneyRows', 'validateSpatialV3MigrationAcceptance', 'applySpatialV3WorldMigration', 'applySpatialV3PartyMigration']) if (!tool.includes(symbol)) throw new Error(`P24 missing ${symbol}`);
for (const code of ['migration_hard_gap', 'migration_g5_hard_gap', 'journey_migration_gap', 'migration_source_inventory_coverage_gap', 'migration_party_scope_gap']) if (!tool.includes(code)) throw new Error(`P24 missing typed gap ${code}`);
