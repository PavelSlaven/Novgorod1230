...
export async function compileP12V11PhysicalRows({ root = ROOT } = {}) {
  ...
  const [nodes, profiles, candidates, orientations, exitRules, connectionProfiles, connectionBindings, entryBindings, directions, traversals, exits, routes, points, environments, costs, rechecks, segments, contexts, endpoints, dependencies] = await Promise.all([
  ...
  put('spatial_v3_approved_physical_source_pairs', [...sourcePairRefs].map((ref) => { const [id, v] = pin(ref); return { id, version: v, provenance_ref: PROVENANCE, source_payload_sha256: digest(ref), status: 'approved' }; }));
  ...
  const kindOf = (id) => id.startsWith('cg5bindv3__') ? 'canonical_g5_connection_binding' : id.startsWith('cprofv3__') ? 'canonical_g5_connection_profile' : id.startsWith('g4entryv3__') ? 'g4_entry_endpoint_binding' : id.startsWith('dctxv3__') ? 'topological_direction_context' : id.startsWith('g4exitv3__') ? 'g4_directional_exit' : id.startsWith('wrpointv3__') ? 'world_route_point' : id.startsWith('wrsegv3__') ? 'world_route_segment' : id.startsWith('wrebv3__') ? 'world_route_endpoint_binding' : id.startsWith('wrv3__') ? 'world_route' : id.startsWith('g4v3__') || id.startsWith('cg5v3__') ? 'spatial_node' : id.startsWith('smpv3__') ? 'scene_materialization_profile' : id.startsWith('env.') ? 'transition_environment_profile' : id.startsWith('cost.') ? 'movement_method_cost_profile' : id.startsWith('recheck.') ? 'dynamic_recheck_policy' : 'external_dependency';
  const dependencyRows = dependencies.map((row) => { const [source_entity_id, source_version] = pin(row.source_ref); const [target_entity_id, target_version] = pin(row.target_ref); return { source_entity_kind: kindOf(source_entity_id), source_entity_id, source_version, world_revision_id: WORLD_REVISION, dependency_role: row.dependency_role, target_entity_kind: kindOf(target_entity_id), target_entity_id, target_version, canonical_ordinal: row.canonical_ordinal, provenance_ref: row.provenance_ref }; });
