...
active player-safe marker. Authored profile SHA-пинит source garment, owned
knife tool, полный mechanics snapshot, `generic_check` и semantic activity.
Model работает до SQL transaction и возвращает только qualitative
`preserve_source | no_useful_result`; code-owned item owner рассчитывает
identity/mechanics, а combined P16 под повторными DB locks/rechecks атомарно
сохраняет physical transition и append-only A1 causal ledger. Current profile
