...
test('parse failure skips semantic repair and preserves responseRaw', async () => {
  let repairCalls = 0;
  ...
  try {
    ...
    const retry = journal.snapshot({ includeRawDetails: true }).find((entry) => entry.repair?.kind === 'parse_repair');
    assert.ok(retry);
    assert.ok(retry.responseRaw);
    assert.match(retry.message, /truncated at maxTokens/i);
  } finally {
    mock.restore();
  }
});
