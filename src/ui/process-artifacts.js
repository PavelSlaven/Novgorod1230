...
export async function saveNewGameProcessArtifact(options = {}) {
  ...
  const diagnostics = buildFailureSummary(journal, {
    ...
    pipelineRuntime: options.pipelineRuntime ?? options.pipeline_runtime ?? options.world?.pipeline_runtime ?? null,
    legacyProviderRuntimeUsed: options.legacyProviderRuntimeUsed
      ?? options.legacy_provider_runtime_used
      ?? options.world?.legacy_provider_runtime_used
      ?? null
  });
  const payload = redactSecrets({
    ...
    pipeline_runtime: options.pipelineRuntime ?? options.pipeline_runtime ?? options.world?.pipeline_runtime ?? null,
    legacy_provider_runtime_used: options.legacyProviderRuntimeUsed
      ?? options.legacy_provider_runtime_used
      ?? options.world?.legacy_provider_runtime_used
      ?? null,
...
export function renderNewGameProcessArtifact(payload) {
  ...
  const summaryItems = [
    ['Статус', statusLabel],
    ['LLM / модель', provider],
    ['Scope', failure.scope ?? 'нет данных'],
    ['Role', failure.roleId ?? 'нет данных'],
    ['Tier', failure.tierId ?? 'нет данных'],
    ['Config hash', failure.configHash ?? 'нет данных'],
    ['Output mode', failure.outputContractMode ?? 'нет данных'],
    ...
    ['Pipeline runtime', failure.pipelineRuntime ?? data.pipeline_runtime ?? 'нет данных'],
    ['Legacy provider used', String(failure.legacyProviderRuntimeUsed ?? data.legacy_provider_runtime_used ?? false)],
    ...
    ['Failed gate', failure.failedGate ?? 'нет'],
    ['First failed gate', failure.firstFailedGate ?? 'нет'],
    ['First invalid artifact', failure.firstInvalidArtifact ?? 'нет'],
    ['First regression stage', failure.firstRegressionStage ?? 'нет'],
    ['Recovery class', failure.recoveryClass ?? 'нет'],
    ['Repair target', failure.repairTargetStage ?? 'нет'],
    ['Rerun from', failure.rerunFromStage ?? 'нет'],
    ['Forbidden local fix', failure.forbiddenLocalFix ?? 'нет'],
    ['Repair attempt', failure.repairAttemptIndex ?? 'нет'],
    ['Model tier', failure.modelTier ?? 'нет'],
    ['Terminal status', failure.terminalStatus ?? 'нет'],
  ...
  const fullSnapshot = redactSecrets({
    ...
    pipeline_runtime: data.pipeline_runtime ?? null,
    legacy_provider_runtime_used: data.legacy_provider_runtime_used ?? null,
    state: data.state,
    world: data.world,
    diagnostics: failure,
...
function normalizeArtifactPayload(payload = {}) {
  ...
  return {
    ...
    pipeline_runtime: payload.pipeline_runtime ?? null,
    legacy_provider_runtime_used: payload.legacy_provider_runtime_used ?? null,
...
function normalizeStateSummary(state = {}) {
  ...
  return {
    provider: state.provider && typeof state.provider === 'object'
      ...
      : null,
    pipeline_runtime: state.pipeline_runtime ?? null,
    legacy_provider_runtime_used: state.legacy_provider_runtime_used ?? null
  };
}
...
function normalizeWorldSummary(world = {}) {
  ...
  return {
    ...
    pipeline_runtime: world.pipeline_runtime ?? null,
    legacy_provider_runtime_used: world.legacy_provider_runtime_used ?? null
  };
}
