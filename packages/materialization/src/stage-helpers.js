...
export function assertG5TemplateBundle(selected, g4Id) {
  ...
  if (selected.materialization_profile.layout_template_id !== selected.layout_template.layout_template_id
    || selected.materialization_profile.world_revision_id !== selected.layout_template.world_revision_id
    || selected.materialization_profile.region_id !== selected.layout_template.region_id) {
    throw new MaterializationError('G5_PROFILE_LAYOUT_MISMATCH', 'Approved materialization profile and layout must share revision and region.');
  }
...
export function indexApproved(values, key, code) {
  const map = new Map();
  for (const value of values ?? []) {
    if (value?.status !== 'approved' || !value[key] || map.has(value[key])) throw new MaterializationError(code, 'Template records must be approved and have unique IDs.');
    map.set(value[key], value);
  }
  return map;
}

export function chooseApprovedCount(rule, random, choices) {
  const width = rule.max_count - rule.min_count + 1;
  const draw = width > 1 ? random.nextUint32() : 0;
  const count = rule.min_count + (width > 1 ? draw % width : 0);
  const candidateDigest = canonicalDigest([{ rule_id: rule.rule_id, min_count: rule.min_count, max_count: rule.max_count }]);
...
export function materializeApprovedItems(candidates, { input, partyId, runId, anchors, kind }) {
  return candidates.filter((candidate) => candidate?.status === 'approved' && candidate?.required === true)
    ...
    .map((candidate, ordinal) => {
      ...
      if (candidate.placement.g5_anchor_id) {
        const anchor = anchors.get(candidate.placement.g5_anchor_id);
        const capability = kind === 'item' ? 'can_hold_item' : 'can_hold_container';
        if (!anchor || anchor.supports?.[capability] !== true) throw new MaterializationError(`${kind.toUpperCase()}_PLACEMENT_INVALID`, `${kind} ${candidateId} references an ineligible anchor.`);
      }
      ...
      return {
        ...
        property_state: structuredClone(candidate.property_state),
        visibility_state: structuredClone(candidate.visibility_state),
        access_state: structuredClone(candidate.access_state),
        risk_state: structuredClone(candidate.risk_state),
        physical_state: structuredClone(candidate.physical_state ?? {}),
        ...(kind === 'container' ? { content_state: structuredClone(candidate.content_state) } : {}),
        hidden_state_projection: structuredClone(candidate.hidden_state_projection ?? null),
        source_trace: structuredClone(candidate.source_trace)
      };
    });
}

export function requireFields(value, keys, code) {
  const missing = keys.filter((key) => value?.[key] == null || value[key] === '');
  if (missing.length > 0) throw new MaterializationError(code, `Missing required approved fields: ${missing.join(', ')}.`, { missing });
}

export function parentScene(input) {
  return { g4_node_id: input?.selected_start_node?.selected_node_chain?.g4_node_id ?? null, selected_place_template_id: input?.selected_start_node?.selected?.selected_place_template_id ?? null };
}

export function assertConnectedG5Graph(nodes, anchors, edges, startAnchorId, requiredExitAnchorIds = []) {
  if (!Array.isArray(nodes) || nodes.length === 0 || !Array.isArray(anchors) || anchors.length === 0) throw new MaterializationError('G5_GRAPH_EMPTY', 'Approved G5 graph requires at least one node and anchor.');
  const nodeIds = new Set(nodes.map((node) => node.g5_minilocation_id));
  const anchorIds = new Set(anchors.map((anchor) => anchor.anchor_id));
  if (!anchorIds.has(startAnchorId)) throw new MaterializationError('G5_START_ANCHOR_UNRESOLVED', 'Start anchor must belong to the materialized graph.');
  for (const anchor of anchors) if (!nodeIds.has(anchor.minilocation_id)) throw new MaterializationError('G5_ANCHOR_PARENT_INVALID', `Anchor ${anchor.anchor_id} references a missing G5 node.`);
  const anchoredNodeIds = new Set(anchors.map((anchor) => anchor.minilocation_id));
  const orphanedNodeIds = [...nodeIds].filter((id) => !anchoredNodeIds.has(id));
  if (orphanedNodeIds.length > 0) throw new MaterializationError('G5_NODE_ORPHANED', 'Every materialized G5 node must contain a reachable anchor.', { orphaned_node_ids: orphanedNodeIds });
  const nodeById = new Map(nodes.map((node) => [node.g5_minilocation_id, node]));
  ...
  for (const edge of edges) {
    if (!anchorIds.has(edge.from_anchor_id) || !anchorIds.has(edge.to_anchor_id) || edge.from_anchor_id === edge.to_anchor_id) throw new MaterializationError('G5_EDGE_REFERENCE_INVALID', `Edge ${edge.edge_id} must connect two different existing anchors.`);
    const from = anchorById.get(edge.from_anchor_id);
    const to = anchorById.get(edge.to_anchor_id);
    if (isTraversable(edge.access) && isTraversable(from.access) && isTraversable(to.access) && isTraversable(nodeById.get(from.minilocation_id)?.access) && isTraversable(nodeById.get(to.minilocation_id)?.access)) {
      adjacency.get(edge.from_anchor_id).add(edge.to_anchor_id);
      adjacency.get(edge.to_anchor_id).add(edge.from_anchor_id);
    }
  }
  const visited = new Set();
  const queue = [startAnchorId];
  while (queue.length) {
  ...
  if (visited.size !== anchorIds.size) throw new MaterializationError('G5_GRAPH_DISCONNECTED', 'Every materialized G5 anchor must be reachable from the approved player start anchor.', { start_anchor_id: startAnchorId, unreachable_anchor_ids: [...anchorIds].filter((id) => !visited.has(id)) });
  const unreachableExits = requiredExitAnchorIds.filter((id) => !visited.has(id));
  if (unreachableExits.length > 0) throw new MaterializationError('G5_EXIT_UNREACHABLE', 'Every required exit must be traversable from the player start anchor.', { unreachable_exit_anchor_ids: unreachableExits });
}

function isTraversable(policy) {
