...
const COMBAT_INTENT_STATUSES = new Set([
  'active', 'completed', 'blocked', 'invalidated', 'no_progress'
]);
...
function validCurrentIntent(value) {
  return exact(value, ['intent_kind','target_refs','status'])
    ...
    && COMBAT_INTENT_STATUSES.has(value.status);
}
