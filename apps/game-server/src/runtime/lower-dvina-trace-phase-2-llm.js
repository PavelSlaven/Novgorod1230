    ...
    const response = await roleRunner.run({
      ...
      messages: [{
        ...
        content: [
          ...
          'Choose only listed result/output/weapon classes. For action production, source_refs are one or more consumed material items, tool_refs are unchanged tools, item_ref is source_refs[0], and target_refs contain every remaining source/tool ref. For preserve_source, item_ref keeps identity and later source_refs are consumed materials; material_extent is null with one source and minor|half|major|whole with additional materials. output_count is zero except for independent_outputs and must not exceed the visible max_new_entities. For an independent output material_extent is whole for full partition and minor|half|major for partial separation. removed_physical_fact_refs may contain only visible fact_ref values made false by this change. Choose only the qualitative extent implied by the attempt and never invent numeric mechanics.',
