...
| `@rus/movement-routes` | `packages/movement-routes` | route availability and requirements; travel cost formulas; traversal request/result shapes | `./src/index.js` | `@rus/contracts`, `@rus/kernel` |
...
| `@rus/party-store` | `packages/party-store` | party persistence contract; idempotency key и committed-result semantics; Stage 25 persistence port | `./src/index.js` | `@rus/contracts`, `@rus/kernel`, `@rus/turn` |
...
| `@rus/runtime-catalog` | `packages/runtime-catalog` | domain catalog pin и typed runtime-catalog errors; exact reconstruction по immutable import membership; проверкой compatible full-world pin и runtime contract | `./src/index.js` | `@rus/materialization`, `@rus/world-base` |
...
| `@rus/turn` | `packages/turn` | контрактом `PlayerTurnInput`; декларативным порядком блоков хода; orchestration context и run summary | `./src/index.js` | `@rus/checks-rng`, `@rus/contracts`, `@rus/kernel`, `@rus/materialization`, `@rus/narration`, `@rus/pipeline-engine`, `@rus/presentation`, `@rus/time-events-history`, `@rus/visibility-knowledge-memory` |
