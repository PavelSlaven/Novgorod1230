...
test('world_base schema expands to exactly 190 unique tables', async () => {
  const result = await inspectWorldBaseSchema({ root: process.cwd() });
  assert.equal(result.schema, 'world_base');
  assert.equal(result.table_count, 190);
  assert.equal(result.table_names.length, 190);
  assert.deepEqual(result.duplicate_table_names, []);
  assert.equal(result.part_files.length, 18);
});
