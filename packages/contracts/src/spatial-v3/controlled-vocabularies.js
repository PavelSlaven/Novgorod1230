...
import registry from '../../../../data/contracts/spatial-v3/controlled-vocabularies.v1.json' with { type: 'json' };
...
export function validateControlledVocabularyRegistry(input = registry) {
  ...
  if (input?.version !== '1.0.0') errors.push('version');
  if (input?.status !== 'approved') errors.push('status');
  if (!Array.isArray(input?.vocabularies) || input.vocabularies.length !== 13) errors.push('vocabulary_count');
  ...
  for (const vocabulary of input?.vocabularies ?? []) {
    ...
    if (vocabulary.open_ended !== false || vocabulary.status !== 'approved' || vocabulary.version !== '1.0.0') errors.push(`not_closed:${vocabulary.pseudo_type}`);
