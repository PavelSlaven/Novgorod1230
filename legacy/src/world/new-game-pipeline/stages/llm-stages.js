import { getNewGameG5PlacementRoleDescriptor, getNewGameKnowledgeHiddenRoleDescriptor, getNewGameVisibleContextRoleDescriptor } from '@rus/llm-runtime';
...
export async function runStage15NpcPlacement(context, options = {}) {
  return runStage15NpcPlacementIsolated(context, withG5PlacementExecutor(options));
}

export async function runStage16ItemPlacement(context, options = {}) {
  return runStage16ItemPlacementIsolated(context, withG5PlacementExecutor(options));
}

function withG5PlacementExecutor(options = {}) {
  const executor = options.executor ?? options.llmStageExecutor;
  if (typeof executor !== 'function') return options;
  const wrapped = async (payload = {}) => {
    const role = payload.stage?.role;
    const descriptor = getNewGameG5PlacementRoleDescriptor(role);
    return executor({ ...payload, stage: { ...payload.stage, ...descriptor } });
  };
  return { ...options, executor: wrapped, llmStageExecutor: wrapped };
}
...
export async function runStage18MapKnowledge(context, options = {}) {
  ...
  const roleCall = (role) => {
    ...
    return async (roleInput) => executor({
    input: roleInput,
    stage: {
      id: 18,
      slug: 'map_knowledge',
  ...
  try {
    result = await runStage18CharacterKnowledgeMapBlock({
      input,
      build: options.build ?? options.characterKnowledgeMapBuilder ?? roleCall('CharacterKnowledgeMapBuilder'),
      audit: options.audit ?? options.characterKnowledgeMapAuditor ?? roleCall('CharacterKnowledgeMapAuditor'),
      formatRepair: options.formatRepair ?? options.characterKnowledgeMapFormatRepairer ?? roleCall('CharacterKnowledgeMapFormatRepairer'),
      semanticRepair: options.semanticRepair ?? options.characterKnowledgeMapSemanticRepairer ?? roleCall('CharacterKnowledgeMapSemanticRepairer'),
      seniorRepair: options.seniorRepair ?? options.characterKnowledgeMapSeniorRepairer ?? roleCall('CharacterKnowledgeMapSeniorRepairer')
    });
  } catch (error) {
...
export async function runStage19HiddenState(context, options = {}) {
  ...
  const result = await runStage19HiddenStateBlock({
    input,
    build: options.build ?? options.hiddenStateBuilder ?? roleCall('FullHiddenStateBuilder'),
    audit: options.audit ?? options.hiddenStateAuditor ?? roleCall('FullHiddenStateAuditor'),
    formatRepair: options.formatRepair ?? options.hiddenStateFormatRepairer ?? roleCall('FullHiddenStateFormatRepairer'),
    semanticRepair: options.semanticRepair ?? options.hiddenStateSemanticRepairer ?? roleCall('FullHiddenStateSemanticRepairer'),
    seniorRepair: options.seniorRepair ?? options.hiddenStateSeniorRepairer ?? roleCall('FullHiddenStateSeniorRepairer')
  });
...
export async function runStage20VisibleContext(context, options = {}) {
  ...
  const roleCall = (role) => async (roleInput) => executor({
    ...
    stage: {
      ...
      ...getNewGameVisibleContextRoleDescriptor(role),
  ...
  try {
    result = await runStage20VisibleContextBlock({
      input,
      build: options.build ?? options.visibleContextBuilder ?? roleCall('VisibleContextBuilder'),
      formatRepair: options.formatRepair ?? options.visibleContextFormatRepairer ?? roleCall('VisibleContextFormatRepairer'),
      semanticRepair: options.semanticRepair ?? options.visibleContextSemanticRepairer ?? roleCall('VisibleContextSemanticRepairer'),
      seniorRepair: options.seniorRepair ?? options.visibleContextSeniorRepairer ?? roleCall('SeniorVisibleContextSemanticRepairer'),
...
export async function runStage21VisibleContextAudit(context, options = {}) {
  ...
  const roleCall = (role, outputSchema) => async (roleInput) => executor({
    input: roleInput,
    stage: {
      id: 21,
  ...
  try {
    result = await runStage21VisibleContextAuditBlock({
      input,
      auditor: options.auditor ?? options.visibleContextSemanticAuditor ?? roleCall('VisibleContextSemanticAuditor', STAGE21_OUTPUT_SCHEMA),
      formatRepairer: options.formatRepairer ?? options.visibleContextAuditFormatRepairer ?? roleCall('VisibleContextAuditFormatRepairer', STAGE21_OUTPUT_SCHEMA),
      seniorAuditor: options.seniorAuditor ?? options.visibleContextSeniorAuditor ?? roleCall('SeniorVisibleContextSemanticAuditor', STAGE21_OUTPUT_SCHEMA),
      auditRouter: options.auditRouter ?? options.visibleContextAuditRouter ?? roleCall('VisibleContextAuditRouter', STAGE21_ROUTE_SCHEMA)
    });
  } catch (error) {
