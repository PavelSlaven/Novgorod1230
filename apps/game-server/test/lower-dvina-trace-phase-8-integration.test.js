...
import { registerPhase8HealthZeroTest } from './lower-dvina-trace-phase-8-health-zero.test-cases.js';

const bundle = await loadScenarioBundle(16);
const revision24Bundle = await loadScenarioBundle(24);
const ROUTE_TEXT = 'Идти к Жданко всем вместе. Ратшу держать между нами. Не входить тайком.';

test('revision 16 does not construct Phase 8 before an escort commits', () => {
  assert.equal(
    createTracePhase8Runtime({
...
test('revision 24 admits the Phase 8 route only after escort commitment', async () => {
  const state = phase8CampState();
  const ids = actorIds(state);
  const runtime = fixture({ scenarioBundle: revision24Bundle, materializationBundle: revision24Bundle, committedState: state, turnStepModel: (request) => phase8Plan(request, ids) });
  await runtime.runtime.submitTurn({
    partyId: runtime.partyId,
    input: {
      request_id: 'phase8-revision24-route',
      idempotency_key: 'phase8-revision24-route',
      raw_text: ROUTE_TEXT,
    },
  });
...
test('Phase 8 leaves a prepared first-entry NPC outside escort at camp', async () => {
  const state = phase8CampState();
  const ids = actorIds(state);
  const campAnchorId = state.position.g5_anchor_id;
  const onisim = state.npcs.find(({ instance_id: id }) => id === ids.onisim);
  state.first_entry_preparation = {
    ...
    npcs: [structuredClone(onisim)],
  };
  const runtime = fixture({ scenarioBundle: revision24Bundle, materializationBundle: revision24Bundle, committedState: state, turnStepModel: (request) => phase8Plan(request, ids) });
  await runtime.runtime.submitTurn({
    partyId: runtime.partyId,
    input: {
      request_id: 'phase8-keep-camp-npc',
      idempotency_key: 'phase8-keep-camp-npc',
      raw_text: ROUTE_TEXT,
    },
  });
  assert.equal(runtime.state.npcs.find(({ instance_id: id }) => id === ids.onisim).anchor_id, campAnchorId);
});
...
test('Phase 8 reaches the storehouse, opens combat, and commits one exchange', async () => {
  ...
  await runtime.runtime.submitTurn({
    partyId: runtime.partyId,
    input: {
      request_id: 'phase8-route',
      idempotency_key: 'phase8-route',
      raw_text: ROUTE_TEXT,
    },
  });
  assert.equal(runtime.state.position.location_ref, 'trace_ld_v1_loc_zhdanko_storehouse');
  ...
  for (const id of [ids.eremey, ids.ratsha, ids.fisher]) {
    assert.equal(runtime.state.npcs.find(({ instance_id }) => instance_id === id).anchor_id, runtime.state.position.g5_anchor_id);
  }

  await runtime.runtime.submitTurn({
    partyId: runtime.partyId,
    input: {
      request_id: 'phase8-accusation',
      idempotency_key: 'phase8-accusation',
  ...
  assert.equal(
    terminalSignals.every(({ signal_id: id }) => runtime.state.consumed_npc_decision_signal_ids.includes(id)),
    true,
  );
  ...
  assert.equal(
    runtime.state.combat_history.at(-1).outcome_event_refs.some((eventId) => eventId.endsWith(':ended')),
    true,
  );
...
test('Ratsha reaches the factual road bag through the movement owner', async () => {
  ...
  const runtime = fixture({
    ...
    npcCombatModel: (request) => {
      ...
      return combatPlan(request, ids, { ratsha: 'reach', companions: 'hold' });
    },
  });
  const startMinute = Number(runtime.state.clock.whole_minutes);
  ...
  assert.equal(Number(runtime.state.clock.whole_minutes), startMinute + 19);
  const movementEvent = runtime.state.last_turn.consequence.combat.outcome_events.find(
    ({ actor_ref: actor, event_kind: kind }) => kind === 'combat_position_transition_completed' && actor?.entity_id === ids.ratsha,
  );
  ...
  const movementSignal = runtime.state.last_turn.consequence.combat.signal_records.find(
    ({ source_event_ref: source }) => source.entity_kind === 'combat_event' && source.entity_id === movementEvent.event_id,
  );
  ...
  assert.equal(
    ratshaRequests[1].decision_reasons.perceived_changes.some((summary) => summary.includes('достиг выбранного')),
    true,
  );
...
test('Zhdanko break_contact uses the approved local exit before leaving combat', async () => {
  ...
  const runtime = fixture({
    ...
    npcCombatModel: (request) => {
      requests.push(structuredClone(request));
      return combatPlan(request, ids, { zhdanko: 'break_contact', companions: 'hold' });
    },
  });
  const startMinute = Number(runtime.state.clock.whole_minutes);
  ...
  assert.equal(
    runtime.state.last_turn.consequence.combat.outcome_events.some(({ actor_ref: actor, event_kind: kind }) => kind === 'combat_position_transition_completed' && actor?.entity_id === ids.zhdanko),
    false,
  );
  ...
  assert.equal(
    runtime.state.active_combat_step_progress.some(({ actor_ref: actor }) => actor.entity_id === ids.zhdanko),
    false,
  );
...
export function phase8CampState(scenarioBundle = bundle) {
  ...
  const camp = state.prepared_scenes.find(({ location_profile_ref }) => location_profile_ref === 'trace_ld_v1_loc_fishing_camp');
  state.position = { g5_node_id: camp.node.instance_id, g5_anchor_id: camp.anchor.instance_id, location_ref: camp.location_profile_ref, zone_ref: 'fire_side' };
  const ids = actorIds(state);
  for (const npc of state.npcs) {
    if ([ids.eremey, ids.ratsha, ids.fisher, ids.onisim].includes(npc.instance_id)) {
      npc.anchor_id = camp.anchor.instance_id;
      npc.location_profile_ref = camp.location_profile_ref;
      npc.zone_ref = 'fire_side';
      npc.machine_state = { ...npc.machine_state, location_ref: camp.location_profile_ref, spatial_zone_ref: 'fire_side' };
    }
  }
...
export function actorIds(state) {
  const bySlot = Object.fromEntries(state.npcs.map((npc) => [npc.participant_slot_ref, npc.instance_id]));
  return {
    player: state.actor_id,
...
export function phase8Plan(request, ids, combatIntentKind = 'control') {
  ...
  const operation = combat
    ? {
        ...
        target_refs: hold ? [] : [ids.zhdanko],
        protected_refs: [],
        scope_ref: hold ? 'trace_ld_v1_loc_zhdanko_storehouse' : null,
        destination_ref: null,
        force_limit: combatIntentKind === 'engage' ? 'ordinary' : hold ? 'avoid_harm' : 'nonlethal_if_possible',
        risk_posture: 'ordinary',
      }
    : {
        op: 'emit_interaction',
        actor_ref: request.actor.actor_id,
        target_actor_refs: [ids.zhdanko],
  ...
  return {
    ...
    interpretation: { player_goal: request.root_player_action, grounded_attempt: request.remaining_intent, adaptation: 'literal' },
...
export function combatPlan(request, ids, choices = {}) {
  const postDisarm = request.decision_reasons.perceived_changes.some((summary) => summary.includes('оруж'));
  const postReach = request.decision_reasons.perceived_changes.some((summary) => summary.includes('достиг выбранного'));
  let intentKind = 'hold',
    targetRefs = [],
    scopeRef = {
  ...
  if (request.npc_ref.entity_id === ids.zhdanko) {
    ...
    targetRefs = postDisarm ? [] : [{ entity_kind: 'player_character', entity_id: ids.player }];
    scopeRef = null;
    forceLimit = 'ordinary';
  ...
  } else if (request.npc_ref.entity_id === ids.ratsha && choices.ratsha === 'reach' && !postReach) {
    intentKind = 'reach';
    targetRefs = [];
    scopeRef = null;
    forceLimit = 'avoid_harm';
  ...
  const destinationRef =
    intentKind === 'reach' ? request.operation_contract.reachable_destination_refs[0] : intentKind === 'break_contact' ? request.operation_contract.break_contact_destination_refs[0] : null;
  const plan = {
    ...
    reason: postDisarm ? 'Сопротивление более невозможно.' : 'Участник выбирает ближайшее допустимое действие.',
  };
  assert.deepEqual(validateNpcCombatPlanApplicability(plan, request), { pass: true, errors: [] });
  return plan;
}
...
function ref(entityId) {
  return { entity_kind: 'npc', entity_id: entityId };
}

registerPhase8HealthZeroTest({
  ROUTE_TEXT,
  actorIds,
  bundle,
  combatPlan,
