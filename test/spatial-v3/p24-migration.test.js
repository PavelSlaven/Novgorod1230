...
import { adaptV2PartyEntities, applySpatialV3PartyMigration, buildSpatialV3MigrationInventory, buildSpatialV3SourceExtract, classifyV2Journey, classifyV2PartyG5, constructP14JourneyRows, validateSpatialV3MigrationAcceptance } from '../../tools/spatial-v3/p24-migration.mjs';
...
test('P24 party apply includes reviewed P14 journey target rows atomically', async () => {
  ...
  assert.equal(result.ok,true); assert.equal(result.applied,4); assert.equal(calls.filter((sql)=>sql.startsWith('INSERT INTO party_runtime.')).length,4);
});
