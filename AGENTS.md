...
- F1 активен только для SHA-pinned Lower Dvina revision 22 profile: `request_world_process` допускает start/add только с authored ignition basis и concrete approved whole non-stacked `ordinary_solid_fuel_unit` refs текущего anchor. `@rus/world-processes` чисто и детерминированно строит local transition; `@rus/time-events-history` остаётся единственным владельцем due ordering. Каждый due boundary retire-ит ровно первый ordered fuel unit, add только append-ит новый approved unit, а actor leave/death не прекращает процесс. Тот же combined P16 атомарно фиксирует process, binding, item retirement, replay и один party bump; bound fuel нельзя отдельно переместить или retire. Player-safe projection раскрывает лишь уже видимые approved refs и active process refs. Extinguish/water fail closed, потому что revision 22 не содержит exact finite-water authority;
- S1 активен только для SHA-pinned Lower Dvina revision 23 profile и только как последний remainder существующего `request_discovery/look` после registered/external/authored и O1/A1/F1 owners. First-entry P16 provision-ит две finite capacity-1 envelopes текущего wreck-shore G5/G6/position: authored ordinary structural detail и descriptive-only local natural feature. Код до model резервирует envelope, строит party-scoped identity и закрытый descriptor catalog; fully-authored detail имеет model calls 0, а bounded model может выбрать только разрешённый `descriptor_ref`. Тот же combined P16 re-lock-ит baseline/G5/G6/position, profile/policy/property/function/environment/template pins, атомарно consume-ит reservation, сохраняет resolution и делает один party bump. Reload/revisit раскрывает committed result без reroll. S1 не меняет baseline, G0–G6 topology, positions, edges, routes, movement/hazard mechanics, water или F1 state; unsupported mechanics fail closed;
- N1 активен только для SHA-pinned Lower Dvina revision 24 profile и только на существующей Phase 7 autonomous boundary Жданко. После authored/registered actor operations genuinely open `request_discovery/look` может вызвать actor-scoped O1 owner ровно один раз; request строится только из NPC-safe perception/knowledge/goals/access и не получает hidden ledgers, player-private state или unseen contents. Sealed NPC actor-step handoff связывает semantic trace, exact objective/profile/location/access pins и обычный O1 atomic plan с тем же combined P16; P16 повторно блокирует и проверяет NPC objective, сохраняет decision trace и ordinary resolution с одним party bump. Reload/replay не reroll-ит committed result. Current N1 profile не активирует NPC O2a/O2b/A1/F1/S1 и не даёт NPC authority придумывать resources, items или bases;
...
Для Lower Dvina Trace revision 14 активен conversation path, revision 15 активирует autonomous NPC path, revision 16 / `spatial-v3-production-v6` — общий combat path, revision 17 / `spatial-v3-production-v7` — Phase 9 recovery/evidence/temporary-disposition path, revision 18 / `spatial-v3-production-v8` — deterministic Phase 10 completion и player-safe epilogue, revision 19 / `spatial-v3-production-v9` — canonical appearance, revision 20 / M8 / Phase 1A v16 / Phase 1B v15 добавила O2b existing-container contents, revision 21 / M9 / Phase 1A v17 / Phase 1B v16 / `spatial-v3-production-v10` активировала A1, revision 22 / M10 / Phase 1A v18 / Phase 1B v17 / `spatial-v3-production-v11` добавила F1, revision 23 / M11 / Phase 1A v19 / Phase 1B v18 / `spatial-v3-production-v12` добавила finite S1, а active revision 24 / M12 / Phase 1A v20 / Phase 1B v19 / `spatial-v3-production-v13` активирует узкий N1 actor-scoped O1 remainder Жданко. Revision 23 и прежние публикации остаются immutable historical recovery paths. Meaningful NPC response, автономное и боевое решение проходят через общие `npc_decision_signal_v1` → `npc_decision_boundary_v1`, профильные semantic request/plan и общий code-owned execution/commit. Бой использует `request_combat`, persisted `combat_session_v1`, общие checks/body/items/time owners и не создаёт scenario-local resolver. Phase 9 делегирует property/container transitions `@rus/items-property`, evidence resolution `@rus/visibility-knowledge-memory`, а временное disposition `@rus/social-law`; она не вскрывает документ. Phase 10 после отдельного committed Phase 9 change set детерминированно вычисляет `full|partial|case_open` только из committed producers, фиксируется отдельным zero-time P16 commit и передаёт narration только persisted player-safe projection. Bounded NPC selection допустим только для genuinely closed choices и явно pinned historical revisions, но не как fallback semantic path.
