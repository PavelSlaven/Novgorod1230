...
test('playtest gate does not impose an obsolete aggregate turn deadline', () => { const gate = roleGate({ observed_role_ids: [], turns: [{ turn_id: 'x', status: 200, client_duration_ms: 199_999, llm: { turn_duration_ms: 199_999, turn_deadline_ms: null, aggregate: { repair_calls: 0 } } }] }); assert.equal(gate.gaps.includes('deadline exceeded: x'), false); });
...
test('pending presentation and missing combat role remain failures', () => {
...
test('missing accusation combat state fails before combat probes', async () => {
  ...
  await assert.rejects(runPublicPlaytest({ manifest,
    ...
    fetchImpl: branchFetch(manifest, 'ongoing_combat', bodies, null, null,
      true),
    now: counter(), log() {} }), {
      code: 'PUBLIC_PLAYTEST_COMBAT_BRANCH_INVALID'
    });
...
test('repair gate scopes uniqueness to each presentation attempt', () => { const repair = { role: 'narrator_semantic_repair', repair: true }; const turn = (waterfall, attempts = []) => ({ turn_id: 'repair', status: 200, llm: { turn_duration_ms: 1, aggregate: { repair_calls: waterfall.length }, waterfall, attempts } }); const distinct = roleGate({ observed_role_ids: [], turns: [turn(['planner_repair', 'narrator_format_repair', 'narrator_semantic_repair'].map((role) => ({ role, repair: true })))] }); assert.equal(distinct.gaps.some((gap) => gap.includes('repair')), false); const replay = roleGate({ observed_role_ids: [], turns: [turn([repair, repair], [{ waterfall: [repair] }, { waterfall: [repair] }])] }); assert.equal(replay.gaps.some((gap) => gap.includes('repair')), false); const repeated = roleGate({ observed_role_ids: [], turns: [turn([repair, repair], [{ waterfall: [repair, repair] }])] }); assert.ok(repeated.gaps.includes('repeated repair role: repair: narrator_semantic_repair')); });
...
test('runner reads diagnostics before pending ack and retains both attempts', async () => { const responses = [health(), catalog(), newGame(), ack(), pendingTurn(), diagnostics({ duration: 15_000, deadline: 60_000, role: 'turn_step_planner', repairCalls: 1, llmCalls: 2 }), turn(), diagnostics({ duration: 15_000, deadline: 60_000, role: 'turn_step_presenter', llmCalls: 1, budgetExhausted: true })]; const requests = []; await assert.rejects(() => runPublicPlaytest({ manifest: evidenceFirstManifest(), impossibleProbe: IMPOSSIBLE_PROBE, git: cleanGit, start: localStart, fetchImpl: async (url, request) => { requests.push({ url, body: request.body && JSON.parse(request.body) }); return responses.shift(); }, now: counter(), log() {} }), (error) => { const turn = error.report.turns[0]; assert.equal(turn.presentation_ack.screen_status, 'ready'); assert.deepEqual(turn.role_ids, ['turn_step_planner', 'turn_step_presenter']); assert.equal(turn.llm.turn_duration_ms, 30_000); assert.equal(turn.llm.aggregate.repair_calls, 1); assert.equal(turn.llm.aggregate.llm_calls, 3); assert.equal(turn.llm.aggregate.budget_exhausted, true); assert.deepEqual(turn.llm.attempts.map((attempt) => attempt.turn_deadline_ms), [60_000, 60_000]); assert.equal(error.report.gates.gaps.includes('deadline exceeded: probe-0'), false); return true; }); assert.match(requests[5].url, /developer\/llm-turn-reports/); assert.match(requests[6].url, /\/turns$/); assert.match(requests[7].url, /developer\/llm-turn-reports/); assert.deepEqual(requests[4].body, requests[6].body); });
