...
Difficulty описывает только задачу и заявленные внешние условия:

- `trivial` — почти автоматическая попытка без значимого препятствия;
- `ordinary` — обычная выполнимая задача без заявленного внешнего препятствия;
- `risky` — есть значимое внешнее препятствие и правдоподобен провал;
- `dangerous` — внешняя трудность или опасность уже тяжёлая;
- `limit` — задача близка к обычному человеческому пределу;
- `nearly_impossible` — задача физически возможна, но требует чрезвычайных условий.

Характеристики, навыки, состояние тела, экипировка и личная значимость
применяются кодом как точные модификаторы и не меняют выбранную LLM
качественную difficulty.
