...
test('cue emission requires an emitter category and never bypasses its rule scope', () => {
  const state = initialized().environment_state;
  assert.throws(() => update(state, 1, [{ ...emitter, emitter_category_id: null, propagation_wind: 'west' }], 'turn:missing-category'), (error) => error instanceof EnvironmentFeatureError && error.code === 'ENVIRONMENT_EMITTER_CATEGORY_INVALID');
  const mismatched = update(state, 1, [{ ...emitter, emitter_category_id: 'environment_emitter.beacon', propagation_wind: 'west' }], 'turn:other-category');
  assert.deepEqual(mismatched.created_cues, []);
});
