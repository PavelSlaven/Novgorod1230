...
- O2a на текущем active head включает authored abundant ambient source берега и authored first-entry context-bound finite stock подготовленной глины. Обе capability раскрывают только code-owned ref и безопасный public label; owner назначает profile-owned type/name, проверяет permission/property/placement и для любого `finite_source` обязательно формирует owner-native decrement. Model wording не является admission password. Restricted weapon/currency/document/other запрос без exact authored capability фиксируется как `absent`, а не становится authority или повторным броском. Сам enum `ambient_ordinary` не включает O2a: legacy clay/wood/bark/grass/stone/shell/root/worm direct-action сохраняет прежний admission;
