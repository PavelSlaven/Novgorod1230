...
import { isWorldDataPostgresEnabled, loadRegionsFromDb } from './world-base-db.js';

let cachedCatalog = null;
let cachedCatalogSourceKey = null;

export async function loadRegionCatalogAsync(env = process.env) {
  if (isWorldDataPostgresEnabled(env)) {
    try {
      const regions = await loadRegionsFromDb();
      if (regions.length > 0) {
        cachedCatalog = regions;
        cachedCatalogSourceKey = 'postgres';
        return cachedCatalog;
      }
    } catch (_error) {
      // ponytail: postgres unavailable — fall back to filesystem catalog
    }
  }
  return loadRegionCatalog();
}
...
export function parseRegionList(text) {
  const regions = [];
