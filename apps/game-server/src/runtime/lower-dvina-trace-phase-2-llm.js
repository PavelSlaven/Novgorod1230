      ...
      response = await roleRunner.run({
        ...
        messages: [{
          ...
          content: [
            ...
            'Plan exactly one executable step. One supplied exact code-owned capability and domain owner may cover multiple verbs as one composite activity. If it covers all request.remaining_intent, emit one domain_request and continuation is null. Every domain_request uses goal_result pending, including a complete composite with continuation null: pending means code-owned execution, not unhandled intent. If continuation is present because an independent later part remains uncovered, goal_result must be pending and continuation.remaining_intent must preserve that part.',
            repairing
