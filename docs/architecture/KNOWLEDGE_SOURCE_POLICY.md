...
Код knowledge-source не создаёт нормативные правила или историю. По активным нормативам LLM конкретизирует ordinary-экземпляры внутри категорий кода, а runtime-код проверяет и сохраняет их (PC §9.1; #133 D9). LLM не расширяет каталог и не пишет state patches.
