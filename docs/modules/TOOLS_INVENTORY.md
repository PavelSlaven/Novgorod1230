...
| `tools/docs-tools/src/knowledge-materializer-v2.js` | Preserve approved semantic graph/RAG snapshots only after semantic hash/chunk parity and fail-closed provenance validation of matching source paths and logical line ranges; add deterministic structural/lexical coverage for native documents without fabricated embeddings; own CLI paths and public writer | Writes only declared `generated/knowledge-source/*` outputs through explicit generate commands |
