import { TURN_STEP_PERCEPTION_INSTRUCTION } from
...
export const TURN_STEP_PLAN_MAPPINGS = JSON.stringify({
  ...
  focused_ordinary_discovery: { interpretation: { adaptation: 'literal' }, resolution: 'domain_request', goal_result: 'pending', activity: { owner: 'domain', duration_class: null, effort: null }, operations: [{ op: 'request_discovery', actor_ref: '<copy current actor ref from request>', discovery_kind: '<copy inspect or search from intent>', target_refs: ['<copy one current visible searched location or entity ref>'], query: '<copy request.remaining_intent verbatim>' }], check: null },
...
export const TURN_STEP_PLANNER_INSTRUCTIONS = [
  ...
  TURN_STEP_PERCEPTION_INSTRUCTION,
  ...
  'When player_safe_state.ordinary_resolution.discovery_available is true, it is exact code-owned authority for a focused inspect or search for an unspecified ordinary physical object, material, resource, or local physical detail: before and over general paths use focused_ordinary_discovery exactly; exposed ambient extraction uses ambient_ordinary_portion_take first. Emit exactly one request_discovery: discovery_kind inspect or search, actor_ref from request.actor, one current visible target_ref, and query copied verbatim from request.remaining_intent; do not summarize, shorten, translate, or omit its stated purpose. This standalone mapping has continuation null. For comparison inspect one target now, preserve the rest and comparison in continuation; target_refs has one ref. target_ref is the location or entity being searched, not a preexisting ref for the sought ordinary detail; sought ordinary detail need not be visible, and absence from player-safe state is for discovery, not a reason for a direct failure. A supplied request_discovery choice covers only its own fixed query; never select or copy a broad authored inspection to stand in for a different ordinary candidate query. general current situation, ongoing activity, or who is nearby are ordinary_scene_seed while scene_seed_available is true and visible_general_look afterward. This does not authorize authored, significant, or hidden facts.',
  'Visible ambient_ordinary_capability is exact code-owned permission and source, not an existing item alias or a discovery target. Take: ambient_ordinary_portion_take before ordinary_material_prerequisite or action_production: create_entity, capability entity_id sole origin.source_refs, ambient_ordinary, facts [], held_by current actor. Copy quantity.unit from its ambient_portion_bounds; choose quantity.value and mass_grams only within those exact min/max bounds. Owner rechecks effective type, name, mechanics, source, and profile values at commit. Never substitute any nearby, worn, held, or listed item ref. Take-only achieved; later action continuation, making that compound plan pending.',
  'Without a matching ambient_ordinary_capability or item entity_ref, take/use/transform must first acquire the ordinary referent through ordinary_material_prerequisite. This includes material explicitly described by current visible sensory facts and an ordinary referent merely sought in the current visible physical scope. Use ordinary_material_prerequisite exactly, never focused_ordinary_discovery: emit one request_discovery with discovery_kind inspect against the current visible scope, a query naming only the needed ordinary referent, material, or physically connected group, and continuation containing the complete intended handling or transformation. The player\'s or an NPC\'s words are not evidence that the referent exists: this handoff only asks the ordinary owner to resolve presence. It MUST win over action_production and a direct reality_limited failure unless authoritative, significant, hidden, or physically impossible truth is requested. Never substitute an unrelated inventory, worn, held, or merely listed item_ref for the material named by the player. Do not call missing item refs impossible, do not invent refs, and do not resolve the later physical action in this step. After materialization the server exposes the committed candidate to the next step and action_production owns the transformation.',
  'An item in player_safe_state.items held, worn, or equipped by the current actor is an already accessible exact item ref even when current_visible_context does not repeat it. Its explicit relocation uses direct_item_relocation; its physical manipulation or durable change uses its existing item owner, never a direct no-operation or reality_limited absence. A focused inspection of any current visible item, including one held by the actor, seeks new detail: when discovery_available is true, use focused_ordinary_discovery with that exact visible item ref rather than direct observation. A sensory detail that physically places ordinary environmental material in the current scope is sufficient for ordinary_material_prerequisite, but is not an item ref and never authorizes an authoritative, significant, hidden, or already-resolved fact.',
  ...
  'Taking, dropping, wearing, putting inside, attaching, or otherwise relocating one whole existing item without changing its physical form is a physical placement change and uses direct_item_relocation. move_entity has exactly op, entity_ref, and placement; placement has exactly relation and target_ref. held_by and worn_by target only the current actor; attached_to targets only a visible item. If the supplied player-safe placement already has that same relation and target, the relocation is already satisfied: emit no move_entity, consume that clause, and continue with the next independent intent. Never represent cutting, tearing, partitioning, reshaping, wrapping, binding, or using part of a source by moving the whole source. When one clause first separates a new physical result from a source and a later clause uses, wears, or places that result, plan the separation first through action_production independent_outputs and preserve the complete later use and every following action in continuation. Never invent a preliminary relocation merely because manipulation might be easier after it: when the player manipulates a worn, held, or otherwise accessible source in place without explicitly relocating it, plan the manipulation itself. A direct preparation and action_production cannot share one plan. Before action_production, identify every explicit requested destination or spatial relation. If no exact player-safe target ref supports one, transformation may occur only at the source\'s committed placement: omit that relocation from every result descriptor and preserve it as unexecuted continuation. An action_production descriptor is item-local: it may state only a changed physical property, never placement, attachment, holder, wearer, destination, or relocation. For ordered explicit relocation, transformation, and placement, emit the first move_entity now and preserve every later action in continuation; each later placement again needs move_entity. When relocation is the whole intent, continuation is null and goal_result is achieved. action_production never implies relocation.',
  'Direct empty achieved or partially_achieved requires semantic moment/none and direct_result_kind: player_safe_item_observation for reviewing the identity, placement, or condition of supplied carried/worn items; player_safe_body_observation for inspecting the actor body when request.actor.body is supplied; player_safe_observation for other supplied sensory facts; otherwise no_state_gesture. Body observation confirms only already supplied body state and leaves any requested new injury or diagnosis unconfirmed; clothing mentioned only as covering the body does not make the action an item or ordinary discovery. Preserve uncertainty; never infer local state, cause, forecast, timing, new physical detail, or code-owned effects. Use perception or its owner for those.',
  ...
  'action_production represents only a durable item-local physical result after the action ends. Never use its physical_description, qualitative_facts, or source_fact_delta to claim, create, preserve, or describe an active, ongoing, self-propagating, or time-dependent world process. Such a process requires an exact supplied code-owned domain operation; select its matching choice_id. Without one, return the honest reality_limited no-operation attempt with no process or physical-fact claim.',
