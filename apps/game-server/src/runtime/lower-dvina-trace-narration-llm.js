...
export function createLowerDvinaTraceNarrationService({ roleRunner } = {}) {
  ...
  return createNarrationService({
    ...
    semanticRepairer: { repair: (request) => runNarrationRole(roleRunner,
      ...
      'Return only {"replacements":[{"prose":"<replacement in Russian>"}]} in the same order as supplied flagged segments. The server assembles version, schema, and immutable segment_id values. Repair only supplied flagged segments using their concerns, read-only nearby_context, player-safe visible_context, and optional action_intent_context. Remove unsupported claims: remove every unsupported claim in the flagged segment, including claims named by the concern. For technical_presentation, preserve the supported meaning but rewrite it as connected, restrained literary Russian without field language, IDs, numeric-stat dumps, system reports, modern slang, or pseudo-archaic decoration. A faithful natural paraphrase of visible_context is allowed; it must not add detail, causation, exact object use, or success. Empty visible_npc or visible_objects arrays do not prove that nobody or nothing is present, nor do they support silence or emptiness. A visible physical feature does not support an unstated sound, smell, temperature, bodily sensation, history, or recent use. action_intent_context may ground only that the player attempted an action or spoke, never its success. If the flagged sentence has no supported meaning left, replace it with one short natural statement copied or faithfully paraphrased from visible_context or the attempt-only context. Return one replacement for each flagged segment and no others. Do not use hidden state, change neighboring segments, infer facts, add a fallback, or call any other role.',
      request) }
  });
}
