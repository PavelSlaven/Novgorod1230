...
- `INTERACTION_MAP.md` — владельцы данных, контракты, hard blocks и границы с runtime/party state;
- `DECISION_LOG.md` — решения, причины, отклонённые альтернативы и compatibility boundary.
...
3. разрешить individual historical source records и template-source bindings — начато: 15 agriculture/fishing item templates имеют FK-normalized `item_template_source_bindings` на существующие source IDs base bundle, но остаются `draft/needs_review`; остальные 105 требуют individual bibliography;
...
- создан один draft project-authoring `source_record`; 15 существующих historical source IDs base bundle привязаны через FK-normalized `item_template_source_bindings` только к claim `historical_presence` и остаются `draft/needs_review`, 105 templates остаются без individual source link;
...
Технические artifacts: `TARGET_TABLE_COVERAGE.md`, `PHYSICAL_PARAMETER_AUTHORING_POLICY.md`, `PHYSICAL_PARAMETER_REVIEW_TABLE.md`, `NORMALIZATION_COVERAGE_REPORT.md`, `DATA_GAPS.md`, `INTERACTION_MAP.md`, `DECISION_LOG.md`, `CODEX_INTEGRATION_REPORT.md`, `POSTGRESQL_INTEGRATION_REPORT.md`, `PROMOTION_READINESS_REPORT.md` и `ACTIVATION_PROPOSAL.md`. Последний является неисполняемым документом контроля: он предлагает `0` approved records и не изменяет runtime candidate sets. Public contracts редакторского validator-а и parent-source utility зарегистрированы в `tools/world-catalog-workflow/CONTRACTS.md`.
...
Обязательный повторный code critic завершился `PASS WITH NOTES`. После его `CHANGES REQUIRED` добавлены item/container revision-consistency guards в DDL, supplemental и canonical validators, отрицательные tests для обоих путей, а verified parent-source collection теперь включает typed bindings независимо от audit ledger. Финальный critic фактически выполнил 34/34 targeted tests и `git diff --check`; full PostgreSQL 16 lifecycle и полный suite проверены исполнителем и отражены выше. PostgreSQL lifecycle больше не является ограничением: он выполнен в disposable PostgreSQL 16 с явно заданными временными credentials.
