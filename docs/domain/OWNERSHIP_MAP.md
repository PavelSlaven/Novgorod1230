...
| `@rus/materialization` | deterministic materialization traces; начальный выбор погоды/света (`projectApprovedCurrentEnvironment` через `target-current-factual-context`, v17) | database access / commit; живая смена погоды (владелец — `@rus/environment-state`) |
| `@rus/environment-state` | применяет погоду и свет, считает моменты смены; каждое наступившее состояние — факт партии; данные — преемник `weather_transition_profiles_processes.json` (novgorod v2); следующее состояние выбирает turn через RandomSource ([#133](https://github.com/PavelSlaven/Novgorod1230/issues/133#issuecomment-5839745154) D7) | начальный выбор (materialization); выбор следующего состояния (turn) |
