...
Оружие, деньги и документы не запрещены классом; LLM не выдумывает их без сохранённого исхода.
...
LLM даёт категорию и, по желанию, предложение механики; значения присваивает код (§18.1 / §18.4; CR #146 шаг 2).
...
Используется только после фактического физического изменения сущности. LLM может предложить qualitative изменение; числовые mechanics пишет код.
...
- perception result — `recognized` как неизвестный человек (тот же результат `recognized` / не узнал код может выдать и для вещи по приметам; LLM не решает, узнал ли NPC вещь — D14);
