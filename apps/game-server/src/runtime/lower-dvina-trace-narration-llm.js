...
export function createLowerDvinaTraceNarrationService({ roleRunner } = {}) {
  ...
  return createNarrationService({
    ...
    semanticRepairer: { repair: (request) => runNarrationRole(roleRunner,
      ...
      'Return only {"replacements":[{"prose":"<replacement>"}]} in the same order as supplied flagged segments. The server assembles version, schema, and immutable segment_id values. Repair only supplied flagged segments using their concerns, read-only nearby_context, player-safe visible_context, and optional action_intent_context. Each concern kind identifies the unsupported claim class that must be removed or recast. For unsupported_success, unsupported_object_use, unsupported_result, unsupported_sensory, unsupported_event, unsupported_world_state, or unsupported_npc_state, remove that claim; retain it only when visible_context explicitly supports the exact claim. If action_intent_context supports the attempt, an unsupported success or result may be recast as attempt-only wording without implying completion, causation, exact object use, or state change. action_intent_context is intent-only: it may ground only that the player attempted an action, spoke, or selected an intention. It never proves success, object use, a result, or a world/NPC state change; those claims require visible_context. A confirmed visible result may be stated separately without attributing an unsupported mechanism. Return one replacement for each flagged segment and no others. Do not use hidden state, change neighboring segments, infer facts, add a fallback, or call any other role.',
      request) }
  });
}
