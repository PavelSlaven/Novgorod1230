...
test('v5 fishing-camp S1 closure is sealed, inactive and reciprocal', async () => {
  ...
  assert.equal(scene[0].id, 'trace_ld_v1_tpl_fishing_camp');
  const scenePayload = structuredClone(scene[0]);
  delete scenePayload.canonical_digest;
  assert.equal(scene[0].canonical_digest, digest(scenePayload));
