...
const MANIFEST_PATH = `${ROOT}/phase-1b-v6/manifest.json`;
const BINDING_PATH = `${ROOT}/phase-1b-v6/publication-binding.json`;
...
export async function loadLowerDvinaTracePhase1BPublication({
  ...
  if (manifest?.schema !== 'rus.lower_dvina_trace_phase_1b_manifest.v1'
    || manifest.package_id !== 'lower_dvina_trace_phase_1b_v6'
    || manifest.revision !== 6
  ...
  assertExactRef(manifest.superseded_package_ref, supersededManifest, {
    path: `${ROOT}/phase-1b-v5/manifest.json`,
    id: 'lower_dvina_trace_phase_1b_v5',
    revision: 5,
  ...
  assertExactRef(binding.superseded_binding_ref, supersededBinding, {
    path: `${ROOT}/phase-1b-v5/publication-binding.json`,
  ...
  assertExactRef(binding.scenario_definition_ref, definition, {
    path: `${ROOT}/phase-5-content/definition.json`,
  ...
  if (definition.value.required_unresolved_refs?.length !== 0) {
    fail(
      ...
      'Only the complete scenario definition revision 11 can be published.'
    );
  }
...
function assertBinding(binding) {
  ...
  if (binding?.schema !== 'rus.lower_dvina_trace_publication_binding.v1'
    ...
    || projection?.projection_id
      !== 'lower_dvina_trace_phase_1b_opening_projection_v6'
