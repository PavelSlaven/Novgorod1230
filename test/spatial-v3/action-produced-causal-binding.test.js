...
test('trace P16 binds action-produced action and step to approved trace', () => {
  ...
  const sealed = { actor_ref: 'pc', context_pin: { context_ref: 'context-1' },
  ...
  assert.equal(validActionProducedOuterCausalBinding(plan, sealed), true);
  const forgedAction = structuredClone(sealed);
  forgedAction.causal_identity.action_ref = 'forged';
  assert.equal(validActionProducedOuterCausalBinding(plan, forgedAction),
    false);
  const forgedStep = structuredClone(sealed);
  forgedStep.causal_identity.step_index = 1;
