...
export function loadApprovedG4NaturalCatalog({ verifiedCatalog, pin } = {}) {
  ...
  const profiles = (verifiedCatalog.records_by_table?.procedural_scene_compiled_records ?? [])
    .filter((row) => row.payload?.schema === 'rus.g4_natural_baseline_profile.v1');
  const seen = new Set();
  if (!profiles.length) invalid('records');
  for (const row of profiles) {
    const payload = row.payload;
    validateG4NaturalProfile(payload);
    const key = `${payload.g4_ref.id}@${payload.g4_ref.version}`;
