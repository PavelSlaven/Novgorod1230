export const TURN_STEP_PERCEPTION_INSTRUCTION = 'Evaluate the entire remaining_intent and preserve action order. Speech to the active interlocutor precedes a momentary look at that already supplied person; seeking new detail remains an earlier focused perception. Reviewing the identity, placement, or condition of supplied carried/worn items is direct achieved or partially achieved observation with direct_result_kind player_safe_item_observation. Inspecting the actor body when request.actor.body is supplied uses direct achieved or partially_achieved player_safe_body_observation: it confirms only supplied body state and leaves any requested new injury or diagnosis unconfirmed. Clothing mentioned only as covering the body does not make that action an item or ordinary discovery. Reviewing other facts already explicit in player-safe sensory context uses player_safe_observation. Each current_visible_context.visible_npc visible_status is an already supplied current observation about that exact visible NPC; a request fully answered by those statuses is direct achieved player_safe_observation with no discovery or continuation. A colon or question specifying what the same look observes is not an independent continuation. Preserve uncertainty. Inspect/search for a new physical detail is focused perception: use matching available_domain_operations first, otherwise focused_ordinary_discovery when ordinary_resolution.discovery_available is true, and only without either use reality_limited not_achieved. Select a matching supplied discovery choice_id exactly and preserve later independent speech in continuation. A passive look cannot absorb a focused clause. General scanning of the current situation, activity, or nearby people uses ordinary_scene_seed while scene_seed_available is true and visible_general_look afterward, even when phrased as trying to understand what is happening.';
