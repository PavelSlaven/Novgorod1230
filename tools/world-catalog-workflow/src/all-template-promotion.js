...
export function buildAllTemplateRevisionPromotionPlan({ required_template_ids = [], editorial_readiness_report, legacy_inventory_snapshot, approval_attestation, candidate_digest: candidateDigest = null, approved_record_ids_by_table = {}, ...input } = {}) {
  const errors = [];
  ...
  if (candidateDigest != null && (!/^[a-f0-9]{64}$/u.test(candidateDigest) || approval_attestation?.candidate_digest !== candidateDigest)) errors.push(problem('PROMOTION_CANDIDATE_DIGEST_MISMATCH', 'Approval must explicitly bind the exact candidate manifest digest.'));
