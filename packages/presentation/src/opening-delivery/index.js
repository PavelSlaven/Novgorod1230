...
import {
  ...
  validateStage26ResultForDelivery
} from '@rus/contracts';

export const STAGE27_INPUT_SCHEMA = STAGE27_FIRST_TURN_INPUT_SCHEMA;
...
export function createFirstScreenDeliveryAttempt({ stage26_result, existing_attempt = null, created_at = new Date().toISOString() } = {}) {
  const concerns = validateStage26ResultForDelivery(stage26_result);
  ...
  if (screenDigest !== computeStage26ScreenDigest(screen)) throw deliveryError('DELIVERY_SCREEN_DIGEST_MISMATCH');
...
export function buildStage27FirstTurnInput({
  ...
  return deepFreeze({
    version: 1,
    schema: STAGE27_FIRST_TURN_INPUT_SCHEMA,
    request_id: stage26_result.request_id,
    stage26_screen_approval: buildStage26ScreenApproval(stage26_result),
    party_id: stage26_result.party_id,
    transaction_id: stage26_result.transaction_id,
    current_turn_number: 1,
    party_state: { status: 'ready', current_phase: 'awaiting_player_input' },
...
export function validateStage26ToStage27IntentHandoff({ stage26_result, delivery_attempt, delivery_ack_result, selected_action_option_id = null } = {}) {
  const issues = validateStage26ResultForDelivery(stage26_result);
  if (stage26_result?.schema !== STAGE26_FIRST_GAME_SCREEN_RESULT_SCHEMA) issues.push(issue('STAGE27_STAGE26_RESULT_INVALID', 'Stage 26 result schema is invalid.', 'stage26_result'));
  ...
  else if (delivery_ack_result.screen_digest !== stage26_result?.screen_digest || delivery_ack_result.message_id !== stage26_result?.first_game_screen?.delivery_state?.message_id) issues.push(issue('STAGE27_DELIVERY_BINDING_MISMATCH', 'Delivery acknowledgement does not match Stage 26 screen.', 'delivery_ack_result'));
...
function deterministicAttemptId(partyId, messageId, screenDigest) { const hash = createHash('sha256').update(`${partyId}|${messageId}|${screenDigest}`).digest('hex').slice(0, 24); return `first-screen-delivery:${hash}`; }
function deliveryError(code, details = []) { const error = new Error(code); error.code = code; error.concerns = Array.isArray(details) ? details : []; return error; }
function issue(code, message, path = null) { return { code, message, path }; }
