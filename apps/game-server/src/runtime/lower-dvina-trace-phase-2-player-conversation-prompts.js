...
export function playerConversationInstructions(repair, request = null) {
  ...
  return [
    ...
    'player_safe_context.allowed_references.knowledge_refs; otherwise use [].',
    'When the player directly greets an intended listener, include greeting in',
    'speech.interaction_tags even when question or request is the dominant act.',
    'Do not add greeting when the player did not greet that listener.',
    ...
    'For intent_paraphrase, speech.utterance_text contains only the words the',
    'character would naturally say; omit action narration and UI descriptions',
    'such as saying that the character greets or addresses a visible person.',
    ...
    'When operation_contract permits offer_conditional_protection, select it only',
    'when the player actually offers the target protection or no further harm in',
    'exchange for surrender. Then use check_required and copy available_check into',
    'the mapped check. A different proposal, bargain, cooperation, aid, movement,',
    'or permission remains ordinary automatic speech with no supporting operation.',
