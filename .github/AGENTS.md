...
сами по себе не выполняли production activation. Отдельный
`versioned production activation cutover` завершён release
`spatial-v3-production-v1`; Spatial v3/Temporal World v4 являются sole
production owner, а production v2 не выбирается как runtime fallback.
...
## Post-merge складирование локальных материалов

Полное каноническое правило находится в корневом `AGENTS.md` и обязательно для GitHub-, Codex- и Cursor-oriented workflows.

Локальные временные материалы не удаляются автоматически. Для задачи с PR они складируются только после подтверждённого merge в канонический `origin/main`, успешного обязательного CI и фиксации evidence; для задачи без PR требуется отдельное явное указание пользователя.

Это правило не предоставляет право удалять исходные или архивные материалы. Удаление является отдельной задачей и требует нового явного решения пользователя после проверки точных путей.

Перед перемещением обязательны read-only инвентаризация tracked/untracked/ignored изменений и worktree через `git status`/`git ls-files`, проверка содержимого, владельца и уникальности, точный пользовательский манифест для чужих или неизвестных материалов и SHA-256 до и после. `git clean` не используется ни в каком режиме; broad cleanup, wildcard deletion, сброс tracked-изменений, `git worktree remove`/`prune` и ручное удаление `.git/worktrees` запрещены.

Архив размещается вне checkout по шаблону `C:\Users\Slaven\Documents\Новгород-archive\<YYYY-MM-DD>__<task>__<merge-SHA>\`, не подключается к workspace, Graphify, RAG или другим индексаторам и не связывается с checkout через symlink/junction. Он содержит `README.md`, `MANIFEST.json`, `SHA256SUMS.txt` и дерево `payload/`; автоматически не удаляется.

Нельзя автоматически перемещать или удалять tracked-файлы, пользовательские данные, `.agents/`, `.codex/`, `.cursor/`, локальную конфигурацию и секреты, базы и snapshots, рабочий README, audit/test/evidence artifacts, незавершённые изменения, неизвестные материалы и единственные копии данных. Зарегистрированный disposable worktree перемещается только через `git worktree move`; при невозможности безопасного перемещения он остаётся на месте.

После складирования агент повторяет `git status --short --branch --untracked-files=all`, обе команды инвентаризации `git ls-files`, `git worktree list --porcelain` и `git diff --check`. Итогом является чистое состояние либо точный перечень намеренно сохранённых локальных путей с Git-статусом, назначением, владельцем, причиной сохранения и условием будущего решения пользователя.
