...
async function installSchemas(pool) { const files = (await readdir('schemas/party-db')).filter((file) => /^\d+.*\.sql$/u.test(file)).sort(); for (const file of files.filter((file) => !file.startsWith('012_') && !file.startsWith('013_') && !file.startsWith('014_'))) await pool.query(await readFile(`schemas/party-db/${file}`, 'utf8')); assert.equal((await runPartyRuntimeCatalogMigration(pool)).status, 'applied'); await pool.query(await readFile('schemas/party-db/012_party_runtime_external_ownership.sql', 'utf8')); await pool.query(await readFile('schemas/party-db/013_party_runtime_obligations.sql', 'utf8')); await pool.query(await readFile('schemas/party-db/014_party_runtime_activity_resume_terminal.sql', 'utf8')); }
