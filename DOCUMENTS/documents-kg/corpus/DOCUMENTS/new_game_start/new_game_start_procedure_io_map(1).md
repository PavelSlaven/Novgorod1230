...
Этот документ не заменяет отдельные нормативные документы этапов `1.txt`–`26.txt`. Если есть расхождение, приоритет имеет профильный документ конкретного этапа. Документ нужен как общая карта входов/выходов и как техническая опора для реализации конвейера.
...
| 18 | Карта знаний персонажа | LLM | character, frame, selected place, routes, known facts | `character_knowledge_map` | не давать персонажу скрытое знание |
| 19 | Hidden state | LLM | full approved start data | `hidden_state_package` | не выводить hidden в player-facing слой |
...
| 24 | DB write plan | code + LLM structuring if needed | all approved stage outputs | `db_write_plan` | не писать напрямую, только план записи |
| 25 | Commit в party DB | code-only | approved `db_write_plan` | `committed_party_snapshot` | не менять смысл при записи |
| 26 | Первый игровой экран | code/UI + prose package | committed state, visible context, prose | `first_game_screen_payload` | не показывать скрытые данные и raw diagnostics |
...
Отдельно записать, что персонаж реально знает о ближайших путях, людях, слухах, опасностях, адресах, ориентирах и региональном фоне.
...
```text
давать точную скрытую карту;
давать будущие события как знание;
давать hidden motives NPC;
приравнивать слух к факту.
```
...
## Этап 24. DB write plan

### Назначение

Сформировать план записи утверждённых данных в party-базу. Этот этап ещё не пишет в базу.
...
```json
{
  "schema": "db_write_plan",
  "party_state_writes": [],
  "party_current_position_writes": [],
  "party_graph_node_writes": [],
  "party_graph_edge_writes": [],
  "party_character_writes": [],
  "party_npc_writes": [],
  "party_item_writes": [],
  "party_event_writes": [],
  "party_knowledge_writes": [],
  "journal_writes": [],
  "diagnostic_log_writes": [],
...
## Этап 25. Commit в party DB

### Назначение

Код применяет утверждённый `db_write_plan` к party-базе. Это code-only этап.
...
```json
{
  "schema": "committed_party_snapshot",
  "party_id": "string",
  "commit_id": "string",
  "current_position": {},
