...
Для Lower Dvina Trace revision 14 активен conversation path, revision 15 активирует autonomous NPC path, revision 16 / `spatial-v3-production-v6` — общий combat path, а revision 17 / `spatial-v3-production-v7` — Phase 9 recovery/evidence/temporary-disposition path. Meaningful NPC response, автономное и боевое решение проходят через общие `npc_decision_signal_v1` → `npc_decision_boundary_v1`, профильные semantic request/plan и общий code-owned execution/commit. Бой использует `request_combat`, persisted `combat_session_v1`, общие checks/body/items/time owners и не создаёт scenario-local resolver. Phase 9 делегирует property/container transitions `@rus/items-property`, evidence resolution `@rus/visibility-knowledge-memory`, а временное disposition `@rus/social-law`; она не вскрывает документ и не пишет completion. Bounded NPC selection допустим только для genuinely closed choices и явно pinned historical revisions, но не как fallback semantic path.
