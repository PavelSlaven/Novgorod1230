...
Этот файл нужен для быстрой ориентации LLM, разработчика или аудитора в папке `DOCUMENTS`. Он не заменяет профильные документы и не вводит новых правил. Если этот README расходится с профильным документом, приоритет имеет профильный документ.
...
Нельзя придумывать новые механики, сущности, формулы, исторические допущения, игровые факты или UI-слои, если они не следуют из текущих документов проекта. Код хранит, проверяет и передаёт состояние; LLM создаёт смысл, формулирует сцену и структурирует данные только в границах документации.

## 1. Приоритет источников

1. Профильный документ по конкретной системе имеет высший приоритет.
2. `formulas.md` помогает быстро найти формулу, но не заменяет профильный документ, где эта формула объяснена.
3. `README.md` и этот `llm_documentation_navigation.md` являются навигацией, а не источником новых правил.
4. `world_regions.txt` задаёт допустимую региональную сетку, но не описывает всю внутреннюю логику региона.
5. Исторический слой всегда уточняется через год, сезон, регион, текущую сцену и уже зафиксированные факты партии.
6. LLM не должна менять уже зафиксированные факты без игровой причины и явной фиксации изменения.
7. Новые предметы, NPC, места, слухи, исторические события и последствия материализуются только при причинном основании и в пределах текущего контекста.
8. Скрытые факты мира, мотивы NPC, будущие события, технические промты и raw-диагностика не должны попадать в player-facing прозу или UI.
...
3. `read_only_database_and_graph_architecture.md` — read-only `world_base`, слои карты, graph_nodes/edges
4. `world_base_schema_reference.md` — поля и ограничения DDL
5. `party_character_knowledge_layer.md` — Stage 18 output, root/version, normalized runtime tables, immutable snapshots и projection invariants
6. `world_generation_and_turns.txt`
7. `interface_ux.md`
...
| `world_generation_and_turns.txt` | Основной документ о создании и расширении мира: регион, место, локация, минилокация, точка сцены, позиция персонажа, ходы, сохранение фактов, скрытые процессы и обработка заявок игрока. Фиксирует принцип: LLM создаёт смысл, код хранит и передаёт данные. | При генерации новой партии, стартовой сцены, нового места, обработке хода, обновлении мира, проверке материализованных фактов и разделении роли кода/LLM. |
...
| `party_character_knowledge_layer.md` | Каноническая модель хранения знаний персонажа: цельный Stage 18 output, root/version, нормализованные runtime-таблицы, immutable snapshots, nearby paths и commit invariants. | При реализации Stage 18/20/21/24/25, базового хода, карты знаний, миграций и commit gate. |
...
| `read_only_database_and_graph_architecture.md` | Каноническая read-only схема `world_base` (50 таблиц): граф G0–G5, **слоистая модель карты** (landscape / water / route / place / land_use + region join), landscape validation, party DB отдельно. | При генерации или утверждении G1/G2-графа, выборе шаблонов среды/воды/дорог/мест/хозяйства, NocoDB, проверке допустимости slug для региона. |
| `world_base_schema_reference.md` | Сгенерированный справочник полей PostgreSQL `world_base` (типы, CHECK, FK, пояснения). Регенерация: `npm run world-db:schema-doc`. | При заполнении NocoDB, написании LLM-процедур материализации графа, сверке имён полей и ограничений DDL. |
