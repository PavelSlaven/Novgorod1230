...
package and its exact target compilation are approved authoring inputs; this
does not mean production import, runtime visibility or activation. Historical
P28 exact-head evidence did not switch production. Until the separate
versioned production activation cutover, production remains on materialization
...
| `novgorod-g1-semantic-catalog` | `data/world-catalogs/novgorod/G1_SEMANTIC_CATALOG.md` | approved P12 source-to-target authoring compilation; historical P28 evidence is accepted, while production import and runtime activation remain `not_performed` |
...
fallback, dual write or partial activation before the versioned production activation cutover.
...
| `@rus/game-server` | future composition only through the versioned production activation cutover; no duplicate spatial logic | target |
