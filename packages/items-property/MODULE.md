...
Для `partial_transformation + independent_outputs` тот же transition оставляет изменённый source и создаёт outputs; нечётная масса распределяется детерминированно, а output hand/packing/carry выводятся code-owned и не наследуют форму source.
