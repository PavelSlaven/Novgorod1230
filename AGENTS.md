...
- O1 — единственный активный discovery-путь common ordinary world: существующий `request_discovery` после authored/committed проверки и exact persisted resolution может закрыть ordinary gap только при meaningful engagement. Его Stage A получает лишь committed objective context, не содержит candidate/player wishlist и не вправе создавать concrete entities; LLM предлагает только `sparse|ordinary|dense`, а versioned code-owned policy переводит density в numeric identity budget. Stage B имеет `evidence_weight = 0`, а код строит classification, coverage и policy fields. Нормализованный discovery query (NFKC, trim, collapse whitespace, ru-RU lowercase) вместе с exact target выводит code-owned candidate identity и передаётся модели только как `candidate_hint`: это не noun/recipe allowlist и не authority для класса или механики. Exact повтор использует persisted resolution без reroll; другой нормализованный запрос получает другую identity. Один discovery имеет общий бюджет максимум двух semantic calls: repair расходует оставшийся call и не создаёт третий вызов; если repair Stage A исчерпал бюджет, сохраняется seed-only. Активация требует SHA-pinned adversarial Stage B eval для weapon/currency/document/evidence/significant-hidden/anachronism/misleading-common probes и versioned approval receipt, связанный с profile digest и exact production provider/model/config identity; игровой ход только локально проверяет receipt и не выполняет eval probes. Sensitive `materialize` блокирует активацию. Положительный non-container `man_made` результат требует independently committed либо prepared supporting basis, code-owned admission и exact immutable mechanics/property/placement в пределах bounded mechanics policy. Модель вызывается вне physical transaction; P16 атомарно сохраняет seed/basis/resolution/item (если есть), версии и idempotency, а positive/negative exact resolution переживает reload без reroll по code-owned identity. Preflight `no_change` при исчерпанных budget/cap не создаёт granular resolution; если в том же turn только что выполнен Stage A, P16 сохраняет seed-only. Player получает capability marker только через `@rus/visibility-knowledge-memory` и committed/approved visible result; narration не создаёт entity;
- O2a на текущем active head включает authored abundant ambient source берега и authored first-entry context-bound finite stock подготовленной глины. Ambient capability раскрывает code-owned ref и безопасный public label; context-bound marker сообщает только `discovery_available`, а committed stock виден как обычный source только при отдельном approved disclosure state, без permission/capacity/expected-result leakage. Capability availability не означает disclosure: concealed source остаётся server-only. Stage B свободно конкретизирует ordinary semantic type/name внутри code-owned admission class, но не меняет source/property/permission/mechanics и не добавляет facts; каждый выбранный `finite_source` связывается со своим current committed row и owner обязательно формирует decrement. Restricted weapon/currency/document/other запрос без authored authority фиксируется как `absent`, а не становится authority или повторным броском. Сам enum `ambient_ordinary` не включает O2a: legacy clay/wood/bark/grass/stone/shell/root/worm direct-action сохраняет прежний admission;
- O1/O2a не активируют остальные O2, A1/F1/S1/N1, template-less runtime containers либо не provisioned authored precious/remnant profiles. Эти ограничения не ослабляют существующий direct-action path;
...
Для Lower Dvina Trace revision 14 активен conversation path, revision 15 активирует autonomous NPC path, revision 16 / `spatial-v3-production-v6` — общий combat path, revision 17 / `spatial-v3-production-v7` — Phase 9 recovery/evidence/temporary-disposition path, revision 18 / `spatial-v3-production-v8` — deterministic Phase 10 completion и player-safe epilogue, revision 19 / `spatial-v3-production-v9` — canonical appearance, а active revision 20 / M8 / Phase 1A v16 / Phase 1B v15 добавляет только O2b existing-container contents для exact authored pouch. Revision 19 и прежние публикации остаются immutable historical recovery paths. Meaningful NPC response, автономное и боевое решение проходят через общие `npc_decision_signal_v1` → `npc_decision_boundary_v1`, профильные semantic request/plan и общий code-owned execution/commit. Бой использует `request_combat`, persisted `combat_session_v1`, общие checks/body/items/time owners и не создаёт scenario-local resolver. Phase 9 делегирует property/container transitions `@rus/items-property`, evidence resolution `@rus/visibility-knowledge-memory`, а временное disposition `@rus/social-law`; она не вскрывает документ. Phase 10 после отдельного committed Phase 9 change set детерминированно вычисляет `full|partial|case_open` только из committed producers, фиксируется отдельным zero-time P16 commit и передаёт narration только persisted player-safe projection. Bounded NPC selection допустим только для genuinely closed choices и явно pinned historical revisions, но не как fallback semantic path.
