...
test('trace dispatch commits before its safe screen', async () => {
  ...
  assert.equal(f.materializeCalls[0].scenario_definition_revision, 23);
  assert.equal(
  ...
  assert.equal(session.stage26_result.publication_binding_revision, 18);
  assert.equal(session.stage26_result.scenario_definition_revision, 23);
