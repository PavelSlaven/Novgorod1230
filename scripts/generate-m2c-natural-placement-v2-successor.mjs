...
const startPath = `${base}live-world-runtime-v17/capacity-v2-start-successors/novgorod_pine_ridge_approach_v1.start.json`;
...
export async function buildNaturalPlacementV2Successor() {
  const [sourceBytes, startBytes, approvalBytes, sceneBytes, oldApprovalBytes] = await Promise.all(
    [sourcePath, startPath, approvalPath, scenePath, oldApprovalPath].map(read));
  ...
  const candidateBytes = Buffer.from(deriveApprovedNaturalPlacementV2Successor({
    sourceCandidateBytes: sourceBytes.toString(), approvedStartBytes: startBytes.toString(),
  ...
  const manifestBytes = encode({
    ...
    approved_start: pin(startPath, startBytes),
    ...
    exact_change: {
      placement_id: start.initial_perception_rule.placement_candidate.placement_ref.id,
      field: 'scene_template_ref.version',
      from: 1,
      to: 2,
