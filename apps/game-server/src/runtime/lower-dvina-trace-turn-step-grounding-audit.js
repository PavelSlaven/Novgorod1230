...
const PROMPT = 'Return only {"pass":true,"concerns":[]} or {"pass":false,"concerns":[{"kind":"source_semantic_mismatch","reason":"<brief exact mismatch>"}]} using only these concern kinds: source_semantic_mismatch, missing_required_source_move, action_production_identity_mismatch, operation_semantic_mismatch, duration_semantic_mismatch, missing_material_transformation; report every present concern once in that order. Every concern has exactly kind and a brief reason grounded in supplied text; name the mismatched or uncovered action, but do not propose a repair. Audit six things independently. Apply the first three checks only when action_productions is non-empty; when it is empty, never report source_semantic_mismatch, missing_required_source_move, or action_production_identity_mismatch. First, every selected action-production source must denote the ordinary material named in remaining_intent, resolving pronouns from root_player_action and completed_steps. Use only that source own supplied player-safe descriptors as grounding evidence. An unbound sensory sentence, another item descriptor, inventory order, and the player claim do not ground a source ref. Tools are not material sources. Second, independently identify every explicit change of possession or placement in remaining_intent. If the actor explicitly takes, picks up, drops, wears, attaches, or otherwise relocates the selected action-production source, planned_operations must contain a matching move_entity; action production never implies relocation. Do not require movement when the intent only manipulates or transforms the source in place without a separate placement change. Third, compare the physical identity topology in each action_production with the explicit transformation. A modification that leaves one physical object uses preserve_source. Cutting, tearing, breaking off, or otherwise separating a physically independent result from a source uses independent_outputs; preserve_source is invalid even when a later clause immediately wraps, binds, wears, or uses the detached result. A partial separation from one surviving source requires origin direct_partition, result_class partial_transformation, material_extent minor, half, or major, a non-null output physical_form, and a non-null source_fact_delta describing the surviving source. Every later use and later independent action must remain in continuation. Report action_production_identity_mismatch for wrong identity_mode, origin, result_class, extent, descriptor topology, or lost later clauses. Do not judge exact numeric mechanics. Fourth, each selected_domain_operation, including a direct move_entity, must semantically cover the next independently executable action in remaining_intent. Its own supplied query, description, kind, and player-safe referent evidence must match that action; sharing only an actor, place, broad verb, or operation type is insufficient. move_entity covers only relocation of the whole existing item: it never covers cutting, tearing, partitioning, reshaping, wrapping, binding, or using a separated part of its source. A fixed authored inspection does not cover a broader general look or a different ordinary search. Every independent clause not covered by that operation must remain in continuation.remaining_intent. Report operation_semantic_mismatch for a different action, over-broad consumption, or lost uncovered clause. In its reason identify the earliest uncovered action and every later clause missing from continuation. Do not report it merely because exact mechanics are absent from player text. Fifth, when requested_duration_minutes is supplied, verify that remaining_intent explicitly requests exactly that whole-minute duration, including an equivalent natural-language duration such as half an hour; otherwise report duration_semantic_mismatch. Sixth, only when available_materials is supplied, decide whether the no-operation plan erased an independently feasible physical manipulation or transformation of one semantically matching visible/player-held material merely because a later purpose or effect lacks an owner. If so report missing_material_transformation and name the omitted transformation plus every later uncovered clause. Do not report it for observation, gesture, speech, waiting, impossible actions, absent sources, or a plan that preserves the transformation in continuation. Do not plan, repair, invent refs, or judge exact mechanics.';
...
export async function auditTurnStepSourceGrounding({ roleRunner, plan,
  ...
  const allowedKinds = new Set([
    ...(productions.length === 0 ? [] : [
      ...
      'action_production_identity_mismatch'
    ]),
  ...
  const definitions = [
    ...
    ['action_production_identity_mismatch',
      ...
      'action-production identity topology must match the transformation'],
...
function selectedDomainOperations(plan, request) {
  ...
  return plannedOperations(plan).filter(({ operation }) =>
    operation?.op === 'move_entity' || choices.some(
...
function actionProductions(plan, state) {
  ...
  const add = (operation, path) => {
    ...
    found.push({ path,
      action_production: structuredClone(operation.action_production),
      source_refs: structuredClone(refs),
