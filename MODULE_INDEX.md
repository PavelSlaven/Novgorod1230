...
| `@rus/perception` | `packages/perception` | `resolveSoundPaths`; `resolveSoundPerception`; `resolveVisibility` | `./src/index.js` | `@rus/contracts`, `@rus/kernel` |
...
| `@rus/travel` | `packages/travel` | `validateTravelIntent`, `validateTravelPosition`, `validateJourney`, `validateTravelRulesBundle`; `buildJourneyPlan`, `createJourney`, `advanceJourney`; `interruptJourney`, `resumeJourney`, `changeJourneyPace`, `abandonJourney`, `completeJourney` | `./src/index.js` | `@rus/kernel` |
| `@rus/turn` | `packages/turn` | контрактом `PlayerTurnInput`; декларативным порядком блоков хода; orchestration context и run summary | `./src/index.js` | `@rus/checks-rng`, `@rus/contracts`, `@rus/kernel`, `@rus/materialization`, `@rus/narration`, `@rus/perception`, `@rus/pipeline-engine`, `@rus/presentation`, `@rus/time-events-history`, `@rus/visibility-knowledge-memory` |
