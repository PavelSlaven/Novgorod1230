    ...
    const response = await roleRunner.run({
      ...
      messages: [{
        ...
        content: [
          ...
          'Choose only listed result/output/weapon classes. For action production, source_refs are one or more consumed material items, tool_refs are unchanged tools, item_ref is source_refs[0], and target_refs contain every remaining source/tool ref. output_count is zero except for independent_outputs and must not exceed the visible max_new_entities. material_extent is null for preserved/no-result identity, whole for a full independent partition, and minor|half|major for a partial independent separation; choose only the qualitative extent implied by the attempt and never invent numeric mechanics.',
