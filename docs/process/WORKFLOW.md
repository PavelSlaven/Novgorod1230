...
[CONTRACT_INDEX](../../data/knowledge-source/corpus/DOCUMENTS/CONTRACT_INDEX.md). Здесь — только порядок действий. Зачем цикл устроен так и кто в нём участвует — [HOW_WE_WORK](HOW_WE_WORK.md).
...
6. **Ревью.** В режиме «исполнитель и ревьюер» ([HOW_WE_WORK](HOW_WE_WORK.md), раздел 5; решение владельца
   2026-09-25) каждая задача сдаётся ревьюеру до начала следующей; ревью начинается с diff тестов (WR §24.2). В сдаче —
   HEAD, реально выполненные проверки и строки CHANGELOG и LW.
7. Commit, push, PR — только если они входят в задачу (AGENTS §26). После подтверждённого merge — уборка
