...
test('attachDiagnosticJournal records llm lifecycle from provider hooks', () => {
  ...
  assert.ok(snapshot.some((entry) => entry.scope === 'legacy_world' && entry.roleId === 'legacy.historical_frame.shaper'));
  assert.ok(snapshot.every((entry) => entry.pipelineRuntime === 'legacy_provider'));
  assert.ok(snapshot.every((entry) => entry.legacyProviderRuntimeUsed === true));
});
