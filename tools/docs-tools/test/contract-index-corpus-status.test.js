...
test('CONTRACT_INDEX labels map to active/proposed/reference/deprecated', () => {
  ...
  assert.equal(mapIndexLabelToCorpusFields('REFERENCE').status, 'reference');
  ...
  assert.equal(mapIndexLabelToCorpusFields('PROPOSED').priority_tier, 'proposed');
  assert.equal(mapIndexLabelToCorpusFields('PROPOSED UMBRELLA TARGET').priority_tier, 'proposed');
  assert.throws(() => mapIndexLabelToCorpusFields('TOTALLY_UNKNOWN_LABEL'), /Unsupported CONTRACT_INDEX status label/);
  assert.throws(() => mapIndexLabelToCorpusFields('ACTIVE CANDIDATE'), /Unsupported CONTRACT_INDEX status label/);
  assert.throws(() => mapIndexLabelToCorpusFields('REFERENCE / OBSOLETE'), /Unsupported CONTRACT_INDEX status label/);
});

test('parser rejects duplicate rows with different CONTRACT_INDEX labels', () => {
  const markdown = [
...
test('unseen-equivalent: CONTRACT_INDEX status change updates manifest status and priority_tier via repin', async () => {
  ...
  assert.equal(items.priority_tier, 'proposed');
...
test('knowledge:check rejects manual priority_tier in policy and wrong default_statuses', async () => {
  ...
  const indexName = 'CONTRACT_INDEX.md';
  const docName = 'items_and_property.txt';
  const docBytes = Buffer.from('items\n');
  const indexBytes = Buffer.from([
    ...
    '|---|---|---|',
    '| [`CONTRACT_INDEX.md`](CONTRACT_INDEX.md) | `ACTIVE` | nav |',
    `| [\`${docName}\`](${docName}) | \`ACTIVE\` | items |`,
    ''
  ].join('\n'));
  await writeFile(join(sourceRoot, 'corpus/DOCUMENTS', indexName), indexBytes);
  await writeFile(join(sourceRoot, 'corpus/DOCUMENTS', docName), docBytes);
  const manifest = {
    schema_version: 'rus.knowledge_corpus_manifest.v2',
    corpus_id: 'test',
  ...
  await writeFile(join(sourceRoot, 'retrieval-policy.json'), `${JSON.stringify({
    schema_version: 'rus.knowledge_retrieval_policy.v1',
    policy_version: '1.0.0',
    baseline_manifest_sha256: sha(await readFile(join(sourceRoot, 'corpus-manifest.json'))),
  ...
  const result = await verifyCanonicalCorpus({ root: fixtureRoot });
  assert.equal(result.ok, false);
  assert.ok(result.errors.some((item) => item.includes('default_statuses')));
  assert.ok(result.errors.some((item) => item.includes('must not declare priority_tier')));
});
