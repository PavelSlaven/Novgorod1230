...
export function materializeApprovedItems(candidates, { input, partyId, runId, anchors, kind, quantityRequirements, equipmentCandidates }) {
  return candidates.filter((candidate) => candidate?.status === 'approved' && candidate?.required === true)
    ...
    .map((candidate, ordinal) => {
      ...
      if (!quantityRequirement.quantity_unit_id || !quantityRequirement.quantity_dimension || !Number.isFinite(quantityRequirement.mass_grams_per_unit) || quantityRequirement.mass_grams_per_unit <= 0) throw new MaterializationError('QUANTITY_REQUIREMENT_INCOMPLETE', `${kind} ${candidateId} quantity requirement lacks unit, dimension or unit mass.`);
