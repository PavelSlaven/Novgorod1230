...
- O2a на текущем active head включает только authored abundant ambient source берега: player-safe projection раскрывает code-owned capability ref и безопасный public label, semantic plan ссылается на этот ref, а owner после grounding назначает profile-owned semantic type и имя и проверяет bounded mechanics. Model-owned semantic type и wording остаются описательными и не являются admission password. Сам enum `ambient_ordinary` не включает O2a: legacy clay/wood/bark/grass/stone/shell/root/worm direct-action сохраняет прежний admission. Общие context-bound/property/finite/decrement владельцы присутствуют как dormant infrastructure, но precious/remnant/natural finite профили не считаются активными без отдельного authored provisioning;
- O1/O2a не активируют остальные O2, A1/F1/S1/N1, template-less runtime containers, context-bound weapons/value/currency или natural finite sources. Эти ограничения не ослабляют существующий direct-action path;
