import { deterministicUnit } from './deterministic-random.js';
  ...
  return Array.from({ length: steps + 1 }, (_, index) => {
    ...
    return [
      inverse * inverse * start[0]
        + 2 * inverse * t * control[0] + t * t * end[0],
      inverse * inverse * start[1]
        + 2 * inverse * t * control[1] + t * t * end[1]
    ];
  });
}
...
function wobble(points, { seed, salt, amplitude, closed }) {
  ...
  return points.map((point, index) => {
    ...
    const next = points[
      index === count - 1 ? (closed ? 0 : count - 1) : index + 1
    ];
