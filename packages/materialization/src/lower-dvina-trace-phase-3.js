...
export function materializeLowerDvinaTracePreparedDryingShed({ input, bundle, runId, participantSelections, locationSelections }) {
  ...
  if ([12, 13, 14, 15, 16, 17, 18].includes(input.scenario_definition_revision)) {
...
export function materializeLowerDvinaTracePreparedStorehouse({
  ...
  const weaponRequired = [16, 17, 18].includes(input.scenario_definition_revision);
  ...
  const packet = [17, 18].includes(input.scenario_definition_revision)
