...
Active A1 revision 21 использует sole player boundary `turn_step_request_v1 → turn_step_plan_v1`: qualitative physical result вложен в `request_item_use`, после чего code-owned owner не вызывает LLM. Один или несколько committed либо sealed same-root revealed non-container material sources и ноль или несколько доступных actor-controlled tools явно разделены и могут иметь другого legal owner. Profile не содержит template/recipe whitelist и допускает preserve, до четырёх independent outputs, no-result, partial/nonworking/waste, writing, non-authoritative token-like и closed weapon-capable outcomes. Code перечитывает committed mechanics/property/placement/ownership либо exact same-root ordinary overlay, выводит source/output mechanics из реально списанных allocations, уменьшает finite source или атомарно retire-ит полностью разделённый единичный source без resource row, сохраняет известную массу, выводит hand/packing code-owned и возвращает typed gap только при нецелимой mass allocation. Безопасный non-authoritative semantic descriptor сохраняется в ordinary item metadata и переживает reload. Всё фиксируется в одном combined P16. Model не задаёт numbers, canonical identity, currency/official status, truth или combat mechanics; weapon class получает формальные числа только в `@rus/combat-health`.
