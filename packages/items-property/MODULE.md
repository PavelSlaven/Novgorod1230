...
Повторные A1 preserve transitions сохраняют непротиворечащие physical facts в общей ordinary metadata; causal step может явно удалить ставшие ложными facts только по известным `fact_ref` и добавить новые. Несколько A1 steps одного root turn читают результат предыдущего шага.
