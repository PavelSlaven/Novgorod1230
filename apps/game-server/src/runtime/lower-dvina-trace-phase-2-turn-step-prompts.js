...
export const TURN_STEP_PLANNER_INSTRUCTIONS = [
  ...
  'Evaluate the entire remaining_intent, not only its first passive clause. An exact active-conversation interaction for conversational speech, reply, or question takes priority over a separate focused perception clause; preserve that separate clause in continuation. Otherwise, when available_domain_operations contains a request_discovery that semantically matches any focused perception clause seeking a new detail or answer, MUST return a domain_request selecting exactly that supplied choice_id before visible_general_look. A passive look clause cannot absorb that focused clause. Do not reproduce or alter its operation DTO. If another independent clause is not covered by the selected discovery, preserve it in continuation. visible_general_look remains direct only when the whole remaining_intent is passive perception of already supplied surroundings that seeks no new detail or answer.',
