...
const actionProductionFields = ['source_refs', 'tool_refs', 'requested_output_count', 'identity_mode', 'origin', 'result_class', 'material_extent', 'result_descriptor', 'output_class'];
const actionProductionProperties = {
  source_refs: { type: 'array', minItems: 1, uniqueItems: true, items: refSchema },
  tool_refs: { type: 'array', uniqueItems: true, items: refSchema },
  requested_output_count: { anyOf: [{ type: 'null' }, { type: 'integer', minimum: 1, maximum: 8 }] },
  identity_mode: { enum: ['preserve_source', 'independent_outputs', 'no_useful_result'] },
  origin: { anyOf: [{ type: 'null' }, { enum: ['direct_partition', 'crafted'] }] },
  result_class: { enum: ['ordinary_physical_result', 'partial_transformation', 'nonworking_construction', 'waste', 'written_carrier', 'no_useful_result'] },
  material_extent: { anyOf: [{ type: 'null' }, { enum: ['minor', 'half', 'major', 'whole'] }] },
  result_descriptor: { $ref: '#/$defs/action_production_descriptor' },
  output_class: { anyOf: [{ type: 'null' }, { enum: ['ordinary_mundane', 'weapon_capable', 'money_like_token', 'written_carrier'] }] }
};
const actionProductionSchema = (identityMode, requestedCount, descriptorRef) => strictObject(actionProductionFields, {
  ...actionProductionProperties,
  identity_mode: { const: identityMode },
  requested_output_count: requestedCount,
  ...(descriptorRef == null ? {} : { result_descriptor: { $ref: descriptorRef } })
});
...
const planDefinitions = {
  ...
  create_entity: strictObject(['op', 'temp_ref', 'semantic_type', 'name', 'origin', 'facts', 'mechanics', 'placement'], { op: { const: 'create_entity' }, temp_ref: refSchema, semantic_type: textSchema, name: textSchema, origin: { $ref: '#/$defs/origin' }, facts: { type: 'array', items: { $ref: '#/$defs/fact' } }, mechanics: { $ref: '#/$defs/mechanics' }, placement: { $ref: '#/$defs/placement' } }),
  move_entity: strictObject(['op', 'entity_ref', 'placement'], { op: { const: 'move_entity' }, entity_ref: refSchema, placement: { $ref: '#/$defs/placement' } }),
  change_entity_facts: strictObject(['op', 'entity_ref', 'remove_fact_refs', 'add_facts'], { op: { const: 'change_entity_facts' }, entity_ref: refSchema, remove_fact_refs: { type: 'array', uniqueItems: true, items: refSchema }, add_facts: { type: 'array', items: { $ref: '#/$defs/fact' } } }),
  ...
  request_movement: strictObject(['op', 'actor_ref', 'target_ref', 'movement_kind'], { op: { const: 'request_movement' }, actor_ref: refSchema, target_ref: refSchema, movement_kind: { enum: ['local', 'route', 'long_course'] } }),
  ...
  emit_interaction: strictObject(['op', 'actor_ref', 'target_actor_refs', 'interaction_kind', 'content', 'instrument_refs'], { op: { const: 'emit_interaction' }, actor_ref: refSchema, target_actor_refs: { type: 'array', minItems: 1, uniqueItems: true, items: refSchema }, interaction_kind: { enum: ['speech', 'gesture', 'offer', 'request', 'threat', 'attack', 'aid', 'other'] }, content: textSchema, instrument_refs: { type: 'array', uniqueItems: true, items: refSchema } }),
  request_world_process: { oneOf: [worldProcessSchema('start', { type: 'null' }, 1), worldProcessSchema('affect', refSchema, 0)] }
};

function worldProcessSchema(action, processRef, targetMinimum) {
  return strictObject(['op', 'actor_ref', 'process_action', 'process_ref', 'process_kind', 'source_refs', 'target_refs', 'description'], { op: { const: 'request_world_process' }, actor_ref: refSchema, process_action: { const: action }, process_ref: processRef, process_kind: { const: 'fire' }, source_refs: { type: 'array', minItems: 1, uniqueItems: true, items: refSchema }, target_refs: { type: 'array', minItems: targetMinimum, ...(action === 'affect' ? { maxItems: 0 } : {}), uniqueItems: true, items: refSchema }, description: textSchema });
}
...
planDefinitions.outcomes = strictObject(OUTCOME_BANDS, Object.fromEntries(OUTCOME_BANDS.map((band) => [band, { $ref: '#/$defs/outcome' }])));
planDefinitions.generic_check = strictObject(['purpose', 'attribute_ref', 'skill_ref', 'difficulty_id', 'outcomes'], { purpose: textSchema, attribute_ref: refSchema, skill_ref: nullableRefSchema, difficulty_id: { enum: DIFFICULTIES }, outcomes: { $ref: '#/$defs/outcomes' } });
...
export const TURN_STEP_PLAN_V1_SCHEMA = deepFreeze({
  ...
  ...strictObject(['schema', 'request_id', 'committed_state_version', 'working_revision', 'step_index', 'interpretation', 'resolution', 'goal_result', 'activity', 'operations', 'check', 'continuation', 'clarification', 'reason_code', 'reason'], {
    ...
    activity: { oneOf: [{ $ref: '#/$defs/semantic_activity' }, { $ref: '#/$defs/domain_activity' }] },
