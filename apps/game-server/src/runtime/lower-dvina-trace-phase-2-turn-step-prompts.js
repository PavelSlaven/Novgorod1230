...
export const TURN_STEP_PLANNER_INSTRUCTIONS = [
  ...
  'Taking, dropping, wearing, putting inside, attaching, or otherwise relocating an item is a physical placement change and uses direct_item_relocation. move_entity has exactly op, entity_ref, and placement; placement has exactly relation and target_ref. A direct preparation and action_production cannot share one plan. When relocation precedes transformation in the same sentence, plan only move_entity now and preserve only the still-unexecuted transformation in continuation. When relocation is the whole intent, continuation is null and goal_result is achieved. action_production never implies relocation.',
