...
import { migrateSkillKeys, SKILL_KEY_LEGACY_MAP } from './social-generation-gate.js';
...
function resolveAttributeKey(action, intent, frame, player) {
  ...
  if (action === 'attack') {
    ...
    if (method === 'ranged_shot') return focus === 'ranged' || focus === 'ranged_combat' ? 'agility' : 'attention';
    ...
    return focus === 'ranged' || focus === 'ranged_combat' ? 'agility' : 'strength';
  }
  if (action === 'defend') {
...
function resolveSkillKey(action, intent, frame, player) {
  ...
  const normalizedPreferred = SKILL_KEY_LEGACY_MAP[preferred] ?? preferred;
  if (normalizedPreferred && SKILL_LABELS[normalizedPreferred]) return normalizedPreferred;

  const focus = String(intent?.focus ?? '').toLowerCase();
  if (action === 'attack') {
    if (method === 'quick_strike') return 'melee_combat';
    if (method === 'ranged_shot') return 'ranged_combat';
    if (method === 'grapple' || method === 'shove') return focus === 'athletics' ? 'athletics' : 'melee_combat';
    if (method === 'intimidation') return focus === 'melee_combat' ? 'melee_combat' : 'communication_trade';
    return focus === 'ranged_combat' ? 'ranged_combat' : 'melee_combat';
  }
  if (action === 'defend') return focus === 'stealth' ? 'athletics' : 'melee_combat';
  ...
  if (action === 'trade' || action === 'talk') return 'communication_trade';
  if (action === 'claim') return 'custom_law_literacy';
...
function readSkillValue(player, key) {
  const bonuses = migrateSkillKeys(player.skill_bonuses ?? {});
