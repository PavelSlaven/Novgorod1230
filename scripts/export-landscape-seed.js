...
const xlsxPath = path.join(root, 'infra/world-base/landscape_templates_scientific_v1_fixed.xlsx');
const outPath = path.join(root, 'infra/world-base/landscape_templates.seed.json');
...
FLOAT_COLS = {"base_movement_multiplier"}
JSON_ARRAY_COLS = {"sources"}
...
for row_idx in range(1, max_row + 1):
    ...
    for key, val in raw.items():
        ...
        elif key in ("id", "slug", "title", "landscape_group", "base_environment", "status", "confidence"):
...
if (result.status !== 0) {
  console.error(result.stderr || 'export-landscape-seed: python failed');
