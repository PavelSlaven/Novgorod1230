...
test('world_base schema expands to exactly 174 unique tables', async () => {
  const result = await inspectWorldBaseSchema({ root: process.cwd() });
  assert.equal(result.schema, 'world_base');
  assert.equal(result.table_count, 174);
  assert.equal(result.table_names.length, 174);
  assert.deepEqual(result.duplicate_table_names, []);
  assert.equal(result.part_files.length, 14);
});
