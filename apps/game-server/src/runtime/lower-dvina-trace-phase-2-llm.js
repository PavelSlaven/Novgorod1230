    ...
    const response = await roleRunner.run({
        ...
        messages: [{
          ...
          content: [
            ...
            'FINAL SOURCE-GROUNDING GATE: before emitting request_item_use, verify every intended material against the selected ref own supplied player-safe label, category, description, or facts and state that exact grounding in reason. If any intended ordinary material appears only in current visible sensory facts and has no semantically matching item ref, request_item_use and action_production are invalid: return ordinary_material_prerequisite with a material-only query and the complete handling or transformation in continuation. An unrelated worn, held, inventory, visible, or first-listed ref never satisfies this gate. A supplied broad inspection with a different fixed query never satisfies it. This final gate overrides action_production availability.'
          ].join(' ')
        }, {
          role: 'user',
