...
const [gate, test, signingTool, signingTest, packageJson, log] = await Promise.all([
  readFile('tools/spatial-v3/p28-activation-gate.mjs', 'utf8'),
  readFile('test/spatial-v3/p28-activation.test.js', 'utf8'),
  readFile('tools/spatial-v3/p28-signing-payloads.mjs', 'utf8'),
  readFile('test/spatial-v3/p28-signing-payloads.test.js', 'utf8'),
...
for (const token of ['P28_P12_GAPS', 'validateP28P12GapIdentities']) if (!gate.includes(token)) throw new Error(`P28 gate lacks shared P12 identity contract ${token}`);
for (const token of ['prepareP28SigningPayloads', 'computeP28ManifestDigest', 'P28_P12_GAPS', 'validateP28P12GapIdentities', 'loadP28TrustStore', 'validateP28HashedEvidence', 'validateP28RoleSignature', 'resolveSafeRepositoryPath', 'ready_for_offline_signing', 'private_keys_accessed: 0', 'signatures_created: 0', 'p27_signature_missing', 'fresh_checkout_signature_missing']) if (!signingTool.includes(token)) throw new Error(`P28 signing tool lacks ${token}`);
for (const token of ['real Ed25519 verification in a two-commit repository', 'stages independent payloads but withholds release payload', 'rejects placeholder signatures', 'canonical SPKI identity', 'equal the tracked HEAD blob', 'duplicate P12 gap codes', 'before reading bytes']) if (!signingTest.includes(token)) throw new Error(`P28 signing test lacks ${token}`);
const scripts = JSON.parse(packageJson).scripts;
if (!scripts['spatial-v3:test-p28']?.includes('p28-signing-payloads.test.js') || !scripts['spatial-v3:check-p28'] || !scripts['spatial-v3:p28-signing-payloads']) throw new Error('P28 package scripts are incomplete');
