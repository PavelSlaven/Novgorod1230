...
**Статус:** `active`
**Версия:** `0.2.0`
**Дата повышения:** `2026-09-26`
**Основание повышения:** [#146](https://github.com/PavelSlaven/Novgorod1230/issues/146) (решения владельца PC §9.1 / #133 D1–D13)
...
Этот документ — действующий (`ACTIVE`) норматив универсальной категоризации. Статус задаёт `CONTRACT_INDEX.md`; повышение принято решением владельца в [#146](https://github.com/PavelSlaven/Novgorod1230/issues/146).

Документ задаёт каркас категорий и фасетов. Он не создаёт runtime-экземпляры и не заменяет:

1. `code_driven_world_materialization_architecture.md` (граница код/LLM и наличие);
2. `world_base_materialization_table_requirements.md` (назначение таблиц authoring);
3. профильные документы затронутых подсистем;
4. актуальный DDL `infra/world-base/schema.sql` и `infra/world-base/schema/*.sql`.

При конфликте применяется установленный проектом приоритет. Документ не разрешает коду, LLM или импортёру создавать отсутствующие категории, исторические факты либо fallback-значения.

Правила наличия ставятся только на один иерархический фасет домена: у предметов — `object_type`, у контейнеров — `container_form`; у новых доменов — фасет, который объявит эта политика ([#133 поправка п.2](https://github.com/PavelSlaven/Novgorod1230/issues/133#issuecomment-5843988793)). Правило на иных фасетах того же домена — ошибка валидации импорта.

Незаполненные домены категорий (флора, фауна места, природное сырьё места, постройки как presence-категории, часть NPC/ремёсел, иерархия `parent_category_id` внутри фасета) — пробел данных M2c, а не разрешение invent/fallback.
