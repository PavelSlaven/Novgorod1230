...
import { buildGameplayGapBacklog } from './gameplay-gap-audit.js';
...
try {
  ...
  } else if (command === 'build-gameplay-gap-backlog') {
    const campaign = await readJsonRequired(options.campaign);
    if (campaign.schema !== 'world_knowledge_gameplay_campaign_v1'
        || !Array.isArray(campaign.turns) || campaign.turns.length === 0) {
      throw new TypeError('campaign must contain actual gameplay traces');
    }
    const result = buildGameplayGapBacklog({ traces: campaign.turns,
      auditor_output: await readJsonRequired(options.audit) });
    await emit(options.out, result);
...
function usage() { throw new TypeError('usage: validate-revision --revision FILE [--out FILE] | prepare-legacy-g1 --revision FILE --tsv FILE --mask-out FILE --report-out FILE --queue-out FILE | validate-cell-package --package FILE [--out FILE] | compile-world-knowledge --pack FILE [--out FILE] | build-world-knowledge-vectors --bundle FILE --profile FILE --metadata-out FILE --vectors-out FILE | benchmark-world-knowledge-vectors --bundle FILE --profile FILE --metadata FILE --vectors FILE --benchmark FILE --report-out FILE | build-gameplay-gap-backlog --campaign FILE --audit FILE [--out FILE]'); }
