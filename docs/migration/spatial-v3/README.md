...
- This closes the four P12 source/target authoring gaps only. P27 independent acceptance, clean fresh-checkout/reproducibility evidence, release-authority approval, and the atomic P28 gate remain required. Production is not activated and v2 remains the sole production composition.
...
- P27-S01 through P27-S05 are accepted with `PASS WITH NOTES` for subject commit `45b4b9697663426a9543cd959bc927081334da5c`; the persisted report is `p27-exact-commit-critic-report.md`. A detached clean worktree, a fresh exact-branch clone, hosted `clean-clone-generation-test`, full `npm test`, real Chrome, the complete isolated spatial PostgreSQL matrix, two P23 concurrency passes, and all five explicit `PARTY_DATABASE_URL` tests passed. This acceptance is evidence about the subject tree, not P28 authority.
...
- Gate hardening: `release-evidence.v1.json` is the single versioned Appendix D manifest (all 58 mandatory items). To avoid an impossible commit-SHA self-reference, its exact `activation_candidate_commit` identifies the subject commit and the signed manifest is committed in one strict direct child evidence commit. That evidence commit must be the current HEAD, have the candidate as its sole parent, contain the exact assessed manifest bytes, and change only the manifest plus explicitly referenced files under `docs/migration/spatial-v3/`; arbitrary descendants, merge commits and runtime/composition files fail closed. The trust store and every configured public-key path are protected candidate roots and cannot change in the evidence commit. The manifest, trust store, public keys and every hashed evidence object must be tracked and byte-equal to their exact HEAD blobs; untracked or working-tree-only substitutions fail closed. The same candidate is bound into the P27 audit, every signed fresh-checkout item, and the release-authority decision. `activation-trust-store.v1.json` contains exactly the independently role-bound `p27_critic`, `fresh_checkout_attestor`, and `p28_release_authority` entries: every role requires a distinct key ID and a distinct canonical Ed25519 SPKI DER SHA-256 identity (not merely distinct PEM text); malformed, non-Ed25519, shared, unknown, revoked or role-misused keys fail closed. Evidence SHA-256 is calculated over raw bytes; paths are canonical repository-relative POSIX paths and reject empty, drive, UNC, traversal, ambiguous and realpath/symlink-escape forms. Each passed checklist item must name hash-verified evidence. The four P12 authoring gaps are individually pinned to their exact inventory quantities and require hash-verified resolution evidence. `assessSpatialV3Activation` exposes dependency injection only as a test seam; production `requireSpatialV3Activation` always reads the real repository and ignores every caller-supplied root/read/realpath/Git dependency as well as caller-provided approval objects.
- **Superseded quantitative blocker statement.** The approved compiled Novgorod authoring manifest now has **0/4 blocking P12 source/target data gaps**: the exact 195 / 358 / 600 / 17+195+195 inventories are digest-pinned and have isolated PostgreSQL readback evidence. The G1 production statements remain unchanged: `Production import: not_performed` and `Production readiness: not_verified`; P27, fresh-checkout, release-authority, and P28 evidence remain blocking.
- **Superseded P27-S05 absence statement.** Exact-subject P27-S05 acceptance is now persisted with `PASS WITH NOTES`. The current evidence commit is not yet the signed activation candidate, however: `p27_independent_critic`, Appendix D, fresh-checkout signature, candidate-commit binding and release-authority signature remain deliberately unpassed in `release-evidence.v1.json`. No production status is flipped, v2 stays the sole production composition, and no v3 request may use it as fallback.
