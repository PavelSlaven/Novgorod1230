...
| `@rus/space-map` | **не владелец runtime**; пакет удаляется вместе с потребителями (tools/map-maker, spatial-v3 checks, DEPENDENCY_RULES, tests, retrieval-policy, MODULE_INDEX; ACTIVE spatial §16.3 — CORPUS_EDIT, шаг #146) ([#133](https://github.com/PavelSlaven/Novgorod1230/issues/133#issuecomment-5839796951)) | не использовать как owner |
...
| `@rus/visibility-knowledge-memory` | visible projection, hidden boundary, knowledge map, memory facts, leak detection, safe narrator package, factual conversation audience projection, deterministic authored evidence resolution, typed composite completion и pure `portrait_spec_v1` projection из sanitized committed facts | prose generation, hidden-state mutation, DB reads, выбор responder, перенос private knowledge между NPC или legal consequence |
...
| `@rus/new-game` | пакеты живых стадий (8/13/14/compat/16 и др. на bootstrap v17 / `play:local` через `scripts/run-pr17-item-container-stage3c.mjs`); оркестратор `runModularNewGamePipeline` — **не** production-владелец (production-старт — композиция game-server, LW-026; см. [pipelines/new-game.md](../pipelines/new-game.md)) | physical DB transaction, HTTP |
| `@rus/materialization` | deterministic materialization traces; начальный выбор погоды/света (`projectApprovedCurrentEnvironment` через `target-current-factual-context`, v17) | database access / commit; живая смена погоды (target owner `@rus/environment-state`, D7; сейчас не меняется) |
| `@rus/environment-state` | применяет погоду и свет, считает моменты смены; каждое наступившее состояние — факт партии; данные — преемник `weather_transition_profiles_processes.json` (novgorod v2); следующее состояние выбирает turn через RandomSource (target, [#133](https://github.com/PavelSlaven/Novgorod1230/issues/133#issuecomment-5839745154) D7; сейчас runtime пакет не импортирует, погода после начальной не меняется) | начальный выбор (materialization); выбор следующего состояния (turn) |

## Spatial v3 production ownership (P08)

Historical P28 evidence did not itself activate production. На ветке PR #98 загружаемые
bindings — `spatial-v3-production-v16` (default) и `spatial-v3-production-v17` (пара БД /
`RUS_SPATIAL_V3_BINDINGS_MODULE`); sole-production claim «только v16» устарел (LW-033).
Dual write, mixed reads and compatibility fallback remain forbidden.

| Owner | Sole target responsibility | Explicit boundary |
|---|---|---|
| `@rus/space-map` | пакет удаляется вместе с потребителями ([#133](https://github.com/PavelSlaven/Novgorod1230/issues/133#issuecomment-5839796951); CORPUS_EDIT spatial §16.3 — #146) | do not treat as runtime owner |
...
| `@rus/game-server` | sole production composition after the completed `versioned production activation cutover` (target; текущий долг LW-026) | no duplicate spatial/movement/materialization logic |
...
responsibilities authoritative. Current selectable production bindings on this
branch are v16 and v17 (not «v16 only»). Older releases v2–v15 remain on disk
but are not selected by the loader; removal after M2c acceptance — [#133](https://github.com/PavelSlaven/Novgorod1230/issues/133#issuecomment-5836830425).
...
| `@rus/environment-state` | применяет погоду и свет, моменты смены; данные — преемник `weather_transition_profiles_processes.json` (novgorod v2); следующее состояние выбирает turn (RandomSource: seed партии, G0, интервал); начальное — materialization (target, [#133](https://github.com/PavelSlaven/Novgorod1230/issues/133#issuecomment-5839745154) D7; сейчас runtime пакет не импортирует, погода после начальной не меняется) | do not invent parallel weather catalogs |
