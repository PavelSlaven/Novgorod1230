...
> **Статус: completed / historical roadmap.** Phase 0–11 выполнены; Phase 11
> закрыта merged PR №61. Этот документ сохраняет причины архитектурного
> разбиения и первоначальные критерии фаз, но больше не является списком
> предстоящей реализации. Текущее production-состояние описано в
> [reference](lower-dvina-trace-reference.md), фактическая карта реализации —
> в [implementation map](lower-dvina-trace-implementation-plan.md).
