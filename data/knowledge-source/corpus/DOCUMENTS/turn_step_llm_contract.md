...
Существующий bounded decision по закрытым `option_id` остаётся допустимым только для действительно закрытых domain choices и materialization candidate sets. Эта активация не меняет NPC, conversation или combat semantics: их отдельные документы остаются `proposed` до собственного code cutover.
...
```json
{
  ...
  "next_domain_operation": null
}
```

`next_domain_operation` опционально резервирует один уже полностью
grounded domain request, когда профильному владельцу текущего шага нужно знать
его до завершения parent activity. Это не исполнение и не скрытая вторая
операция: следующий план обязан вернуть эту операцию заново, после чего код
повторно выполняет exact binding и applicability admission. Если следующий
domain request ещё нельзя назвать точно, поле отсутствует или равно `null`.
...
Если продолжение зависит от открытия контейнера, поиска, перемещения, проверки, NPC, боя, activity или нового видимого состояния, верни `continuation`. Заполняй `next_domain_operation` только когда следующий domain request уже однозначно grounded; не угадывай его до появления нового состояния.
