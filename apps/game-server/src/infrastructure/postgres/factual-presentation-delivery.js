...
export function buildFactualTurnDelivery({
  ...
  const screen = rebuildExpectedFactualTurnDelivery({ envelope, payload,
...
export function rebuildExpectedFactualTurnDelivery({ envelope, payload = envelope?.snapshot_payload,
  presentation = null, visibleContext = phase2VisibleContextFromPayload(envelope?.visible_payload) }) {
  const turnNumber = factualEnvelopeTurnNumber({ ...envelope, snapshot_payload: payload });
  if (turnNumber == null || payload == null || canonicalDigest(payload) !== envelope?.state_digest) {
    throw presentationError();
  }
  ...
  return createFactualTurnDeliveryScreenReadModel({
    ...
    uncertainties: visibleContext.uncertainties, actions: carrier.actions,
    checks: carrier.checks, panels: carrier.panels, actionPanel: carrier.action_panel,
...
export function factualTurnDeliveryMatchesExpected(screen, expected) {
  return validateLowerDvinaFactualTurnDeliveryScreen(screen).ok
    && canonicalDigest(screen) === canonicalDigest(expected);
}
