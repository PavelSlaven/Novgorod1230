...
const versions = () => ({ entries: [{ entity_ref: { entity_kind: 'actor', entity_id: 'actor' }, state_version: 1 }], canonical_digest: 'b'.repeat(64) });
...
const cost = { cost_kind: 'action', action_units_min: 1, action_units_max: 1, minutes_min: null, minutes_max: null, precision: 'exact', canonical_digest: 'c'.repeat(64) };
const risk = { risk_class: 'none', knowledge_precision: 'exact', canonical_digest: 'd'.repeat(64) };
...
function query(overrides = {}) {
  const value = {
    ...
    knowledge_subject_ref: null, recovery_binding_ref: null, administrative_authorization_pins: null, knowledge_scope: 'factual', cost_mode: 'action', capability_context: {}, expected_state_versions: versions(), planning_state_version: 1
  };
  return { ...value, ...overrides, canonical_digest: digest({ ...value, ...overrides }) };
}
...
test('P18 respects direction and does not derive travel from containment or reverse edges', async () => {
  ...
  const result = await planner([edge]).resolve(query({ target_request: null, intended_direction_id: 'west', canonical_digest: 'e'.repeat(64) }));
...
test('P18 resolves knowledge token separately and returns typed data gap without a resolver', async () => {
  const result = await planner([]).resolve(query({ knowledge_scope: 'character_known', knowledge_subject_ref: { entity_kind: 'actor', entity_id: 'actor' }, target_request: { target_kind: 'knowledge_spatial', factual_target_ref: null, knowledge_target_ref: { knowledge_kind: 'known_party_site', knowledge_id: 'belief' } }, canonical_digest: 'f'.repeat(64) }));
  assert.equal(result.ok, false); assert.equal(result.error.code, 'knowledge_target_resolution_gap');
});
...
test('P18 rejects sparse endpoint/static snapshots and carrier-local non-scene traversal', async () => {
  ...
  const local = await planner([connection]).resolve(query({ journey_scope: 'carrier_local', target_request: null, intended_direction_id: 'go', canonical_digest: 'f'.repeat(64) }));
...
test('P18 verifies dependency pin digests and readiness reason matrix', async () => {
  ...
  const pinResult = await createMovementPlanner({ loadTopology: async () => ({ ok: true, edges: [edge], target_resolution_dependency_pins: brokenPins }), snapshotEndpoint: async ({ endpoint_ref }) => snapshot(endpoint_ref) }).resolve(query());
