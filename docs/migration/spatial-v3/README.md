...
- **Superseded P12 recovery step.** P12 has closed the four authoring gaps with approved version-pinned target data, independent critic `PASS`, and isolated PostgreSQL import/readback evidence. The remaining recovery sequence is unchanged: P27 must persist an independent acceptable verdict for the exact candidate commit, clean fresh-checkout/reproducibility evidence and release authority must be recorded, and only then may P28 rerun before any atomic activation patch. These remain release blockers, not permissible notes.
