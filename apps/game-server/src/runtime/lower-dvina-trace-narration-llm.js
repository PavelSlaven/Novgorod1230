...
const UNRESOLVED_QUESTION_BOUNDARY = 'A current unresolved question explicitly supplied in visible_context.uncertainties is material: convey its subject and uncertainty naturally once. A quoted query supplies only the question, never evidence that its objects, ownership, history, or physical actions exist or occurred. Do not replace uncertainty with factual absence or narrate the query as an executed search. Omitting this material uncertainty MUST FAIL as missing_visible_change.';
...
async function runNarrationRole(roleRunner, roleId, instruction, request) {
  ...
  const response = await roleRunner.run({ scope: 'turn_runtime', role_id: roleId,
    ...
    messages: [{ role: 'system', content: `${OBSERVATION_SCOPE_BOUNDARY} ${UNRESOLVED_QUESTION_BOUNDARY} ${ATTEMPT_OUTCOME_BOUNDARY} ${instruction} ${proseBoundary} ${ELAPSED_ONLY_PERSISTENCE_BOUNDARY} ${ELAPSED_ONLY_LITERAL_GROUNDING} ${NPC_OBSERVATION_BOUNDARY}` },
