...
function requireSelectedOwnerOutput(outputs, operation, binding) {
  ...
  if (typeof op !== 'string'
      || [...directKinds, ...consequenceKinds].some((kind) => kind !== op)
  ...
  for (const fragment of outputs.write_fragments) {
    requireTurnStepOwnerCarrierBinding({ ...binding, semanticOperation: operation,
...
function requireOwnerCarrierBindings({ ordinaryPlan, actionProductionPlans,
  ...
  for (const plan of actionProductionPlans) {
    ...
    carrierOperation: { ...semanticOperation,
      ...semanticOperation.action_production },
  ...
  for (const plan of localFirePlans) {
    ...
    requireTurnStepOwnerCarrierBinding({ ...binding, carrier: {
      ...
      process_ref: semanticOperation.process_action === 'affect'
