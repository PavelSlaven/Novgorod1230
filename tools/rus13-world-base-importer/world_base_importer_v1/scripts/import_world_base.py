...
def transform_region_profile(path: Path, dataset: Dict[str, Any]) -> List[Batch]:
    ...
    return [Batch(table="regions", rows=[normalize_row(row, "regions")], dataset_name=dataset["name"], priority=dataset.get("priority", 0), source_path=dataset["path"])]
...
def transform_region_neighbors(path: Path, dataset: Dict[str, Any]) -> List[Batch]:
    ...
    return [
        Batch(table="regions", rows=rows_regions, dataset_name=dataset["name"] + "_stubs", priority=dataset.get("priority", 0) - 1, source_path=dataset["path"]),
        Batch(table="region_neighbors", rows=rows_neighbors, dataset_name=dataset["name"], priority=dataset.get("priority", 0), source_path=dataset["path"]),
    ]
...
def load_dataset(input_root: Path, dataset: Dict[str, Any], manifest: Dict[str, Any]) -> List[Batch]:
    ...
    if not path.exists():
        return [Batch(table=dataset.get("table", "unknown"), rows=[], dataset_name=dataset["name"], priority=dataset.get("priority", 1000), source_path=dataset["path"], warnings=[f"File not found: {dataset['path']}"])]
    fmt = dataset["format"]
    ...
    return [Batch(table=table, rows=rows_out, dataset_name=dataset["name"], priority=dataset.get("priority", 1000), source_path=dataset["path"])]
...
def validate_batches(batches: List[Batch]) -> Dict[str, Any]:
    ...
    for b in batches:
        ...
        table_rows.setdefault(b.table, []).extend(b.rows)
    ...
    for table, rows in table_rows.items():
        payload = json.dumps(sorted(rows, key=lambda row: str(row.get("id", ""))),
                             ensure_ascii=False, sort_keys=True,
                             separators=(",", ":")).encode("utf-8")
        summary["tables"][table]["payload_sha256"] = hashlib.sha256(payload).hexdigest()
