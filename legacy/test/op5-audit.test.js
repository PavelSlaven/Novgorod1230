...
test('handlePlayerInput does not mutate position before master response', async () => {
  ...
  globalThis.fetch = async (_url, options) => {
    ...
    throw new Error('stop before master commit');
  };

  try {
    await assert.rejects(
      () => handlePlayerInput(world, 'иду к реке'),
      /stop before master commit/
    );
    assert.deepEqual(world.current_position, beforePosition);
  } finally {
...
test('handlePlayerInput rolls back world when master response fails', async () => {
  const world = createWorldState({ startText: 'переправа и двор' });
  ...
  globalThis.fetch = async (_url, options) => {
    ...
    throw new Error('master stage failed');
  };
