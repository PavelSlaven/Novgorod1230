...
export async function readPartyDatabaseSchemaSnapshot(partyPool) {
  ...
  const tables = PHASE_1A_TABLES.map((name) => ({
    ...
    columns: metadata.columns.filter((row) => row.table_name === name)
      .map((row) => ({
  ...
  const foreignKeys = metadata.foreignKeys.flatMap((row) =>
    row.columns.map((column, index) => ({
      name: row.name, table: row.table_name, from_table: row.table_name,
      from_column: column, columns: row.columns,
      ...
      column_ordinal: index, on_delete_code: row.on_delete_code,
      on_update_code: row.on_update_code
    })));
  const uniqueConstraints = metadata.uniqueConstraints.map((row) => ({
    name: row.name, table: row.table_name, columns: row.columns,
    constraint_type: row.constraint_type === 'p' ? 'primary_key' : 'unique'
  }));
  ...
  const enumDefinitions = metadata.enumDefinitions.map((row) => ({
    enum_name: row.enum_name, values: row.values
  }));
  const indexes = metadata.indexes.map((row) => ({
    name: row.name, table: row.table_name, definition: row.definition
  }));
  ...
  if (tables.some((table) => table.columns.length === 0)
    ...
    fail('TRACE_PHASE_1B_PARTY_SCHEMA_INCOMPLETE',
      `The complete Phase 1A party schema must be installed before public start: ${missingContracts.join(',')}.`);
  }
  const snapshot = {
    version: 1, schema: 'party_database_schema_snapshot',
    schema_version: 'party_runtime_v2', tables, columns: [],
    foreign_keys: foreignKeys, unique_constraints: uniqueConstraints,
    check_constraints: checkConstraints, enum_definitions: enumDefinitions,
    indexes, allowed_operations: ['insert_only']
  };
  return Object.freeze({ ...snapshot, readonly_checksum: digest(snapshot) });
}

export async function readWorldBaseReferenceSnapshot(worldPool, compatibility) {
  requirePool(worldPool, 'worldPool');
  const ids = [compatibility?.source_world_revision_id,
    ...(compatibility?.lineage ?? []).map((entry) => entry.world_revision_id)];
  if (ids.some((id) => !id) || new Set(ids).size !== ids.length) {
    fail('TRACE_PHASE_1B_WORLD_LINEAGE_INVALID',
  ...
  if (rows.length !== ids.length) {
    fail('TRACE_PHASE_1B_WORLD_LINEAGE_MISSING',
  ...
  if (source?.catalog_digest !== compatibility.source_world_catalog_digest
      || compatibility.source_status !== 'approved'
      || source.status !== compatibility.source_status) {
    fail('TRACE_PHASE_1B_WORLD_LINEAGE_MISMATCH',
  ...
  for (const ref of compatibility.lineage) {
    ...
    if (row?.parent_revision_id !== parent
        ...
        || ref.status !== 'approved' || row.status !== ref.status) {
      fail('TRACE_PHASE_1B_WORLD_LINEAGE_MISMATCH',
  ...
  const [closures, canonicalG5] = await Promise.all([
    Promise.all(sceneIds.map((id) => reader.readPinnedSceneTemplateClosure({
      id, version: 1,
      world_revision_id: compatibility.production_world_revision_id
    }))),
    Promise.all(canonicalIds.map((id) => reader.readPinnedCanonicalG5SceneBinding({
      id, version: 1,
      world_revision_id: compatibility.production_world_revision_id
    })))
  ]);
  if ([...closures, ...canonicalG5].some(({ ok }) => !ok)) {
    fail('TRACE_S1_WORLD_CATALOG_GAP',
      'S1 template closure is unavailable from world-base.');
  }
  return Object.freeze({
    version: 1, schema: 'world_base_reference_snapshot',
    readonly_checksum: digest({
      rows, closures: closures.map(({ value }) => value),
    ...
    allowed_source_ids: [],
    scene_template_closures: closures.map(({ value }) => value),
