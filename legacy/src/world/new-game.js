...
import { loadRegionCatalog, pickRandomRusRegion } from './region-catalog.js';
...
export async function createFreshWorld(options = {}) {
  ...
  const placeSeedResult = await generatePlaceSeed(world, env, hooks);
  applyGeneratedPlaceSeed(world, placeSeedResult.data);

  const socialTissueResult = await generateSocialTissue(world, env, hooks);
  applyGeneratedSocialTissue(world, socialTissueResult.data);

  const playerSeedResult = await generatePlayerSeed(world, env, hooks);
  applyGeneratedPlayerSeed(world, playerSeedResult.data);

  const profileResult = await generateActorProfiles(world, env, hooks);
  applyGeneratedActorProfiles(world, profileResult.data);

  const locationProfileResult = await generateLocationProfiles(world, env, hooks);
  ...
  const openingMasterResult = await generateMasterResponse(openingPlan.frame, 'opening_scene', env, hooks);
  const openingNarrative = openingMasterResult?.narrative;
  if (!openingNarrative?.scene) {
    throw new Error('LLM не вернул master_narrative для первой сцены.');
  }
  const visibleContextResult = await generateVisibleContextPackage(world, openingNarrative, env, hooks);
  const openingProseResult = await generateNarratorProse(openingPlan.frame, visibleContextResult.data, env, hooks);
