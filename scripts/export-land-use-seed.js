...
const xlsxPath = path.join(root, 'infra/world-base/land_use_templates_scientific_v3_fixed.xlsx');
const outPath = path.join(root, 'infra/world-base/land_use_templates.seed.json');
...
JSON_ARRAY_COLS = {
    "compatible_landscape_template_ids", "compatible_water_body_template_ids", "sources",
}
...
for row_idx in range(1, max_row + 1):
    ...
    for key, val in raw.items():
        ...
        elif key in ("id", "slug", "title", "land_use_kind", "status", "confidence"):
...
if (result.status !== 0) {
  console.error(result.stderr || 'export-land-use-seed: python failed');
