...
| `movement_locations_regions.txt` | Правила перемещения через иерархию позиции, сохранённые journey/legs, route и course только по fact graph, actual/perceived navigation и атомарный первый вход в G4. | При любом перемещении внутри места, между местами, по дороге, лесу, воде, региону; при попытке идти в направлении, искать место, вернуться назад или заблудиться. |
...
| Переместить персонажа, построить маршрут, вернуть назад, заблудиться | `movement_locations_regions.txt` | `time_system.txt`, `world_generation_and_turns.txt`, `read_only_database_and_graph_architecture.md`, `world_base_materialization_table_requirements.md` |
