...
Все методы принимают структурированный объект. Результаты возвращаются как новые глубоко замороженные объекты. Перед RAG-операцией проверяются SHA-256 и размер каждого канонического документа, а также source location каждого committed chunk. Отсутствующие или повреждённые источники завершаются typed failure. Явно возвращаемый конфликт не становится обычным контекстом поиска: он сохраняет собственный статус и полный provenance (`canonical_path`, SHA-256, диапазон строк, нормативный приоритет). CLI является тонким adapter-слоем и не дублирует retrieval logic.
