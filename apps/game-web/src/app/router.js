...
function renderNewGame({ scenarios = [], theme = 'light', loading = false } = {}) {
  ...
  const disabled = loading ? ' disabled' : '';
  return `<div class="new-game-page"><header class="game-header"><button class="brand-button" type="button" data-return-start${disabled}><span>Хроника</span><strong>Русь</strong></button><div class="header-actions"><button class="icon-button" type="button" data-llm-settings-open aria-label="Настройки LLM">⚙</button><button class="icon-button" type="button" data-theme-toggle aria-label="Сменить тему">${themeIcon(theme)}</button></div></header><main class="new-game-screen" data-new-game-screen><section class="new-game-intro"><p class="eyebrow">Новая запись</p><h1>Выбери историю</h1><p>Каждая история начинается в своём месте и времени, а дальше ты действуешь свободно.</p></section><section class="scenario-section"><div class="section-heading"><p class="eyebrow">Опубликованные истории</p><h2>Сценарии</h2></div><div class="scenario-list">${scenarioButtons || '<p class="empty-state">Опубликованных сценариев сейчас нет.</p>'}</div><div class="form-actions"><button class="button-quiet" type="button" data-return-start${disabled}>Назад</button></div></section></main></div>`;
}
