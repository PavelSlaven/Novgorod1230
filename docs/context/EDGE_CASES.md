...
Примеры для unseen-equivalent и regression; handlers из них не выводят. Замысел — [PC §9.1](../governance/PRODUCT_CONSTITUTION.md);
решения владельца — [#133](https://github.com/PavelSlaven/Novgorod1230/issues/133).

| Класс проверки | Пример намерения | Ожидаемый характер исхода |
|---|---|---|
| Осмотр места (первый) | «смотрю вокруг», «осматриваю поляну» | один сохраняемый presence-бросок по применимым категориям; повтор не материализует заново |
| Открытие контейнера (первый) | «открываю сундук», «заглядываю в мешок» | содержимое по container scope; повтор — только уже существующее |
| Поиск / умение | «ищу ягоды», «тщательно осматриваю» | умение влияет на поиск, не на presence-reroll |
| Копание / изъятие природного | «копаю глину», «рублю дерево» | только природные конечные запасы места; рукотворное — только при причине |
| Изготовление (A1) | «делаю копьё», «подделываю грамоту» | ресурс + инструмент + навык + технология; не «невозможно» из-за technical gap |
| Ограниченный класс с ненулевой вероятностью | «ищу меч в лесу», «ищу монету» | низкая presence-вероятность допустима; typed gap — только уникальное/квестовое |
| Technical gap | действие, для которого нет механики | игроку не показывают «невозможно» как закон мира (LW-030) |

Нормы mechanics — `items_and_property.txt`, `turn_step_llm_contract.md`, code-driven materialization (CONTRACT_INDEX).
