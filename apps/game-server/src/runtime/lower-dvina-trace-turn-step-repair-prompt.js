export function turnStepRepairSpecificInstructions(repairContext, request) {
  ...
  if (codes.has('direct_result_kind')) instructions.push(
    'Required repair: classify the successful write-free direct result with direct_result_kind player_safe_observation when it reviews only supplied player-safe facts, otherwise no_state_gesture. Do not leave direct_result_kind null.'
  );
