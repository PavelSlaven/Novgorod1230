...
const xlsxPath = path.join(root, 'infra/world-base/water_body_templates_scientific_v2_fixed.xlsx');
const outPath = path.join(root, 'infra/world-base/water_body_templates.seed.json');
...
BOOL_COLS = {
    ...
    "supports_bridge", "supports_winter_crossing",
}
...
for row_idx in range(1, max_row + 1):
    ...
    for key, val in raw.items():
        if key in BOOL_COLS:
            ...
            out[key] = parse_sources(val)
        elif key in ("id", "slug", "title", "water_body_type", "salinity", "status", "confidence"):
...
if (result.status !== 0) {
  console.error(result.stderr || 'export-water-body-seed: python failed');
