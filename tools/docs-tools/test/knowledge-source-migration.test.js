...
test('migrated corpus and generated provenance verify without requiring legacy', { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.equal(result.document_count, 19);
...
test('graph and RAG materializers are deterministic and bound to the new corpus', async () => {
  ...
  assert.equal(graphA.manifest.source_document_count, 19);
  ...
  assert.equal(ragA.manifest.source_document_count, 19);
