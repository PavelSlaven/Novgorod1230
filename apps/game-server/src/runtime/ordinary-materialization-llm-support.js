...
export function buildOrdinaryMaterializationMessages(request, { repair = null,
  ...
  if (isAbsentPresence && responseShape != null) {
    ...
    if (request.mode === 'seed_scope') instructions.push(
      ...
      'For materialize return one entity containing semantic_type, presence_expectation, and mechanics_proposal. semantic_type must be a specific nonempty ordinary semantic type for the actual proposed material or object, not null or a copied placeholder. It is a machine category, not a player-facing name or factual description. Do not return a name, facts, origin, history, condition, or other unsupported property. Classify the concrete referent; the generic authority candidate category does not supply its specific material semantics.',
