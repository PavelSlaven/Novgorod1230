...
async function runNarrationRole(roleRunner, roleId, instruction, request) {
  ...
  const response = await roleRunner.run({ scope: 'turn_runtime', role_id: roleId,
    ...
    messages: [{ role: 'system', content: `${instruction} ${proseBoundary} ${ELAPSED_ONLY_PERSISTENCE_BOUNDARY} ${ELAPSED_ONLY_LITERAL_GROUNDING} ${NPC_OBSERVATION_BOUNDARY}` },
