...
test('Phase 6 PostgreSQL carry persists exact terminal, restart/resume, rechecks and rollback', async (t) => {
  ...
  assert.equal(docker(['run', ...testContainerLabel(), '-d', '--name', name, '-p', '127.0.0.1::5432', '-e', 'POSTGRES_PASSWORD=local_only', '-e', 'POSTGRES_USER=phase6', '-e', 'POSTGRES_DB=pr17_phase6', 'postgres:16-alpine']).status, 0);
