...
Выборка смешанная: для batch из 100 случаев 75 генерируются по независимым
от corpus областям потребностей, а 25 — свободно, без заданной области.
Для большего batch свободная часть составляет не менее четверти; доли
фиксируются до генерации. Стратифицированная часть распределяет случаи
между трудом и ремёслами,
...
Прежние 20 областей по пять случаев не являются постоянным экзаменом.
Состав и детализация strata меняются между batches: области можно добавлять,
разделять и пересекать по модели реальности, а не только по текущему corpus.
В контролируемой части из 75 случаев не менее 60 имеют основную практическую,
природную или физическую потребность; оставшаяся часть охватывает социальную
и институциональную жизнь. Это сохраняет разнообразие без навязывания квот
свободному генератору и заменяет прежнюю квоту 80/20 для всей сотни.

Свободную часть создаёт отдельный независимый random/adversarial генератор.
Он получает историческую рамку и задачу придумать разные осмысленные игровые
ситуации, но не получает corpus, cartography, список primary domains, квоты,
примеры прошлых batches или ситуации стратифицированной части. Основная
область и пересечения классифицируются только после фиксации свободных
ситуаций. Новая или не укладывающаяся в старые strata область допускается
без whitelist; её отмечают как потенциальную слепую зону cartography.
Такие случаи нельзя удалять, переписывать или переклассифицировать ради
попадания в старые двадцать областей либо получения чистого batch.

Случаи внутри области тоже различаются по необходимому знанию, а не только
именам и декорациям. Для контролируемой части генератор сохраняет основную
область и конкретную потребность в знании; для свободной их добавляет
проверяющий, не меняя исходную ситуацию. Пересечения областей допустимы.
Область засчитывается
...
До засчитывания batch публикуются доли обоих способов генерации,
фактическое распределение областей, новые/непокрытые области и пропуски;
...
смешанные выборки подряд, не менее 100 ситуаций каждая, без новых существенных gaps.
...
Предыдущие полностью стратифицированные batches сохраняются как диагностика
и baseline, но не засчитываются в новую серию смешанной приёмки.
...
## 0.4. Place-first environment cartography

Статическое наполнение дополнительно идёт от места и практически значимой
среды, не только от занятия, отдельного вопроса или существующих claims.
`production-v1/place-first-cartography.json` дополняет category cartography
композиционными environment envelopes: потребности → approved WK premises
→ применимость, варианты и ограничения. Это authoring-карта, не второй
materializer, whitelist мест/объектов или готовые декорации для партии.

Семейства сред проектируются независимо от corpus. Перечень открыт:
городские и сельские пространства, вода и берега, открытые и лесные
ландшафты, переходы, жилые, хозяйственные, производственные и
институциональные помещения. Примеры пользователя не ограничивают поиск.
Среды сочетаются: мастерская внутри двора, мост у пристани, лесная дорога
у болота. Противоречивые условия нельзя механически смешивать.

Для каждой среды проверяются поверхности и материалы; растения и животные;
вода и грунт; постройки и предметные возможности; запахи, звуки и видимость;
занятия, работа и передвижение; следы; опасности; изменения по сезону,
погоде, времени суток, достатку и уходу. Это минимальные измерения качества,
не закрытый список environmental families. Нужны характерные отношения
и вариативность, не одинаковые общие фразы или списки предметов.

Потребность имеет реальные claim refs, ограничения и честный статус
`unassessed`, `missing`, `partial` или `supported`. Последний означает
пригодность для обозначенной композиционной потребности, не полноту места.
Пустые refs, число facets и валидный JSON не заменяют содержательный аудит.
Недостающие отношения дополняются в существующем corpus с независимым
source/domain либо reconstruction review по §0.2 и §35.1. Непроверенная
карта потребностей сама не является runtime factual authority.

После наполнения независимые генераторы фиксируют сочетания
`место × сезон × погода × время суток × человек × занятие`, варьируя
также достаток и уход. Сохраняется смешанная дисциплина §0.3: большая
часть контролирует разнообразие, свободная часть не получает список мест,
corpus, прежние примеры или ответы. Исходные сочетания нельзя исправлять
ради чистой серии; новые и смешанные среды допускаются без whitelist.

Другой агент только по WK полноценно восстанавливает окружающую сцену
и взаимодействие человека со средой. Проверяются наблюдаемые свойства,
опора и проходимость, материалы и процессы, источники звуков и запахов,
занятия, следы и опасности. Ordinary-реконструкция обозначается и
композиционно обосновывается. Оговорка «это определяет state» не заменяет
отсутствующую среду: необходимость произвольно придумать существенное
окружение или причинную связь означает knowledge gap. Затем следуют
обычный review, наполнение и новая независимая выборка.

Place-first выборки входят в статическую приёмку и могут засчитываться
в серию §0.3 при сохранении её масштаба, разнообразия и независимости.
Прежние situation-first проверки без восстановления среды не заменяют
этот проход. Это не дополнительный runtime gate и не разрешение live
gameplay или ремонта иных owners. Envelope не создаёт committed presence,
canonical geography, actor knowledge, ownership, finite resources или
exact mechanics. Category → template → profile → rule → instance
остаётся у существующих owners.

## 0.5. Military-first factual cartography

Статическое наполнение отдельно проверяется от военной ситуации, а не только
от отдельных находок оружия или текущего corpus. Authoring-карта
`production-v1/military-first-cartography.json` содержит открытые factual
families, потребности, применимость, approved claim refs, остаточные пробелы
и границы code owners. Это sampling/research aid, не combat ruleset, готовый
сценарий, roster войска, materializer или whitelist допустимых военных действий.

Карта охватывает вооружённых и невооружённых участников, статус и обязанности;
оружие, защиту, ношение, повреждение, обслуживание и ремонт; обучение;
строй, командование и сигналы; марш, лагерь, разведку и засаду; пешее, конное
и стрелковое действие; снабжение, транспорт и переправы; осаду, городскую
оборону, огонь и дым; местность, погоду и освещение; ранения, первую помощь
и вынос; плен, бегство, преследование и трофеи; последствия для мирных жителей,
хозяйства и среды. Эти измерения направляют поиск и могут расширяться либо
делиться: они не являются закрытой таксономией.

Каждая выборка следует смешанной дисциплине §0.3: 75 контролируемых probes
распределяются по меняющимся и пересекающимся военным factual needs, а 25
создаёт отдельный свободный/adversarial генератор без списка областей, corpus,
cartography, прежних примеров или ответов. Несколько blind generators фиксируют
место, местность, сезон, погоду, время, человека, занятие, force context,
экипировку и её состояние. Состав strata ротируется и расширяется; повторение
одного и того же экзамена не является доказательством покрытия.

Отдельные WK-only agents получают замороженные ситуации и восстанавливают,
что участникам и среде уже задано, как имеющаяся экипировка качественно
используется, какие физические и человеческие ограничения действуют, как
организуются работа, движение, связь и снабжение, какие последствия возможны.
Они не добавляют предметы, войско, команды, врага, владение, рану или исход в
committed state. Существенная посылка, без которой приходится произвольно
придумывать работу оружия, защиты, людей, снабжения или среды, является gap.
Редкая деталь, exact combat number либо несколько равноправных ordinary
вариантов отдельного claim не требуют.

Gaps проходят обычное статическое наполнение и независимую проверку §0.2 и
§35.1; затем повторяются только известные cases и создаётся новая unseen
выборка. Точные hit/damage/body/movement/inventory/time/RNG/persistence и
combat outcomes остаются у существующих code owners. Military-first проход
не разрешает live gameplay, ремонт combat или иных gameplay owners и не
утверждает gameplay saturation.
