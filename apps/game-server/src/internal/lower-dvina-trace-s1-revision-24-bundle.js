...
export async function loadLowerDvinaTraceRevision24Bundle({ rootDir, historicalBundle,
  ...
  if (![historicalBundle?.definition_revision === 23,
    ...
    exactRef(manifest.content_refs?.definition, loaded.definition, 'definition.json', 'lower_dvina_trace_v1', 24),
    ...
    phase1a?.package_id === 'lower_dvina_trace_phase_1a_v20', phase1a.scenario_definition_revision === 24,
    ...
    bindings.spatial_semantic_materialization?.fallback_policy === 'forbidden'].every(Boolean)) return fail(
    'TRACE_REVISION_24_CONTENT_INVALID', 'Revision 24 first-entry content is stale or incomplete.');
  ...
  const bundle = { ...structuredClone(historicalBundle), definition_revision: 24,
    ...
    spatial_semantic_profile: loaded.spatial_semantic_profile.value, materialization_bindings: mergedBindings,
