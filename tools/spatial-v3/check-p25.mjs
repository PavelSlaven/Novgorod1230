...
for (const token of ['bindSpatialV3RequestProfile', 'composition_profile_bindings_required', 'composition_profile_binding_conflict', 'runSpatialV3StructuralShadow', 'shadow_divergence_registry_unconsumed', 'shadow_divergence_registry_duplicate', 'runSpatialV3CutoverRehearsal', 'runSpatialV3RollbackDrill', 'production_v2', 'shadow_v3', 'target_rehearsal', 'rollback_restore_required']) if (!tool.includes(token)) throw new Error(`P25 tooling lacks ${token}`);
for (const token of ['dual_writer_forbidden', 'composition_profile_binding_conflict', 'shadow_divergence_registry_unconsumed', 'shadow_divergence_registry_duplicate', 'shadow_write_forbidden', 'cutover_startup_probe_failed', 'local PostgreSQL snapshot/restore']) if (!test.includes(token)) throw new Error(`P25 test lacks ${token}`);
