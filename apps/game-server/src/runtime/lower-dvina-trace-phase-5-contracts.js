...
export function resolveTracePhase5Contracts({ state, bundle }) {
  if (![11, 12, 13, 14].includes(bundle.definition_revision)
      || ![11, 12, 13, 14].includes(bundle.definition?.revision)) {
  ...
  const ropeInventoryProfile = [12, 13, 14].includes(bundle.definition_revision)
