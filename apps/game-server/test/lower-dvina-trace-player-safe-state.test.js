...
test('camp fire observation follows the committed environment snapshot', () => {
  const state = richCommittedState();
  ...
  state.current_visible_context = { visible_scene: 'Рыбацкий стан',
    sensory_details: ['На сухом берегу стоят навес и очаговая площадка.'] };

  const before = projectLowerDvinaTracePlayerSafeState({
    committed_state: state, actor_id: state.actor_id
  }).player_safe_state.current_visible_context.sensory_details;
  ...
  assert.equal(after.some((fact) => fact.includes('не видно ни пламени')), false);
});
