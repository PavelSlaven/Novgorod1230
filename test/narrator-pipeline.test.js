...
function createFreshWorldRecoveryFetchMock({ trackStage, onVisibleShapeInput } = {}) {
  return async (_url, options) => {
    const body = JSON.parse(options.body);
    const systemText = body.messages?.[0]?.content ?? '';
    const userText = String(body.messages?.[1]?.content ?? '');
    ...
    if (/MasterNarrativeRecoveryRepairer/i.test(systemText)) {
      track('master_narrative_upstream_repair');
      return mockChoice({
        version: 1,
        schema: 'master_narrative',
        scene: 'Открытие: двор у переправы.',
        consequence: 'Видно только то, что доступно с места.',
        visible_details: ['двор', 'переправа'],
        npc_reactions: [],
        npc_reaction_refs: [],
        next_pressure: 'Осмотр может привлечь внимание, если рядом окажутся люди.'
      });
    }
    ...
    if (/MasterNarrativeShaper/i.test(systemText)) {
      ...
      return mockChoice({
        ...
        consequence: 'Видна утренняя сцена.',
        visible_details: ['двор', 'переправа'],
        npc_reactions: ['человек в тулупе выглядывает из дверей избы'],
        npc_reaction_refs: [{
          actor_ref: 'npc:unapproved-stranger',
          reaction_text: 'человек в тулупе выглядывает из дверей избы',
          source_stage: 'scene_visible_actor_projection',
          visibility_status: 'visible',
          approved: false
        }],
        next_pressure: 'следующий шаг может вызвать вопрос или оклик'
      });
    }

    if (/VisibleContextShaper/i.test(systemText)) {
      track('visible_context_shape');
      onVisibleShapeInput?.(payload?.input?.narrative ?? null);
      return mockChoice(mockVisiblePackage({
        visible_scene: String(payload?.input?.narrative?.scene ?? 'Открытие: двор у переправы.'),
        visible_npc: []
      }));
    }

    return createFreshWorldFetchMock({ trackStage })(_url, options);
  };
}
...
test('generateVisibleContextPackage returns recovery route before visible context dossier when master narrative has unapproved visible npc', async () => {
  const prev = process.env.NODE_TEST_CONTEXT;
  delete process.env.NODE_TEST_CONTEXT;
  ...
  globalThis.fetch = async () => {
    fetchCalled = true;
    return mockChoice('unexpected');
  };
  try {
    const world = createWorldState({ startText: 'двор' });
    await assert.rejects(
      () => generateVisibleContextPackage(world, {
        scene: 'Двор.',
        consequence: 'Тихо.',
        visible_details: ['двор'],
        npc_reactions: ['человек в тулупе выглядывает из дверей избы'],
        next_pressure: 'может окликнуть'
      }, { DEEPSEEK_API_KEY: 'test-key' }),
      (error) => {
        assert.equal(error?.semanticRecoveryRoute?.schema, 'semantic_recovery_route');
        assert.equal(error?.semanticRecoveryRoute?.class, 'upstream_repair');
        assert.equal(error?.semanticRecoveryRoute?.repair_target_stage, 'master_narrative');
        assert.equal(error?.semanticRecoveryRoute?.reason_code, 'MASTER_NARRATIVE_UNAPPROVED_VISIBLE_NPC');
        return true;
      }
    );
...
test('production visible context package records LLM journal events', async () => {
  ...
  try {
    ...
    assert.ok(snapshot.some((entry) => entry.roleId === 'legacy.visible_context.dossier'));
    assert.ok(snapshot.some((entry) => entry.roleId === 'legacy.visible_context.audit'));
    assert.ok(snapshot.some((entry) => entry.roleId === 'legacy.visible_context.shaper'));
...
test('createFreshWorld repairs unapproved visible npc upstream before visible context', async () => {
  const prev = process.env.NODE_TEST_CONTEXT;
  delete process.env.NODE_TEST_CONTEXT;
  const originalFetch = globalThis.fetch;
  const stages = [];
  let visibleShapeNarrative = null;
  globalThis.fetch = createFreshWorldRecoveryFetchMock({
    trackStage: (name) => stages.push(name),
    onVisibleShapeInput: (narrative) => { visibleShapeNarrative = narrative; }
  });
  try {
    const result = await createFreshWorld({
      startText: 'двор у переправы',
    ...
    assert.ok(result.openingText.length > 0);
    assert.ok(stages.includes('master_narrative_upstream_repair'), stages.join(','));
    assert.ok(stages.includes('visible_context_shape'), stages.join(','));
    assert.deepEqual(visibleShapeNarrative?.npc_reactions ?? null, []);
  } finally {
    globalThis.fetch = originalFetch;
    if (prev !== undefined) process.env.NODE_TEST_CONTEXT = prev;
  }
