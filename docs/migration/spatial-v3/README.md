...
- This closes the four P12 source/target authoring gaps only. The atomic P28 gate still requires completed Appendix D evidence and its GitHub release proof. Production is not activated and v2 remains the sole production composition.
...
- Current deferred state: the approved Novgorod P12 compilation has **0/4** source/target data gaps, but all **58** Appendix D rows are still `blocked`; PR #14 is draft and no exact-head GitHub proof exists. No production status changes, v2 stays sole production composition, and no v3 request may use P28 as fallback.
...
| P28 | activation gate and GitHub-proof tests | GitHub-proof authority is implemented; activation assessment remains intentionally fail-closed | all 58 Appendix D rows, exact PR approval/check/completion proof are absent; no activation patch is authorized |
