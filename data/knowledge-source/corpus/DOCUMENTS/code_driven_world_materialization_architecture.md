...
Active A1 revision 21 использует sole player boundary `turn_step_request_v1 → turn_step_plan_v1`: qualitative physical result вложен в `request_item_use`, после чего code-owned owner не вызывает LLM. Один или несколько видимых actor-held non-container material sources и ноль или несколько видимых actor-held/controller-controlled tools явно разделены и могут иметь другого legal owner. Profile не содержит template/recipe whitelist и допускает preserve, до четырёх finite independent outputs, no-result, partial/nonworking/waste, writing, non-authoritative token-like и closed weapon-capable outcomes. Code перечитывает committed mechanics/property/placement/ownership, выводит exact source/output mechanics из реально списанных allocations, возвращает typed gap при нецелимой дискретной механике, владеет finite conservation и сохраняет всё в combined P16. Model не задаёт numbers, canonical identity, currency/official status, truth или combat mechanics; weapon class получает формальные числа только в `@rus/combat-health`.
