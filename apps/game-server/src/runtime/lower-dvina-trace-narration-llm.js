...
const UNRESOLVED_QUESTION_BOUNDARY = 'A current unresolved question explicitly supplied in visible_context.uncertainties is material: convey its subject and uncertainty naturally once. A quoted query supplies only the question, never evidence that its objects, ownership, history, or physical actions exist or occurred. Preserve the supplied certainty of each factual proposition, including those embedded in relative clauses, participles, or noun phrases. A hedge on one proposition does not by itself qualify another. Do not narrate the query as an executed search. Omitting this material uncertainty MUST FAIL as missing_visible_change.';
