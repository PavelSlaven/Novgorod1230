...
Этот документ задаёт полный действующий контракт автономного решения NPC вне отдельного режима разговора.
...
### 7.0. Восприятие вещи и узнавание (D14)

Действующая норма; код v17 — долг CR реализации M2c (LW-028/LW-029).

Результат «узнал / не узнал» вещь по приметам определяет код при восприятии (видимость, свет, расстояние, спрятана ли; приметы — фасет `condition` и пулы). LLM не решает, узнал ли NPC вещь. Типовую вещь без примет узнают только по обстоятельствам. Запись владения — `@rus/items-property`; знание примет — `@rus/visibility-knowledge-memory`; правовые последствия — `@rus/social-law` ([#133 D14](https://github.com/PavelSlaven/Novgorod1230/issues/133#issuecomment-5844152943)).
...
Инвариант D9 — как в turn_step §11.1: `name` — качественная метка; имена, даты, эмитент, текст грамоты и клеймо — только из пула кода или существующей сущности.
...
`mechanics_proposal` необязателен (качественная оценка или предложение). Финальные числа (`mass_grams`, `external_hand_cost`, packing) пишет код.
...
- уникальный, квестовый, authored/hidden или сюжетно значимый предмет без authority-записи / code-owned основания (D10);
- останки, клад или улику как случайную выдумку без presence/authority;
- contents закрытого контейнера до первого открытия.
Оружие, деньги и документы не запрещены классом: находятся через presence, изготавливаются через A1, не выдумываются как ambient без сохранённого исхода.
...
LLM даёт категорию и, по желанию, `mechanics_proposal`; финальные значения присваивает код (§18.1 / §18.4; CR #146 шаг 2). Действующая норма; код v17 — долг CR реализации M2c (LW-028/LW-029).

`mass_grams` (финал — код):
...
- финальное значение пишет код.

`external_hand_cost` принимает только `0`, `1` или `2` (код).
...
Используется только после фактического физического изменения сущности. LLM может предложить qualitative изменение через необязательный `mechanics_proposal`; числовые mechanics пишет код (CR #146 п.8; REVIEW-028).
...
```json
{
  ...
  "mechanics_proposal": {
    "qualitative_mass": "легче прежнего",
    "external_hand_cost": 1,
    "carry_form": "long",
    "packing_slot_cost": 3,
    "quantity": null,
...
- perception result — `recognized` как неизвестный человек (тот же результат `recognized` / не узнал код может выдать и для вещи по приметам — §7.0 / D14);
...
Модель возвращает семантический выбор; код собирает `npc_step_plan_v1` по §16 и операциям §18. Identity и deterministic envelope пишет код.

### Инварианты

1. Одно самостоятельное действие от лица конкретного NPC по субъективному context.
2. Мир обычный, немагический, если переданное состояние прямо не говорит иное; NPC — обычный человек, если его сохранённое состояние не содержит необычной способности или действующего эффекта.
3. Не выбирать действие ради пользы игроку, наказания игрока, удобства сюжета, драматичности, обязательного конфликта или спасения NPC, исторически желательного результата; ни одна отдельная черта не абсолютная команда. Общие знания и приоритет переданного состояния — §14.2.
4. LLM не объявляет скрытые факты, contents закрытого контейнера, решение другого NPC, combat/conversation result, exact time или numeric domain effects.
5. Обычные физические результаты и ограниченные классы — по [`code_driven_world_materialization_architecture.md`](code_driven_world_materialization_architecture.md) §3A (presence / A1), без классового запрета в тексте промпта.
6. Инвариант D9 — как в `turn_step_llm_contract.md` §11.1 и в §18.1 этого документа: name — качественная метка; имена, даты, эмитент, текст грамоты и клеймо — только из пула кода или существующей сущности.
7. `mechanics_proposal` необязателен; финальные числа пишет код (§18.4).
8. Repair исправляет только формат и refs, не смысл решения.
