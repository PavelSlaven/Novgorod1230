...
function presenceAuthorityEnvelopeSchema() { return objectSchema({ stage: { const: 'resolve_presence' }, candidate: objectSchema({ semantic_type:stringSchema(), functional_bucket:enumSchema(ORDINARY_MATERIALIZATION_V1_ENUMS.functional_bucket), admission_class:enumSchema(ORDINARY_MATERIALIZATION_V1_ENUMS.admission_class), availability_class:enumSchema(ORDINARY_MATERIALIZATION_V1_ENUMS.availability_class), coverage_kind:stringSchema(), coverage_ref:stringSchema() }), allowed_supporting_bases:{ type:'array', items:objectSchema({ basis_ref:stringSchema(), basis_state:enumSchema(ORDINARY_MATERIALIZATION_V1_ENUMS.basis_state) }) }, selected_supporting_basis_ref:{ anyOf:[{type:'null'},stringSchema()] }, property_basis_ref:stringSchema(), placement_refs:stringArraySchema() }); }
