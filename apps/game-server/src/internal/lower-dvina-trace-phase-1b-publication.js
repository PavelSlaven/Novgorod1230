...
export async function loadLowerDvinaTracePhase1BPublication({ rootDir = process.cwd(), phase1AManifestDigest = null } = {}) {
  ...
  if (phase1AManifestDigest == null || phase1AManifestDigest === TRACE_REVISION26_PHASE_1A_MANIFEST_DIGEST) return loadLowerDvinaTraceRevision26Publication({ rootDir, phase1AManifestDigest }); if (phase1AManifestDigest === TRACE_NPC_ACTOR_STEP_PHASE_1A_MANIFEST_DIGEST) return loadLowerDvinaTraceNpcActorStepPublication({ rootDir, phase1AManifestDigest }); if (phase1AManifestDigest === TRACE_SPATIAL_SEMANTIC_PHASE_1A_MANIFEST_DIGEST) return loadLowerDvinaTraceSpatialSemanticPublication({ rootDir, phase1AManifestDigest }); if (phase1AManifestDigest === TRACE_SPATIAL_SEMANTIC_REVISION23_PHASE_1A_MANIFEST_DIGEST) return loadLowerDvinaTraceRevision23SpatialSemanticPublication({ rootDir, phase1AManifestDigest });
