...
The report separately indexes flora/fungi, soil/geology/material and medieval regional context shards by concept, claim scope and source. It measures the active broad profiles, before the later richness candidate. The 31 land-use and 39 place links have no exact G4 selector in the natural baseline and remain `unresolved_for_m2c_g4`; similarity of type names is insufficient. The approved richness candidate supplies bounded, explicitly inferred or analogical alternatives for the 32 exact G4 profiles. It has not been imported or activated; the active profiles still contain only broad categories.

**Remaining typed gaps:** `m2c_nature_successor_activation_required` (deterministic import, exact pin and readback of the approved G4 richness successor); `fauna_live_entity_owner_required` for an actual visible animal rather than ambient traces; `m2c_nature_exact_applicability_required` (source-backed land-use/place applicability to G4/G5); current water acoustic source state. No production import or database write occurred in this benchmark.
...
The candidate separates early-thirteenth-century Ryurik Gorodishche wet-ground pollen and regional archaeological wood/fauna context from contemporary Valdai habitat analogies and exact spatial templates. The pollen does not attest *Phragmites*, and none of these sources establishes a living taxon at an exact G4. Every transferred taxon is low-confidence `inferred` or `analogical`, with season and source limits. The shared version-1 category mapping (dominant/common/occasional/rare = 8/4/2/1) is an editorial preference among eligible alternatives, never a measured historical frequency, biological abundance or encounter chance. Independent Sol high approved the substantive richness rows in `nature-richness-data-approval.json`. The natural/presentation successor contains deterministic derived member fields; `nature-successor-data-approval.json` records the review of its authored content before those machine fields were added. The pure member selector has focused tests, but the successor is not imported, activated or presented in the game. Ambient fauna facts do not assert a currently present animal, and gathering any visible member still requires committed finite rows.
...
| Natural layers, season, night, light and weather | Active broad M2c natural baseline, presentation and placement; Temporal v4; separately approved G4 richness authoring candidate | Exact natural/Temporal readers | Spatial scene materialization and environment owners; pure successor member selector not yet wired to an active pin | Scene baseline, natural and environment rows | Canonical initial P22 natural projection | Canonical start PostgreSQL acceptance passes. Landscape openness→stable-cover candidate is data-approved at exact SHA `5b2fc71f25042e64ea5de8b3256caa33f264baeac4712af49207982a7f1ae41b`. The richness successor is not imported or activated; current member presentation and water acoustic source state remain gaps. |
...
| First-screen delivery | Committed spatial, natural, item and NPC refs | Current factual reader and visibility/knowledge owners | Opening Stage 22/23 and narration | Presentation delivery state | Player-visible opening | Isolated game-web start/ACK/look/replay/reload passed with a real configured provider on the forest start. In the generated-G5 UI attempt, the route panel exposed no exit after look; the run is recorded as blocked. Work and habitation starts have content approval only and have not entered UI acceptance. |
...
| Active | Isolated PostgreSQL import/readback and direct official-root start passed for bounded target fixtures. The old live schema v2 remains untouched after a P12 dry run found 939 conflicting primary keys. A partial v17 pair received schema, Gate1, P12 (12,359 rows) and appearance v3 (129 rows), with exact readbacks; it is not the final production artifact. Per owner NOTE-5/REVIEW-002, the complete v17 pair must be rebuilt from empty databases by one deterministic bootstrap through item/actor activation, then read back. No target item/actor production activation has occurred. |
...
and no gameplay/model run may claim full procedural baseline acceptance.
