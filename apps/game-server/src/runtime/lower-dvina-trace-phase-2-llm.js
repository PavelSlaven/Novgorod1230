    ...
    const call = {
        ...
        messages: [{
          ...
          content: [
            ...
            'For route or long-course movement, never emit a hand-written request_movement: select its supplied operation_choice. Local movement to a currently visible spatial_local_reference is the sole exception: copy the actor and that visible local ref into request_movement with movement_kind local. In particular destination_ref is not a movement field and route_ref is code-owned.',
            'A visible spatial_local_reference with visible_status inside means the actor is currently inside it. Leaving it or returning to its surrounding local place uses the same exact local request_movement target_ref; the unchanged broader location label does not mean the actor has already exited.',
            ...
            'If current player movement is blocked by the supplied body or combat state and no movement operation is available, answer an attempted movement as direct, not_achieved, semantic moment/none, no operations, no continuation, reason_code actor_movement_blocked. Do not claim movement or time passed.',
