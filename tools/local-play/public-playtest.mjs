...
const REQUIRED_ROLES = Object.freeze(['player_conversation_interpreter', 'npc_conversation_responder', 'npc_autonomous_decider', 'ordinary_materialization', 'world_process_step']);
...
export async function runPublicPlaytest({ start = startLocalPlay, fetchImpl = fetch, manifest = PUBLIC_PLAYTEST_MANIFEST, impossibleProbe = IMPOSSIBLE_PROBE, now = () => Date.now(), git = gitState, branch = PUBLIC_PLAYTEST_BRANCH, createRunIdentity = () => `public-playtest-run:${randomUUID()}`, log = console.log, stop = stopOwnedServer } = {}) {
  ...
  try {
    ...
    let branchOutcome = null;
    const turnsToRun = [...manifest, impossibleProbe];
    for (let index = 0; index < turnsToRun.length; index += 1) {
      const turn = turnsToRun[index];
      try { const result = await runTurn({ turn, partyId, http, now,
        scenarioSeed, runIdentity }); turns.push(result); if (result.presentation_pending) { turnFailure = { turn_id: turn.id, code: 'PUBLIC_PLAYTEST_PRESENTATION_PENDING' }; break; } if (!result.public_evidence.pass) { turnFailure = { turn_id: turn.id, code: 'PUBLIC_PLAYTEST_CAUSAL_EVIDENCE_MISSING' }; break; } if (missingTurnRoleEvidence(result).length) { turnFailure = { turn_id: turn.id, code: 'PUBLIC_PLAYTEST_PREREQUISITE_MISSING' }; break; } const alternateBranch = releaseManifestBranch(result, turn.branch); if (alternateBranch === false) { turnFailure = { turn_id: turn.id, code: 'PUBLIC_PLAYTEST_BRANCH_INVALID' }; break; } if (alternateBranch) { branchOutcome = alternateBranch; break; } const combatBoundaryReached = turn.scenario_class === 'combat' || (turn.combat_boundary_role_id != null && (result.role_ids.includes(turn.combat_boundary_role_id) || result.response.combat_state != null)); if (combatBoundaryReached) { const combatOutcome = releaseCombatBranch(result); if (combatOutcome == null) { turnFailure = { turn_id: turn.id, code: 'PUBLIC_PLAYTEST_COMBAT_BRANCH_INVALID' }; break; } if (combatOutcome === 'terminal_combat') { branchOutcome = combatOutcome; while (turnsToRun[index + 1]?.scenario_class === 'combat') index += 1; } else if (turnsToRun[index + 1]?.scenario_class !== 'combat') { branchOutcome = combatOutcome; break; } } if (isTerminal(result)) { stopReason = `terminal completion before remaining probes: ${turn.id}`; break; } }
    ...
    const report = buildReport({ gitSnapshot, finalGitSnapshot, scenarioSeed,
      ...
      stopReason, turnFailure, branchOutcome }); report.gates = roleGate(report);
...
function buildReport({ gitSnapshot, finalGitSnapshot, scenarioSeed, runIdentity, manifest, health, catalog, created, ack, screen, turns, stopReason, turnFailure, branchOutcome }) { return sanitizeReport({ schema: 'public_production_playtest_v1', generated_at: new Date().toISOString(), git: gitSnapshot, git_after: finalGitSnapshot, scenario_seed: scenarioSeed, run_identity: runIdentity, manifest_digest: manifestDigest(manifest), scenario_id: SCENARIO_ID, public_responses: { health: publicSummary(health), scenarios: publicSummary(catalog), new_game: publicSummary(created), opening_ack: publicSummary(ack), screen: publicSummary(screen) }, turns, observed_role_ids: [...new Set(turns.flatMap((turn) => turn.role_ids))].sort(), deterministic_proofs: DETERMINISTIC_PROOFS, latency: aggregateLatency(turns), branch_outcome: branchOutcome, stop_reason: stopReason, turn_failure: turnFailure, limitations: ['Only public HTTP responses and developer LLM reports are read.', `Exact impossible-action nearest realistic failure is proved by deterministic ${IMPOSSIBLE_DOMAIN_REJECTION_TEST}; the live probe runs only after the terminal-combat branch.`, 'An unreached combat branch uses its deterministic focused proof; a reached combat turn still requires npc_combat_decider live.', 'Planner and narration repair paths are deterministic focused proofs, not live waterfalls.'] }); }
...
export function roleGate(report) { const observed = new Set(report.observed_role_ids ?? []); const gaps = REQUIRED_ROLES.filter((role) => !observed.has(role)).map((role) => `required role not observed: ${role}`); if (report.stop_reason) gaps.push(report.stop_reason); if (report.turn_failure) gaps.push(`turn failure: ${report.turn_failure.turn_id}`); const finalCombatState = combatState(report.public_responses?.screen?.combat_state); if (report.branch_outcome === 'ongoing_combat' && (finalCombatState?.status !== 'paused_for_player' || finalCombatState.player_response_required !== true)) gaps.push('ongoing combat missing from final public screen'); for (const turn of report.turns ?? []) { const missing = missingTurnRoleEvidence(turn); if (missing.length) gaps.push(`missing compound prerequisite: ${turn.turn_id}: ${missing.join(', ')}`); const waterfall = missingWaterfallEvidence(turn); if (waterfall.length) gaps.push(`missing live waterfall: ${turn.turn_id}: ${waterfall.join(' -> ')}`); const attempts = turn.llm?.attempts?.length ? turn.llm.attempts : [turn.llm]; const repeatedRepair = attempts.map((attempt) => repeatedRepairRole(attempt?.waterfall)).find(Boolean); if (repeatedRepair) gaps.push(`repeated repair role: ${turn.turn_id}: ${repeatedRepair}`); if (turn.presentation_pending === true) gaps.push(`presentation pending: ${turn.turn_id}`); if (turn.status < 200 || turn.status >= 300) gaps.push(`unexpected non-2xx: ${turn.turn_id}`); if (turn.expected_failure && !turn.response?.turn_id) gaps.push(`impossible probe has no public committed turn: ${turn.turn_id}`); if (turn.expected_failure && !turn.role_ids?.includes('turn_step_planner')) gaps.push(`impossible probe planner not observed: ${turn.turn_id}`); } return Object.freeze({ pass: gaps.length === 0, gaps: Object.freeze(gaps), live_waterfalls: true, deterministic_proofs: DETERMINISTIC_PROOFS }); }
...
function releaseManifestBranch(turn, branch) { if (branch == null) return null; if (turn.status < 200 || turn.status >= 300 || !turn.response?.turn_id || turn.response?.screen_status !== 'ready' || turn.presentation_pending === true) return false; if (turn.response.option_id === branch.continue_option_id) return null; return turn.response.option_id === branch.alternate_option_id ? branch.alternate_outcome : false; }
export function releaseCombatBranch(turn = {}) { const state = combatState(turn.response?.combat_state); if (turn.status < 200 || turn.status >= 300 || !turn.response?.turn_id || turn.response?.screen_status !== 'ready' || turn.presentation_pending === true || state == null) return null; if (state.status === 'ended' && state.player_response_required === false) return 'terminal_combat'; if (state.status === 'paused_for_player' && state.player_response_required === true) return 'ongoing_combat'; return null; }
export function assertCurrentCatalog(catalog) { const scenarios = catalog?.scenarios; if (!Array.isArray(scenarios) || scenarios.filter(({ scenario_id: id }) => id === SCENARIO_ID).length !== 1 || scenarios.find(({ scenario_id: id }) => id === SCENARIO_ID)?.available !== true) throw gateError('PUBLIC_CATALOG_INVALID', 'Public catalog does not contain one available current scenario.'); }
...
function entry(id, scenario_class, raw_text, extra = {}) { return Object.freeze({
  id, scenario_class, raw_text, ...extra }); }
...
function combatState(value) { const status = text(value?.status); const playerResponseRequired = value?.player_response_required; return ['paused_for_player', 'ended'].includes(status) && typeof playerResponseRequired === 'boolean' ? { status, player_response_required: playerResponseRequired } : null; }
function branchOutcome(value) { const outcome = text(value); return ['precombat_unreached', 'terminal_combat', 'ongoing_combat'].includes(outcome) ? outcome : null; }
