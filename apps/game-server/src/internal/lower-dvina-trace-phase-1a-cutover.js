export async function loadLowerDvinaTracePhase1ACutover({
  ...
  const [
    ...
    readJson(rootDir, `${scenarioRoot}/phase-1a/manifest.json`),
    readJson(rootDir, `${scenarioRoot}/phase-0d-v3/manifest.json`),
    readJson(rootDir, `${scenarioRoot}/phase-0d-v2/manifest.json`),
    readJson(rootDir, `${scenarioRoot}/phase-0d-v2/definition.json`),
    readJson(
      rootDir,
      `${scenarioRoot}/phase-0d-v2/body-environment-profiles.json`
    ),
    readJson(
      rootDir,
      `${scenarioRoot}/phase-1a/materialization-bindings.json`
    ),
    readJson(rootDir, `${scenarioRoot}/phase-0d/manifest.json`)
  ]);
  ...
  if (phase1AManifest.schema
    ...
    || phase1AManifest.package_id !== 'lower_dvina_trace_phase_1a_v2'
    || phase1AManifest.revision !== 2
    || phase1AManifest.status !== 'approved'
    || phase1AManifest.scenario_id !== 'lower_dvina_trace_v1'
    || phase1AManifest.scenario_definition_revision !== 6
    ...
    fail(
      ...
      'Phase-1A revision 2 materialization manifest is incomplete.'
    );
  }
  if (correction.schema !== 'rus.trace_phase_0d_correction_manifest.v1'
    || correction.package_id !== 'lower_dvina_trace_phase_0d_v3'
    || correction.revision !== 3
    || correction.scenario_definition_revision !== 6
    || correction.publication_status !== 'unpublished'
    || correction.remaining_unresolved_refs?.length !== 0) {
    fail(
      'TRACE_SCENARIO_MANIFEST_INVALID',
      'Phase-0D correction manifest revision 3 is incomplete.'
    );
  }
  assertVersionedRawPin(
    ...
    supersededPhase1AManifestFile,
    {
      path: `${scenarioRoot}/phase-1a/manifest.json`,
  ...
  assertVersionedRawPin(
    ...
    supersededCorrectionManifestFile,
    {
      path: `${scenarioRoot}/phase-0d-v2/manifest.json`,
      id: 'lower_dvina_trace_phase_0d_v2',
      revision: 2,
      schema: 'rus.trace_phase_0d_correction_manifest.v1',
      idField: 'package_id'
    }
  ...
  assertRawPin(
    supersededCorrectionManifestFile.value.base_package_ref,
    baseManifestFile,
    `${scenarioRoot}/phase-0d/manifest.json`
  );
  assertVersionedRawPin(
    correction.superseded_definition_ref,
    supersededDefinitionFile,
    {
      path: `${scenarioRoot}/phase-0d-v2/definition.json`,
  ...
  assertVersionedRawPin(
    ...
    supersededBodyEnvironmentFile,
    {
      path: `${scenarioRoot}/phase-0d-v2/body-environment-profiles.json`,
      id: 'trace_ld_v1_body_environment_profiles',
      revision: 2,
      schema: 'rus.trace_body_environment_profiles.v1',
      idField: 'set_id'
    }
  );
  return {
...
function assertRawPin(ref, loaded, expectedPath) {
  if (ref?.path !== expectedPath || ref?.digest !== loaded.digest) {
    fail(
      'TRACE_SCENARIO_DEPENDENCY_MISMATCH',
      `Pinned dependency ${expectedPath} is stale.`
    );
  }
}
