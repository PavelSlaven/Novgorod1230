  ...
  const journal = {
    ...
    adaptStage(stage = {}, context = {}) {
      ...
      const base = {
        ...
        scope: stage.scope ?? context.scope ?? null,
        roleId: stage.roleId ?? stage.role_id ?? context.roleId ?? context.role_id ?? null,
        tierId: stage.tierId ?? stage.tier_id ?? context.tierId ?? context.tier_id ?? null,
        configHash: stage.configHash ?? stage.config_hash ?? context.configHash ?? context.config_hash ?? null,
        outputContractMode: stage.outputContractMode ?? stage.output_contract_mode ?? context.outputContractMode ?? null,
        ...
        recoveryClass: stage.recovery_class ?? null,
        repairTargetStage: stage.repair_target_stage ?? null,
        rerunFromStage: stage.rerun_from_stage ?? null,
        forbiddenLocalFix: stage.forbidden_local_fix ?? null,
        repairAttemptIndex: stage.repair_attempt_index ?? null,
        modelTier: stage.model_tier ?? null,
        terminalStatus: stage.terminal_status ?? null,
        failedGate: stage.failed_gate ?? null,
        firstFailedGate: stage.first_failed_gate ?? null,
        firstInvalidArtifact: stage.first_invalid_artifact ?? null,
        firstRegressionStage: stage.first_regression_stage ?? null,
        pipelineRuntime: stage.pipeline_runtime ?? stage.pipelineRuntime ?? context.pipelineRuntime ?? null,
        legacyProviderRuntimeUsed: stage.legacy_provider_runtime_used ?? stage.legacyProviderRuntimeUsed ?? context.legacyProviderRuntimeUsed ?? null
      };
    ...
    adaptCall(call = {}, context = {}) {
      ...
      const payload = {
        ...
        scope: call.scope ?? context.scope ?? null,
        roleId: call.roleId ?? call.role_id ?? context.roleId ?? context.role_id ?? null,
        tierId: call.tierId ?? call.tier_id ?? context.tierId ?? context.tier_id ?? null,
        configHash: call.configHash ?? call.config_hash ?? context.configHash ?? context.config_hash ?? null,
        outputContractMode: call.outputContractMode ?? call.output_contract_mode ?? context.outputContractMode ?? null,
        pipelineRuntime: call.pipelineRuntime ?? call.pipeline_runtime ?? context.pipelineRuntime ?? null,
        legacyProviderRuntimeUsed: call.legacyProviderRuntimeUsed ?? call.legacy_provider_runtime_used ?? context.legacyProviderRuntimeUsed ?? null,
...
export function normalizeDiagnosticEvent(input = {}) {
  ...
  const maxAttempts = resolveMaxAttemptsContract(input, attempt);
  return {
    ...
    roleId: input.roleId ?? input.role_id ?? null,
    tierId: input.tierId ?? input.tier_id ?? null,
    configHash: input.configHash ?? input.config_hash ?? null,
    outputContractMode: input.outputContractMode ?? input.output_contract_mode ?? null,
    ...
    recoveryClass: input.recoveryClass ?? input.recovery_class ?? null,
    repairTargetStage: input.repairTargetStage ?? input.repair_target_stage ?? null,
    rerunFromStage: input.rerunFromStage ?? input.rerun_from_stage ?? null,
    forbiddenLocalFix: input.forbiddenLocalFix ?? input.forbidden_local_fix ?? null,
    repairAttemptIndex: input.repairAttemptIndex ?? input.repair_attempt_index ?? null,
    modelTier: input.modelTier ?? input.model_tier ?? null,
    terminalStatus: input.terminalStatus ?? input.terminal_status ?? null,
    failedGate: input.failedGate ?? input.failed_gate ?? null,
    firstFailedGate: input.firstFailedGate ?? input.first_failed_gate ?? null,
    firstInvalidArtifact: input.firstInvalidArtifact ?? input.first_invalid_artifact ?? null,
    firstRegressionStage: input.firstRegressionStage ?? input.first_regression_stage ?? null,
    pipelineRuntime: input.pipelineRuntime ?? input.pipeline_runtime ?? null,
    legacyProviderRuntimeUsed: input.legacyProviderRuntimeUsed ?? input.legacy_provider_runtime_used ?? null,
    includeRawDetails: Boolean(input.includeRawDetails)
  };
}

function resolveMaxAttemptsContract(input = {}, attempt = null) {
  const maxAttempts = input.maxAttempts ?? null;
  if (attempt == null) return maxAttempts;
  if (maxAttempts !== null && maxAttempts !== undefined && maxAttempts !== '' && Number.isFinite(Number(maxAttempts))) {
    return Number(maxAttempts);
  }
  const error = new Error('RETRY_POLICY_CONTRACT_ERROR: maxAttempts is required when attempt is present.');
  error.code = 'RETRY_POLICY_CONTRACT_ERROR';
  throw error;
}
...
export function buildFailureSummary(events = [], options = {}) {
  ...
  return {
    ...
    scope: failed?.scope ?? lastSuccess?.scope ?? null,
    roleId: failed?.roleId ?? lastSuccess?.roleId ?? null,
    tierId: failed?.tierId ?? lastSuccess?.tierId ?? null,
    configHash: failed?.configHash ?? lastSuccess?.configHash ?? null,
    outputContractMode: failed?.outputContractMode ?? lastSuccess?.outputContractMode ?? null,
    ...
    pipelineRuntime: failed?.pipelineRuntime ?? lastSuccess?.pipelineRuntime ?? options.pipelineRuntime ?? null,
    legacyProviderRuntimeUsed: failed?.legacyProviderRuntimeUsed ?? lastSuccess?.legacyProviderRuntimeUsed ?? options.legacyProviderRuntimeUsed ?? null,
    firstFailedGate: failed?.firstFailedGate ?? failed?.failedGate ?? null,
    firstInvalidArtifact: failed?.firstInvalidArtifact ?? null,
    firstRegressionStage: failed?.firstRegressionStage ?? null,
    recoveryClass: failed?.recoveryClass ?? null,
    repairTargetStage: failed?.repairTargetStage ?? null,
    rerunFromStage: failed?.rerunFromStage ?? null,
    forbiddenLocalFix: failed?.forbiddenLocalFix ?? null,
    repairAttemptIndex: failed?.repairAttemptIndex ?? null,
    modelTier: failed?.modelTier ?? null,
    terminalStatus: failed?.terminalStatus ?? null,
    failedGate: failed?.failedGate ?? null,
