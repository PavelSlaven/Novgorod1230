...
test('buildLlmTurnReport makes deterministic waterfall and aggregates', () => {
  const report = buildLlmTurnReport({ party_id: 'party-1', request_id: 'turn-1', calls: [
    { role: 'intent_router', provider: 'deepseek', model: 'm', durationMs: 10, status: 'ok', configHash: 'a', tokenUsage: { prompt_tokens: 3, completion_tokens: 2 } },
    { role: 'turn_step_planner_repair', provider: 'openai_compatible', model: 'm2', durationMs: 30, status: 'parse_error', errorCategory: 'json_parse_failed', configHash: 'b', tokenUsage: { input_tokens: 5, output_tokens: 7 } },
  ...
  assert.deepEqual(report.aggregate, { calls: 3, success_rate: 2 / 3, parse_or_schema_failure_rate: 1 / 3, repair_rate: 1 / 3, llm_total_ms: 60, p50_ms: 20, p95_ms: 30, usage: { input_tokens: 8, output_tokens: 9 } });
});
...
test('turn context groups calls and excludes probe records', async () => {
  ...
  await diagnostics.runTurn({ party_id: 'party-1', request_id: 'turn-1' }, async () => {
    diagnostics.telemetry.onCall({ roleId: 'intent_router', status: 'ok', durationMs: 4 });
    await Promise.resolve();
    diagnostics.telemetry.onCall({ roleId: 'probe', call_type: 'probe', status: 'ok', durationMs: 1 });
  });
  assert.equal(diagnostics.report({ party_id: 'party-1', request_id: 'turn-1' }).aggregate.calls, 1);
