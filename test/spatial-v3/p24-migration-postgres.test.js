...
test('P24 party migration reads actual v2 relation, applies exact reviewed mapping, dry-runs and rolls back an earlier write on failure', async (t) => {
  ...
  let source_extract = await readV2PartySource(pool, { party_id:'p' });
  const source = source_extract.records.find((row) => row.source_identity.startsWith('party_runtime.party_positions:p:'));
  assert.ok(source, 'read-only adapter must enumerate actual v2 party row');
  const makeInventory = (extract) => buildSpatialV3MigrationInventory({ party_records:extract.records.map((row) => row.source_identity === source.source_identity
    ? { old_identity: row.source_identity, old_type: 'party_position', old_level: 'G5', target_contract: 'party_g5_site', action: 'migrate', reason: 'exact export', evidence: row.evidence, pin_mapping: row.pin_mapping, review_status: 'reviewed' }
    : { old_identity: row.source_identity, old_type: row.source_table, old_level: 'legacy_nonspatial_state', target_contract: 'immutable_v2_snapshot', action: 'keep', reason: 'retained only in immutable P24 source snapshot before P28', evidence: row.evidence, pin_mapping: row.pin_mapping, review_status: 'reviewed' }) });
  let inventory = makeInventory(source_extract); const record = { id:'site', source_identity:source.source_identity, evidence:source.evidence, classification:'canonical_projection', canonical_g5_ref:{ entity_id:'g5', authoring_version:1 }, pin_mapping:source.pin_mapping, target_site:{ id:'site', parent_g4_id:'g4'} };
  // A party snapshot is strictly party-scoped: another party cannot make this
  // migration fail or become part of the persisted source evidence.
  ...
  const extraSourceExtract = await readV2PartySource(pool, { party_id:'p' });
  assert.ok(extraSourceExtract.records.every((row) => !row.source_identity.includes(':q:')), 'other party source rows must not enter party p snapshot');
  const extra = await applySpatialV3PartyMigration(pool,{ inventory, source_extract:extraSourceExtract, party_id:'p', change_set_id:'cs-extra', g5_records:[record], dry_run:true });
  assert.equal(extra.ok,true);
  assert.equal((await pool.query("SELECT count(*)::int AS n FROM party_runtime.party_g5_sites")).rows[0].n,0, 'dry run cannot write target rows');
  ...
  source_extract = await readV2PartySource(pool, { party_id:'p' });
  inventory = makeInventory(source_extract);
  ...
  assert.equal((await pool.query("SELECT count(*)::int AS n FROM party_runtime.spatial_v3_migration_coverage_artifacts WHERE party_id='p'")).rows[0].n,1, 'accepted source snapshot/digests are persisted atomically');
  ...
  const fullDry = await applySpatialV3PartyMigration(pool,{inventory,source_extract:fullExtract,party_id:'p',change_set_id:'cs',g5_records:[g5],target_rows:dynamic,journeys:[journey],dry_run:true}); assert.equal(fullDry.ok,true);
  assert.equal((await pool.query("SELECT count(*)::int AS n FROM party_runtime.party_route_plans WHERE id='plan'")).rows[0].n,0);
  const full = await applySpatialV3PartyMigration(pool,{inventory,source_extract:fullExtract,party_id:'p',change_set_id:'cs',g5_records:[g5],target_rows:dynamic,journeys:[journey]});
  ...
  const missingApproval = await applySpatialV3PartyMigration(pool,{inventory,source_extract:fullExtract,party_id:'p',change_set_id:'cs-anchor-denied',journeys:[{...anchorJourney,anchor_approved:false}]});
  ...
  const anchored = await applySpatialV3PartyMigration(pool,{inventory,source_extract:fullExtract,party_id:'p',change_set_id:'cs-anchor',journeys:[anchorJourney]});
