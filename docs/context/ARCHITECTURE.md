...
| `legacy/` | карантин до-модульного runtime: `legacy/src`, `legacy/test` (`npm run test:legacy`), `legacy/scripts`, `legacy/DOCUMENTS`; новые функции в нём не создаются. `legacy/src` — живая зависимость production new-game через [legacy-adapter.js](../../packages/new-game/src/legacy-adapter.js); `legacy/DOCUMENTS` — зеркало `canonicalized_from_legacy` | [MODULE_RULES](../architecture/MODULE_RULES.md) п.10, [legacy/README.md](../../legacy/README.md) |
