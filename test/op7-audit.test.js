...
import { buildMechanicsProposal } from '../src/world/engine.js';
...
import { handlePlayerInput as handlePartyTurnInput } from './party-turn-test-helpers.js';
...
test('party turn pipeline leaves world intact when audit stage fails after mechanics preview', async () => {
  const world = createWorldState({ startText: 'переправа и двор' });
  const beforeClock = structuredClone(world.clock);
  const beforePosition = structuredClone(world.current_position);

  try {
    await assert.rejects(() => handlePartyTurnInput(world, 'иду к реке', {
      llmExecutors: {
        auditor() {
          throw new Error('audit failed after proposal');
        }
      }
    }), /audit failed after proposal/);
