...
function summarizeVisibleNpcs(world) {
  ...
  return (Array.isArray(world?.npcs) ? world.npcs : [])
    ...
    .map((npc) => ({
      name_or_label: npc?.name ?? 'НПС',
      visible_status: npc?.visibleStatus ?? npc?.status ?? null,
      mood: npc?.mood ?? null,
      source_ref: npc?.id ? `npc:${npc.id}` : null,
      actor_ref: npc?.id ? `npc:${npc.id}` : null,
...
function sanitizeMasterNarrative(narrative = {}) {
  ...
  return {
    ...
    npc_reaction_refs: Array.isArray(narrative.npc_reaction_refs) ? narrative.npc_reaction_refs.slice(0, 6) : [],
