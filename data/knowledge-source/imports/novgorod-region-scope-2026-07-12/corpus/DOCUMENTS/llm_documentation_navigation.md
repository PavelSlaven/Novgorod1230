...
Этот файл нужен для быстрой ориентации LLM, разработчика или аудитора в папке `DOCUMENTS`.

## Обязательное правило для Новгородской земли

При любой работе с Новгородской землёй около 1230 года первым профильным документом является `novgorod_region_scope_and_cell_workflow_1230.md`. Он имеет приоритет над прежними утверждениями о завершённости 70-ячеечной карты v6, старым перечнем G0-регионов и прежней трактовкой G3–G5.
...
→ профиль конкретной системы
```
 Он не заменяет профильные документы и не вводит новых правил. Если этот README расходится с профильным документом, приоритет имеет профильный документ.
