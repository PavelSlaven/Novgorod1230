...
The existing canonical scene materializer and normal G4 expansion owner remain
responsible for topology and subsequent selection. No Lower Dvina scenario,
invented historical site, extra structure or prior travel is transferred.

## Data review scope

- Exact target G4, entry, canonical G5, scene and arrival-position references.
- Editorial player premise, July 1230 date, name, body-profile transfer and
  explicit player applicability of the existing forest-hunting attribute profile.
- Explicit player transfer of the pinned NPC clothing draft. That draft's own
  approval is separate; its presence alone does not authorize player equipment.
- Initial canonical source-binding rule and limited initial sensory conditions.
  Natural placement approval is also separate. No active sound occurrence,
  harvesting stock, logging right or later-state sensory fallback is created.

Source pin checks and approved topology closure checks run with:
...
endpoint. Its database URLs are injected only into its child server process.
The operator requires the intended endpoint supplied in its own process through
the documented CLI arguments; no URL is inferred from local-play and no database
is provisioned as a substitute. Only after approved inputs and endpoint access
may the operator perform read-only preflight, reviewed imports, current-party
preflight, exact activation and readback. Default startup must change separately
after full gameplay readiness. Existing parties and v6 demo data remain pinned.
