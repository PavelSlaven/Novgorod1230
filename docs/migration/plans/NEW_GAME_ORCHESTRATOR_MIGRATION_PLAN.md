...
Собрать единый модульный управляющий слой для непрерывной цепочки new-game Stages 2–26, не возвращая доменную логику в общий файл и не позволяя оркестратору придумывать категории, исторические факты или отсутствующие варианты.

Оркестратор отвечает только за порядок выполнения, передачу явно собранных входов, фиксацию утверждённых артефактов, checkpoints, resume и маршрутизацию repair. Runtime instances создают stage-local code materializers; LLM выполняет только разрешённые bounded decisions, аудит и прозу.
