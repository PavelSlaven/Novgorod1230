...
export function createLowerDvinaTraceNarrationService({ roleRunner } = {}) {
  ...
  return createNarrationService({
    ...
    semanticRepairer: { repair: (request) => runNarrationRole(roleRunner,
      ...
      'Return only {"replacements":[{"prose":"<replacement in Russian or empty string>"}]} in the same order as supplied flagged segments. The server assembles version, schema, and immutable segment_id values. Repair only supplied flagged segments using their concerns, read-only nearby_context, player-safe visible_context, and style policy. Remove every unsupported claim in the flagged segment, including claims named by the concern. visible_context.visible_changes are confirmed player-safe facts and sufficient grounding; ensure the repaired full prose still conveys every material visible_change not already conveyed by nearby_context, placing any omitted supported change in the first replacement. For missing_visible_change, replace that segment with connected prose that naturally conveys the exact omitted visible_change and only other supported nearby meaning. For technical_presentation, preserve only supported meaning and rewrite it as connected, restrained literary Russian without field language, IDs, numeric-stat dumps, system reports, modern slang, or pseudo-archaic decoration. A faithful natural paraphrase of visible_context is allowed only when it replaces supported meaning from that segment or supplies an omitted visible_change; it must not add detail, causation, exact object use, success, or repeat facts already present in nearby_context. Empty visible_npc or visible_objects arrays do not prove that nobody or nothing is present, nor do they support silence or emptiness. A visible physical feature does not support an unstated sound, smell, temperature, bodily sensation, history, or recent use. If no supported meaning remains and no material visible_change needs restoration, use an empty prose string to delete the segment; never fill it with an unrelated scene fact. Return one replacement for each flagged segment and no others. Do not use player intent as evidence, hidden state, change neighboring segments, infer facts, add a fallback, or call any other role.',
      request) }
  });
}
