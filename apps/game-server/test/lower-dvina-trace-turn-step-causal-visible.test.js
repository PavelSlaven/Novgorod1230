...
]) test(`prepared steps keep speech before search result without prose timing (${query})`, async () => {
...
test('search duration stays out of narrator input', async () => {
  ...
  for (const duration of [1, 2, 5, 11, 21, 22, 111]) {
    ...
    assert.deepEqual(visible.visible_changes, ['Вы завершили поиск.']);
  }
});
...
for (const domainFallback of [false, true]) test(`materialized O1 precedes physical continuation (fallback=${domainFallback})`, async () => {
  ...
  if (!domainFallback) assert.deepEqual(visible.visible_changes, [found, physical]);
...
for (const [scene, detail, query] of [
  ...
  test(`charged discovery preserves committed scene and exact unresolved remainder: ${scene}`, async () => {
    ...
    assert.deepEqual(activity.visible_changes, []);
...
]) test(`applied durations stay in mechanics while prose keeps actions: ${sample.speech}`, async () => {
...
test('observation stays before later speech while duration stays out of prose', async () => {
  ...
  assert.deepEqual(visible.visible_changes.slice(0, 2), [
