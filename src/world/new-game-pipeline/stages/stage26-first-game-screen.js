...
} from './stage25-party-commit.js';
import { buildStage21Approval, STAGE22_APPROVAL_SCHEMA } from './stage22-narrator-prose.js';
import { computeNarratorStartingProseDigest } from './stage23-narrator-prose-audit.js';
...
export const STAGE26_APPROVAL_SCHEMA = 'stage26_screen_approval';
export const STAGE26_NARRATOR_APPROVAL_SCHEMA = 'narrator_prose_audit_approval';
...
export function computeStage26Digest(value) {
  return `sha256:${createHash('sha256').update(canonicalStage26Json(value)).digest('hex')}`;
}
...
export function buildNarratorProseApproval(stage23Result = {}) {
  const audit = isObject(stage23Result.narrator_prose_audit) ? stage23Result.narrator_prose_audit : {};
  return deepFreeze({
    version: 1,
    schema: STAGE26_NARRATOR_APPROVAL_SCHEMA,
    request_id: stage23Result.request_id ?? audit.request_id ?? null,
    pass: stage23Result.pass === true && audit.pass === true,
    narrator_output_digest: stage23Result.narrator_starting_prose_digest ?? null,
    visible_context_package_digest: stage23Result.visible_context_package_digest ?? null,
    repair_route: safeClone(stage23Result.repair_route ?? null),
    permissions: {
      can_show_to_player: stage23Result.commit_permission?.can_show_to_player === true,
      can_write_player_visible_message: stage23Result.commit_permission?.can_write_player_visible_message === true,
      can_mark_opening_scene_presented: stage23Result.commit_permission?.can_mark_opening_scene_presented === true
    }
  });
}
...
export function buildStage26Input({
  ...
  const input = {
    ...
    stage25_party_commit_approval: safeClone(stage25_party_commit_approval ?? buildStage25Approval(stage25)),
    ...
    visible_context_approval: safeClone(visible_context_approval ?? buildStage21Approval(stage21_result ?? {})),
...
export function buildStage26Approval(result = {}) {
  return deepFreeze({
    version: 1,
    schema: STAGE26_APPROVAL_SCHEMA,
    request_id: result.request_id ?? null,
    pass: result.pass === true,
    party_id: result.party_id ?? null,
    transaction_id: result.transaction_id ?? null,
    message_id: result.first_game_screen?.delivery_state?.message_id ?? null,
    screen_digest: result.screen_digest ?? null,
    postcommit_state_digest: result.postcommit_state_digest ?? null,
    permissions: safeClone(result.delivery_permission ?? {})
  });
}
...
function validateStage25ApprovalBinding(input) {
  ...
  if (!isObject(approval) || approval.version !== 1 || approval.schema !== STAGE25_APPROVAL_SCHEMA || approval.pass !== true || approval.commit_status !== 'committed') {
  ...
  if (approval.party_start_committed_digest !== computeStage25Digest(input.party_start_committed)) concerns.push(issue('FIRST_SCREEN_STAGE25_DIGEST_MISMATCH', 'Committed-state digest mismatch.', 'party_start_committed', 'hard_block'));
  if (approval.party_public_state_digest !== computeStage25Digest(input.committed_public_read_model)) concerns.push(issue('FIRST_SCREEN_PUBLIC_STATE_DIGEST_MISMATCH', 'Committed public read-model digest mismatch.', 'committed_public_read_model', 'hard_block'));
  return concerns;
}
...
function validateCommittedPublicReadModel(model, input) {
  const concerns = [];
  if (!isObject(model) || model.version !== 1 || model.schema !== STAGE25_PUBLIC_READ_MODEL_SCHEMA) return [issue('FIRST_SCREEN_PUBLIC_READ_MODEL_INVALID', `Committed ${STAGE25_PUBLIC_READ_MODEL_SCHEMA} version 1 is required.`, 'committed_public_read_model', 'hard_block')];
